...
-- Folio shared reading data is private and only accessed by the backend service role.
update media_items
...
create index if not exists idx_folio_highlights_book_chapter on folio_highlights(book_id, chapter_index, start_offset);
create index if not exists idx_folio_thoughts_highlight on folio_thoughts(highlight_id, created_at);
create index if not exists idx_folio_comments_thought on folio_comments(thought_id, created_at);
create index if not exists idx_folio_positions_book on folio_reading_positions(book_id, actor_type, actor_id);
