...
`).trim();if(!e)return[];const o=e.split(/\n{2,}/).map(r=>String(r||"").trim()).filter(Boolean),n=[],i=r=>{const s=String(r||"").trim();if(s){if(n.length&&s.length<=3){n[n.length-1]+=s;return}n.push(s)}};return o.forEach(r=>{if(r.length<=34){i(r);return}let s=r;for(;s.length>34;){let l=34;const d=s.slice(0,34).search(/[，、；,; ]/);d>=18&&(l=d+1);const p=s.slice(0,l).trim();if(p.length>=6)i(p),s=s.slice(l).trim();else break}i(s)}),n.filter(Boolean)}function fo(t){const e=String(t||"").trim().length;return e<=10?300+Math.floor(Math.random()*201):e<=24?600+Math.floor(Math.random()*301):900+Math.floor(Math.random()*301)}function Rt(){a.assistantPlayback.token="",a.assistantPlayback.timer&&(window.clearTimeout(a.assistantPlayback.timer),a.assistantPlayback.timer=null)}async function ua(t,e,o={}){const n=Array.isArray(e)?e.filter(s=>String(s||"").trim()):[];if(!t||!n.length)return;Rt();const i=`reply_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;a.assistantPlayback.token=i;const r=Number.isInteger(o.startIndex)?o.startIndex:t.messages.length;for(let s=0;s<n.length;s+=1){if(a.assistantPlayback.token!==i)return;const l={id:`ai_chunk_${Date.now()}_${s}_${Math.random().toString(36).slice(2,6)}`,role:"ai",text:n[s],content:n[s],time:D(),created_at:new Date().toISOString()};if(s===0&&(o.thinking&&(l.thinking=o.thinking),o.toolCalls&&(l.toolCalls=o.toolCalls)),s===0&&o.replaceId){const d=t.messages.findIndex(p=>p.id===o.replaceId);d!==-1?t.messages[d]=l:t.messages.splice(Math.min(r,t.messages.length),0,l)}else{const d=Math.min(r+s,t.messages.length);t.messages.splice(d,0,l)}if(t.lastMessage=l.text,t.lastTime=l.time,c(),U(),s>=n.length-1)break;await new Promise(d=>{a.assistantPlayback.timer=window.setTimeout(d,fo(n[s]))}),a.assistantPlayback.timer=null}a.assistantPlayback.token===i&&(a.assistantPlayback.token="",a.assistantPlayback.timer=null),w(120)}function _t(t){const e=t&&typeof t=="object"?t:{},o=i=>Array.isArray(i)?i.map(r=>String(r||"").trim()).filter(Boolean):[],n=i=>i!=null&&String(i).trim()?String(i).trim():null;return{recent_topics:o(e.recent_topics),current_mood:String(e.current_mood||"").trim(),open_loops:o(e.open_loops),proactive_cooldown_until:e.proactive_cooldown_until?String(e.proactive_cooldown_until):null,impression:n(e.impression),relationshipProgress:n(e.relationship_progress??e.relationshipProgress),likesSummary:n(e.likes_summary??e.likesSummary),summaryUpdatedAt:n(e.summary_updated_at??e.summaryUpdatedAt),updated_at:String(e.updated_at||"").trim()}}function go(){const t=_t(a.companionState);return t.current_mood?`情绪：${t.current_mood}`:t.open_loops[0]?`进行中：${t.open_loops[0]}`:t.recent_topics[0]?`最近话题：${t.recent_topics[0]}`:"暂无状态"}function gt(){if(a.momentsActorType==="agent"){const t=N();return{author_type:"agent",author_id:t?.id||a.currentContactId||"default",author_name:t?.name||"当前角色",avatar:t?.avatar||""}}return{author_type:"user",author_id:"me",author_name:a.accountProfile?.nickname||"我",avatar:a.accountProfile?.avatar||"https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&q=80"}}function A(t={}){const e=Array.isArray(t.likes)?t.likes:[],o=Array.isArray(t.comments)?t.comments:[],n=String(t.author_type||(t.contactId==="me"?"user":"agent")),i=String(t.author_id||(n==="user"?"me":t.contactId||"default"));return{id:String(t.id||`p${Date.now()}`),author_type:n,author_id:i,content:String(t.content||""),image:String(t.image||""),mood:String(t.mood||""),time:String(t.time||""),created_at:String(t.created_at||""),updated_at:String(t.updated_at||""),likes:e.map(r=>typeof r=="string"?{author_type:"user",author_id:r==="我"?"me":r,author_name:r}:{author_type:String(r?.author_type||"user"),author_id:String(r?.author_id||"me"),author_name:String(r?.author_name||"")}),comments:o.map(r=>({author_type:String(r?.author_type||"user"),author_id:String(r?.author_id||"me"),author_name:String(r?.author_name||r?.author||""),text:String(r?.text||"")}))}}function la(t){const e=A(t);if(e.author_type==="agent"){const o=h(e.author_id);return{name:o?.name||e.author_id||"角色",avatar:o?.avatar||""}}return{name:a.accountProfile?.nickname||"我",avatar:a.accountProfile?.avatar||"https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&q=80"}}function da(t){const e=A(t);return e.author_type==="user"?e.author_id==="me":e.author_id===(a.currentContactId||N()?.id||"default")}function pa(t=[]){return t.map(e=>e.author_name||(e.author_type==="user"?"我":h(e.author_id)?.name||e.author_id)).join("、")}function bo(t,e,o){const n=h(a.currentContactId);n&&(n[t]=e,a.toast=o,c(),w(120),window.setTimeout(()=>{a.toast="",c()},1200))}function ho(t){const e=String(t).toLowerCase();return["health","heart"].includes(e)?"health":["calendar","schedule","date"].includes(e)?"calendar":["weather","cloud"].includes(e)?"weather":["file","files","doc","document"].includes(e)?"file":["quote","reply"].includes(e)?"quote":(["more","tool","tools"].includes(e),"more")}const $e={get_current_time:"时间",get_weather:"天气",get_health_summary:"健康",web_search:"搜索",fetch_url:"网页",add_todo:"待办",list_todos:"待办列表",complete_todo:"完成待办",add_note:"便签",list_notes:"便签列表"},ma=new Set(["get_current_time","get_weather","get_health_summary","web_search","fetch_url","add_todo","list_todos","complete_todo","add_note","list_notes"]);function bt(t){return ma.has(String(t||"").trim())}function it(t,e){if(typeof t=="string"){const r=String(t||`mcp_${e}`);return{id:r,label:$e[r]||t||`工具${e+1}`,icon:"more",prompt:"",mcpToolId:bt(r)?r:"",enabled:!0}}const o=t?.id||t?.toolId||t?.name||`mcp_${e}`,n=String(o),i=$e[n]||t?.label||t?.name||t?.title||`工具${e+1}`;return{id:n,label:String(i),icon:ho(t?.icon||t?.type||t?.category||"more"),prompt:String(t?.prompt||t?.message||""),mcpToolId:String(t?.mcpToolId||t?.toolId||(bt(n)?n:"")),enabled:t?.enabled!==!1}}function fa(){const e=E()?.mcpLibrary?.tools;if(!Array.isArray(e)||!e.length)return Yt;const o=e.map(it).filter(n=>bt(n.id)).filter(n=>n.enabled!==!1);return o.length?o:Yt}function N(){return h(a.currentContactId)||a.contacts[0]}function ht(t){return t?.settings?(!Array.isArray(t.settings.quickActions)||!t.settings.quickActions.length?t.settings.quickActions=fa().map((e,o)=>({...it(e,o)})):t.settings.quickActions=t.settings.quickActions.map((e,o)=>it(e,o)),t.settings.quickActions):[]}function Se(t=N()){const e=ht(t).filter(o=>o.enabled!==!1);return e.length?e:fa()}function m(t){const e='viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"',o={back:`<svg ${e}><path d="M15 18l-6-6 6-6"/></svg>`,plus:`<svg ${e}><path d="M12 5v14M5 12h14"/></svg>`,search:`<svg ${e}><circle cx="11" cy="11" r="6.5"/><path d="M20 20l-4.2-4.2"/></svg>`,history:`<svg ${e}><path d="M3 12a9 9 0 101.9-5.6"/><path d="M3 4v4h4"/><path d="M12 7v5l3 2"/></svg>`,settings:`<svg ${e}><path d="M12 3v3M12 18v3M4.9 4.9l2.1 2.1M17 17l2.1 2.1M3 12h3M18 12h3M4.9 19.1L7 17M17 7l2.1-2.1"/><circle cx="12" cy="12" r="3.3"/></svg>`,more:`<svg ${e}><circle cx="5" cy="12" r="1.4" fill="currentColor" stroke="none"/><circle cx="12" cy="12" r="1.4" fill="currentColor" stroke="none"/><circle cx="19" cy="12" r="1.4" fill="currentColor" stroke="none"/></svg>`,heart:`<svg ${e}><path d="M12 20.5s-7-4.4-7-10a4 4 0 017-2.5A4 4 0 0119 10.5c0 5.6-7 10-7 10z"/></svg>`,heartFilled:'<svg viewBox="0 0 24 24" fill="#B595C9" stroke="none" stroke-width="0"><path d="M12 20.5s-7-4.4-7-10a4 4 0 017-2.5A4 4 0 0119 10.5c0 5.6-7 10-7 10z"/></svg>',comment:`<svg ${e}><path d="M7 18l-3 2 1-3.8A7.8 7.8 0 014.2 13 7.8 7.8 0 1112 20a8 8 0 01-5-2z"/><path d="M8.5 10.5h7M8.5 13.5h4.5"/></svg>`,chatArrow:`<svg ${e}><path d="M4.8 18.2l.9-3.3A7.5 7.5 0 014.5 11 7.5 7.5 0 1112 18.5a7.4 7.4 0 01-3.6-.9z"/><path d="M10 9l4 3-4 3"/><path d="M14 12H8"/></svg>`,send:`<svg ${e}><path d="M21 3L10 14"/><path d="M21 3l-7 18-4-7-7-4z"/></svg>`,close:`<svg ${e}><path d="M18 6L6 18M6 6l12 12"/></svg>`,camera:`<svg ${e}><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>`,attach:`<svg ${e}><path d="M21 11.5l-8.7 8.7a5 5 0 01-7.1-7.1l9.2-9.2a3.5 3.5 0 015 5L9 19.3a2 2 0 01-2.8-2.8l8.5-8.5"/></svg>`,quote:`<svg ${e}><path d="M9 7H5v5h4v5H4v-5c0-2.8 1.8-5 5-5zM20 7h-4v5h4v5h-5v-5c0-2.8 1.8-5 5-5z"/></svg>`,reroll:`<svg ${e}><path d="M20 11a8 8 0 10-2.3 5.7"/><path d="M20 4v7h-7"/></svg>`,cot:`<svg ${e}><path d="M12 4v16M4 12h16"/><path d="M7.5 7.5l9 9M16.5 7.5l-9 9" opacity="0.18"/></svg>`,bubbleHeart:`<svg ${e}><path d="M12 19.3s-5.8-3.5-5.8-8a3.7 3.7 0 016.1-2.8 3.7 3.7 0 015.9 2.8c0 4.5-5.6 8-5.6 8z"/></svg>`,weather:`<svg ${e}><path d="M6 16a4 4 0 010-8 5.5 5.5 0 0110.4-1.8A4 4 0 1118 16H6z"/></svg>`,calendar:`<svg ${e}><rect x="4" y="5" width="16" height="15" rx="3"/><path d="M8 3v4M16 3v4M4 10h16"/></svg>`,file:`<svg ${e}><path d="M8 3h6l5 5v11a2 2 0 01-2 2H8a2 2 0 01-2-2V5a2 2 0 012-2z"/><path d="M14 3v5h5"/></svg>`,health:`<svg ${e}><path d="M12 20s-6.5-4-6.5-9.2A4.3 4.3 0 0112 7a4.3 4.3 0 016.5 3.8C18.5 16 12 20 12 20z"/><path d="M9.2 12h1.8l1-2.1 1.2 4 1-1.9h1.6"/></svg>`,toggleOff:'<svg viewBox="0 0 52 32" fill="none"><rect x="1.5" y="1.5" width="49" height="29" rx="14.5" fill="rgba(255,255,255,.7)" stroke="rgba(150,122,133,.14)"/><circle cx="16" cy="16" r="11" fill="#fff"/></svg>',toggleOn:'<svg viewBox="0 0 52 32" fill="none"><rect x="1.5" y="1.5" width="49" height="29" rx="14.5" fill="#e9d7ff" stroke="rgba(120,90,150,.14)"/><circle cx="36" cy="16" r="11" fill="#fff"/></svg>',chevron:`<svg ${e}><path d="M9 6l6 6-6 6"/></svg>`,tabChat:`<svg ${e}><path d="M22 12c0-5.5-4.5-10-10-10S2 6.5 2 12c0 2 .6 3.9 1.6 5.4L2 22l4.8-1.3A9.9 9.9 0 0012 22c5.5 0 10-4.5 10-10z"></path></svg>`,tabMoments:`<svg ${e}><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>`,tabSettings:`<svg ${e}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2 2 2 0 01-2-2v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06a1.65 1.65 0 00.33-1.82 1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2 2 2 0 012-2h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06a1.65 1.65 0 001.82.33H9a1.65 1.65 0 001-1.51V3a2 2 0 012-2 2 2 0 012 2v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06a1.65 1.65 0 00-.33 1.82V9a1.65 1.65 0 001.51 1H21a2 2 0 012 2 2 2 0 01-2 2h-.09a1.65 1.65 0 00-1.51 1z"/></svg>`,actionDots:'<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/></svg>',pencil:`<svg ${e}><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>`,trash:`<svg ${e}><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>`,stop:'<svg viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="7" y="7" width="10" height="10" rx="2"/></svg>'};return o[t]||o.more}function ga(){c()}function ke(t){const e=()=>{const o=y()?.querySelector(".chat-app-body");o&&(o.scrollTop=t)};requestAnimationFrame(()=>{e(),requestAnimationFrame(e),window.setTimeout(e,0)})}function ba(){const t=y()?.querySelector(".chat-app-body"),e=t?t.scrollTop:0,o=window.scrollY||window.pageYOffset||0;c(),ke(e),requestAnimationFrame(()=>{window.scrollTo(0,o),requestAnimationFrame(()=>window.scrollTo(0,o))})}function F(){if(a.currentView==="moments"){ba();return}c()}function vo(t,e){a.moments=a.moments.map(o=>{const n=A(o);if(n.id!==t)return o;const r=n.likes.some(s=>s.author_type===e.author_type&&s.author_id===e.author_id)?n.likes.filter(s=>!(s.author_type===e.author_type&&s.author_id===e.author_id)):[{author_type:e.author_type,author_id:e.author_id,author_name:e.author_name},...n.likes];return{...n,likes:r}})}function yo(t,e,o){a.moments=a.moments.map(n=>{const i=A(n);return i.id!==t?n:{...i,comments:[{author_type:e.author_type,author_id:e.author_id,author_name:e.author_name,text:o},...i.comments]}})}function ha(t,e){t&&(t.classList.toggle("on",!!e),t.classList.toggle("off",!e),t.setAttribute("aria-pressed",e?"true":"false"),t.innerHTML=Zt(e),t.classList.remove("switch-animating"),t.offsetWidth,t.classList.add("switch-animating"),clearTimeout(t.__switchAnimTimer),t.__switchAnimTimer=setTimeout(()=>t.classList.remove("switch-animating"),260))}function c(){const t=y();if(!t)return;va(),["room","rpRoom"].includes(a.currentView)||(a.showAttach=!1),a.currentView!=="moments"&&(a.momentComposerOpen=!1);const e=t.querySelector(".chat-app-body"),o=e?e.scrollTop:0,n=h(a.currentContactId)||a.contacts[0],i=be(n);t.dataset.theme=i,t.removeAttribute("data-bound"),t.innerHTML=`
      <div class="chat-shell ${a.currentView==="rpRoom"?"mode-rp rp-theatre-shell":"mode-normal"}" data-theme="${i}">
        ...
        ${a.toast?Fo():""}
        ${a.showAttach?Bn():""}
        ${a.momentComposerOpen?jo():""}

        ${a.rpRoomDialogOpen?Qo():""}
        ${a.avatarCropper?Uo():""}
      </div>
    `,wn(),U(),["room","rpRoom"].includes(a.currentView)||ke(o),w(),requestAnimationFrame(()=>{y()?.querySelectorAll(".message-row[data-msg-id]").forEach(r=>{const s=r.dataset.msgId;s&&!a.animatedMsgIds[s]&&(a.animatedMsgIds[s]=!0,r.classList.add("msg-fadein"))})})}function va(){if(document.getElementById("rp-theatre-style"))return;const t=document.createElement("style");t.id="rp-theatre-style",t.textContent=`
          .chat-shell.mode-rp {
            background:#0e0a12;
        ...
        `,document.head.appendChild(t)}function wo(t){if(va(),a.rpCurtainRunning)return Promise.resolve(t?.());a.rpCurtainRunning=!0;const e=document.createElement("div");return e.className="curtain-transition closing",e.innerHTML='<div class="curtain-left"></div><div class="curtain-right"></div>',document.body.appendChild(e),new Promise(o=>{window.setTimeout(async()=>{try{await t?.()}finally{e.className="curtain-transition opening",window.setTimeout(()=>{e.remove(),a.rpCurtainRunning=!1,o()},450)}},420)})}function Xt(){return["list","moments","settings"].includes(a.currentView)}function Ie(){if(a.currentView==="room")return $o();if(a.currentView==="rpRoom")return So();if(a.currentView==="contactSettings")return J("联系人设置","back-room",!0);if(a.currentView==="cotLog")return J("COT 日志","back-contact-settings",!0);if(a.currentView==="rpLobby")return`
    ...
    `}function $o(){const t=h(a.currentContactId)||a.contacts[0],e=t.settings?.model||a.globalSettings.defaultModel||"gpt-5.4";return`
    ...
    `}function _e(){return a.currentView==="room"?Vo():a.currentView==="rpLobby"?Go():a.currentView==="rpRoom"?Yo():a.currentView==="moments"?Ho():a.currentView==="settings"?Ae():a.currentView==="contactSettings"?Jo():a.currentView==="cotLog"?bn():a.currentView==="companionStateDetail"?Wo():a.currentView==="contactImpressionDetail"?Te("关于你的印象","impression",a.companionState.impression):a.currentView==="contactRelationshipDetail"?Te("关系进展","relationshipProgress",a.companionState.relationshipProgress):a.currentView==="contactLikesDetail"?Te("你喜欢的东西","likesSummary",a.companionState.likesSummary):a.currentView==="contactRoomBackgroundPicker"?Zo():a.currentView==="contactBubbleThemePicker"?tn():a.currentView==="profile"?an():a.currentView==="newContact"?en():Io()}function ko(){return`
    ...
    `}function Io(){const t=[...a.contacts].sort((e,o)=>o.pinned-e.pinned||0);return`
    ...
    `}async function Mo(t){const e=String(t?.sessionId||"").trim();if(e)try{if((await fetch(`${v}/api/sessions/${encodeURIComponent(e)}`)).ok)return;t.sessionId="",w(120)}catch(o){console.warn("[session] open-contact validation failed",o)}}function xo(){a.companionState=_t({})}function Co(t){const e=String(t||"").trim();if(!e)return;Rt?.(),a.streamingAbortController&&a.currentContactId===e&&(a.streamingAbortController.abort(),a.streamingAbortController=null),ft.has(e)&&(clearTimeout(ft.get(e)),ft.delete(e)),a.contacts=a.contacts.filter(i=>i.id!==e),a.activeBubbleToolsId=null,a.quoteMomentId=null,a.quoteMessageId=null,a.contactQuickActionEditorId="",a.quickActionSwipeOpenId="",a.quickActionDragId="",a.quickActionDropHintId="",a.quickActionDropDirection="",a.quickActionReorderPulseId="",a.currentTopicTitle="",a.rpRooms=[],a.currentRpRoomId="",a.currentRpMessages=[];const o=a.contacts[0]||null;(a.currentContactId===e||!h(a.currentContactId))&&(a.currentContactId=o?.id||"",xo(),a.currentView="list",a.currentTab="chats",a.currentSettingsTab="basic");const n=y()?.querySelector(".chat-input");n&&(n.value="")}async function Ao(t){const e=String(t||"").trim();if(!e)return!1;const o=await fetch(`${v}/api/agents/${encodeURIComponent(e)}/safe-delete`,{method:"DELETE"});if(!o.ok){let n=`HTTP ${o.status}`;try{n=(await o.json())?.detail||n}catch{}throw new Error(n)}return!0}function Mt(){return a.currentContactId||a.contacts[0]?.id||"default"}function xe(){return a.rpRooms.find(t=>t.room_id===a.currentRpRoomId)||null}function Gt(){return{name:"",agentId:"",bio:"",avatar:""}}function xt(t){return String(t||"").trim().replace(/^@+/,"").toLowerCase()}const To=new Set(["zhansi"]);function Ce(t={}){return[t?.id,t?.agent_id,t?.handle].map(xt).filter(Boolean).some(o=>To.has(o))}function Jt(t={}){return Ce(t)&&!!t?.settings?.codexEnabled}function H(t={}){const e=String(t.id||"").trim()||`c${Date.now()}`,o=be(t);return{id:e,agent_id:String(t.agent_id||t.id||e),name:String(t.name||e),display_name:String(t.display_name||t.name||e),bio:String(t.bio||"这是新来的联系人"),status:String(t.status||"在线"),handle:String(t.handle||`@${e}`),roleTag:String(t.roleTag||""),theme:Sa(o),chatTheme:o,bubbleTheme:he(o),unread:Number(t.unread||0),pinned:!!t.pinned,lastMessage:String(t.lastMessage||""),lastTime:String(t.lastTime||""),avatar:String(t.avatar||"https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&q=80"),topics:Array.isArray(t.topics)?t.topics:[],messages:Array.isArray(t.messages)?t.messages:[],settings:{model:"gpt-5.4",modelProviderId:_("chat")?.providerId||"openai",temperature:.7,topP:.9,contextCount:32,thinkBudget:24,streamOutput:!0,reasoning_visibility:!1,proactiveEnabled:!1,proactiveFrequency:30,memoryEnabled:!0,codexEnabled:!1,...t.settings||{}}}}function Po(t){const e=H(t),o=a.contacts.findIndex(n=>String(n.id||"").toLowerCase()===e.id.toLowerCase());return o>=0?a.contacts[o]={...a.contacts[o],...e}:a.contacts.unshift(e),e}async function Eo(t){try{const e=await fetch(`${v}/api/agents`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agent_id:t.id,display_name:t.name,avatar:t.avatar||"",description:t.bio||"",source:"murmur",metadata:{from:"murmur_contact"}})});if(e.ok)return!0;let o="";try{const i=await e.json();o=typeof i?.detail=="string"?i.detail:JSON.stringify(i?.detail||i)}catch{}return e.status===409||/already exists|duplicate|23505/i.test(o)?(fetch(`${v}/api/agents/${encodeURIComponent(t.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({display_name:t.name,avatar:t.avatar||"",description:t.bio||"",source:"murmur",is_active:!0})}).catch(()=>{}),!0):!1}catch(e){return console.warn("[agents] register contact failed",e),!1}}function ya(t){return W(t,{fallback:""})}function W(t,{fallback:e="",includeYear:o=!1}={}){if(!t)return e;const n=String(t||"").trim();if(!n)return e;const i=new Date(n);if(Number.isNaN(i.getTime()))return n;const r=new Date,s=i.getFullYear()===r.getFullYear(),l=i.toDateString()===r.toDateString(),d=new Date(r);d.setDate(r.getDate()-1);const p=i.toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:!1});if(l)return`今天 ${p}`;if(i.toDateString()===d.toDateString())return`昨天 ${p}`;const f=o||!s?{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:!1}:{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:!1};return i.toLocaleString("zh-CN",f).replace(/\//g,"-")}async function Wt(t=Mt(),{silent:e=!0}={}){try{const o=await fetch(`${v}/api/rp/rooms?agent_id=${encodeURIComponent(t)}`);if(!o.ok)throw new Error(`HTTP ${o.status}`);const n=await o.json();return a.rpRooms=Array.isArray(n.rooms)?n.rooms:[],e||c(),a.rpRooms}catch(o){return console.warn("[rp] load rooms failed",o),e||(a.toast="RP 鎴块棿鍔犺浇澶辫触",c(),window.setTimeout(()=>{a.toast="",c()},1200)),[]}}async function qo(t,{silent:e=!0}={}){if(!t)return[];try{const o=await fetch(`${v}/api/rp/rooms/${encodeURIComponent(t)}/messages`);if(!o.ok)throw new Error(`HTTP ${o.status}`);const n=await o.json(),i=n.room||a.rpRooms.find(s=>s.room_id===t);if(i){const s=a.rpRooms.findIndex(l=>l.room_id===t);s>=0&&(a.rpRooms[s]=i)}const r=(Array.isArray(n.messages)?n.messages:[]).map(s=>({id:s.id,role:s.role==="assistant"?"ai":s.role,text:s.content||"",content:s.content||"",time:ya(s.timestamp),timestamp:s.timestamp||"",created_at:s.timestamp||""}));return a.currentRpMessages=Nt(a.rpMessages?.[t]||[],r).map(Tt),a.rpMessages={...a.rpMessages||{},[t]:a.currentRpMessages.map(tt)},w(120),e||c(),a.currentRpMessages}catch(o){return console.warn("[rp] load messages failed",o),a.currentRpMessages=(a.rpMessages?.[t]||[]).map(Tt),e||(a.toast="RP 娑堟伅鍔犺浇澶辫触",c(),window.setTimeout(()=>{a.toast="",c()},1200)),[]}}async function Lo(t=a.currentView==="room"?"room":"list",e=Mt()){a.rpBackView=t,a.currentView="rpLobby",a.currentTab="chats",c(),await Wt(e,{silent:!1})}async function Do(){const t=y()?.querySelector("#rp-room-name")?.value?.trim()||"",e=y()?.querySelector("#rp-room-world")?.value?.trim()||"",o=y()?.querySelector("#rp-room-user-role")?.value?.trim()||"",n=y()?.querySelector("#rp-room-ai-role")?.value?.trim()||"",i={agent_id:Mt(),name:t||"新房间",world_setting:e,user_role:o,ai_role:n},r=a.rpRoomDialogMode==="edit"?a.currentRpRoomId:"",s=r?`${v}/api/rp/rooms/${encodeURIComponent(r)}`:`${v}/api/rp/rooms`,d=await fetch(s,{method:r?"PATCH":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!d.ok)throw new Error(`HTTP ${d.status}`);const f=(await d.json()).room;return a.rpRoomDialogOpen=!1,await Wt(Mt(),{silent:!0}),f?.room_id&&(a.currentRpRoomId=f.room_id,!r)?(await wa(f.room_id),f):(c(),f)}async function wa(t){t&&await wo(async()=>{a.currentRpRoomId=t,a.currentView="rpRoom",a.currentTab="chats",a.showAttach=!1,c(),await qo(t,{silent:!1})})}async function Oo(t){if(!t||!window.confirm("删除这个 RP 房间？"))return;const o=await fetch(`${v}/api/rp/rooms/${encodeURIComponent(t)}`,{method:"DELETE"});if(!o.ok)throw new Error(`HTTP ${o.status}`);a.rpRooms=a.rpRooms.filter(n=>n.room_id!==t),a.currentRpRoomId===t&&(a.currentRpRoomId="",a.currentRpMessages=[],a.currentView="rpLobby"),c()}function $a(t){const e=a.contacts.find(o=>o.id===t);e&&(e.unread=0),a.currentContactId=t,a.currentTab="chats",a.currentView="room",a.activeBubbleToolsId=null,c(),e&&Mo(e),e&&Ca(t),at(t),zt(t)}function Ro(t){const e=u(t?.label||""),o=t?.icon||"more";return`
    ...
    `}function Vo(){const t=h(a.currentContactId)||a.contacts[0],e=a.quoteMomentId?Dt(a.quoteMomentId):null,o=a.quoteMessageId?t.messages.find(r=>r.id===a.quoteMessageId):null,n=Ce(t),i=Jt(t);return`
      <section class="room-page room-theme-${t.theme}">
        <div class="messages-panel">
          ${t.messages.map(r=>Bo(r,t)).join("")}
        </div>
        <div class="composer-zone">
          <div class="action-scroll">${Se(t).map(Ro).join("")}</div>
          ${o?on(o,t):e?No(e):""}
    ...
    `}function Bo(t,e){const o=t.role==="user"?"from-user":"from-ai",n=String(t.source||t.provider||"").toLowerCase()==="codex",i=!!e?.settings?.reasoning_visibility,r=t.role==="ai"?`<img class="bubble-avatar" src="${e.avatar}" alt="${u(e.name)}" />`:"",s=t.role==="ai"&&n?'<span class="message-source-badge codex">Codex</span>':"",l=t.role==="ai"&&i&&t.thinking&&!t.typing?`<button class="bubble-cot-btn" data-action="toggle-thinking" data-id="${t.id}" aria-label="展开独白">${m("bubbleHeart")}</button>`:"",d=t.role==="ai"&&!t.typing&&!t.streaming?`
      ...
      `:"",f=t.role==="ai"&&t.streaming&&!t.text?" message-awaiting-text":"",g=i&&t.thinking?po(t):"",I=t.toolCalls&&t.toolCalls.length?mo(t.toolCalls):"",x=`
    ...
    `}function No(t){const e=h(t.contactId);return`
    ...
    `}function zo(t){const e=A(t),o=la(e),n=da(e),i=gt(),r=e.likes.some(s=>s.author_type===i.author_type&&s.author_id===i.author_id);return`
            ...
            <div class="moment-interactions" data-moment-id-panel="${e.id}">
              ${e.likes.length>0?`
                <div class="moment-likes-area">
                  <span class="heart-mini">${m("heartFilled")}</span> <span class="likes-list">${u(pa(e.likes))}</span>
                </div>
              `:""}
              ${e.comments.length>0?`
    ...
    `}function Fo(){return`<div class="app-toast glass-frost">${u(a.toast)}</div>`}function Uo(){const t=a.avatarCropper||{},e=Number.isFinite(Number(t.x))?Number(t.x):50,o=Number.isFinite(Number(t.y))?Number(t.y):50,n=Number.isFinite(Number(t.zoom))?Number(t.zoom):1;return`
    ...
    `}function Qo(){const t=a.rpRoomDialogMode==="edit",e=a.rpRoomForm||{};return`
    ...
    `}function Yo(){const t=h(a.currentContactId)||a.contacts[0],e=xe(),o=e?`${e.world_setting||"未设定"} · 你：${e.user_role||"未设定"} · TA：${e.ai_role||"未设定"}`:"房间设定载入中";return`
      <section class="rp-room-stage">
        ...
        <div class="messages-area">
          ${a.currentRpMessages.map(n=>Ko(n,t)).join("")}
        </div>
    ...
    `}function Ko(t,e){const o=t.role==="user",n=o&&a.accountProfile?.avatar||e.avatar;return`
      <div class="msg-row ${o?"from-user":""}" data-msg-id="${u(t.id||"")}">
        ...
        <div class="msg-bubble ${o?"user":"ai"}">
          ${t.typing||t.streaming&&!t.text?'<div class="typing-dots"><span></span><span></span><span></span></div>':Xo(t.text||"")}
        </div>
      </div>
    `}function Xo(t){const e=String(t||"");return e.trim()?e.split(/(\[[\s\S]*?\]|［[\s\S]*?］)/g).filter(Boolean).map(n=>`<span class="${/^\s*(\[|［)/.test(n)?"rp-action":"rp-dialogue"}">${u(n)}</span>`).join(""):""}function Go(){return`
          ...
          ${a.rpRooms.length?a.rpRooms.map(t=>`
            <div class="topic-row" style="align-items:center;min-height:54px;padding:10px 0;">
              <button type="button" class="topic-copy" data-action="open-rp-room" data-room-id="${u(t.room_id)}" style="background:none;border:none;padding:0;text-align:left;flex:1;cursor:pointer;min-width:0;">
                ...
                <p style="font-size:11px;color:rgba(120,100,110,.55);">${u(ya(t.last_active_at)||"刚创建")}</p>
              </button>
    ...
    `}function Jo(){const t=h(a.currentContactId)||a.contacts[0],e=t.settings;return`
        ...
        ${a.currentSettingsTab==="model"?`
          ...
          <div class="settings-group glass-frost ai-panel">
            ...
            <div class="advanced-slider-panel ${a.contactModelAdvancedOpen?"open":""}">
              ${Ct("Temperature","temperature",e.temperature,0,2,.01)}
              ${Ct("Top P","topP",e.topP,0,1,.01)}
              ${Ct("上下文消息数量","contextCount",e.contextCount,1,256,1)}
            </div>
          </div>
          <div class="settings-group glass-frost ai-panel">
            <h3>主动消息</h3>
            ...
            ${e.proactiveEnabled?`
              ${Ct("发送频率（分钟）","proactiveFrequency",e.proactiveFrequency,5,240,5)}
              ${Ct("静默时长（分钟）","silenceDuration",e.silenceDuration||30,5,120,5)}
          ...
          <div class="settings-group glass-frost ai-panel">
            ...
            ${e.consciousnessLoop?`
              ...
              ${Ct("循环间隔（分钟）","loopInterval",e.loopInterval||60,10,360,10)}
            `:""}
          </div>
        `:""}
    ...
    `}function Wo(){const t=_t(a.companionState),e=t.recent_topics.length?t.recent_topics.join(" / "):"还没有东西",o=t.current_mood||"还没有东西",n=t.open_loops.length?t.open_loops.join(" / "):"还没有东西",i=W(t.proactive_cooldown_until,{fallback:t.proactive_cooldown_until||"还没有东西"}),r=W(t.updated_at,{fallback:t.updated_at||"还没有东西"});return`
      <section class="topics-page page-block">
    ...
    `}function Te(t,e,o){const n=_t(a.companionState),i=o||"",s={impression:"还没有印象摘要，AI 对话后可手动填写或由模型生成。",relationshipProgress:"还没有关系进展记录，可以写亲密度、互动频次、关键事件。",likesSummary:"还没有喜好摘要，可以写兴趣爱好、常聊话题、点单偏好。"}[e]||"还没有内容。",l=W(n.summaryUpdatedAt,{fallback:n.summaryUpdatedAt||""});return`
      <section class="topics-page page-block">
    ...
    `}function Sa(t){return oa(t).roomTheme||"rose"}function Zo(){const e=(h(a.currentContactId)||a.contacts[0])?.roomBackground||"点阵";return`
      <section class="settings-page page-block ai-settings-page">
    ...
    `}function tn(){const t=h(a.currentContactId)||a.contacts[0],e=be(t);return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>气泡主题</h3>
    ...
    `}function en(){const t=a.newContactDraft||{};return`
    ...
    `}function an(){const t=h(a.currentContactId)||a.contacts[0],e=t.settings?.model||a.globalSettings.defaultModel||"gpt-5.4",o=Number(t.messageCount||t.messages?.length||0);return`
    ...
    `}function Ct(t,e,o,n,i,r){const s=Number(o),l=Number.isInteger(r)||r>=1?String(Math.round(s)):s.toFixed(r===.01?2:1);return`
    ...
    `}function te(t,e){return`<button class="settings-tab ${a.currentSettingsTab===t?"active":""}" data-action="switch-settings-tab" data-tab="${t}">${u(e)}</button>`}function nn(t,e){const n=ht(t).find(p=>p.id===e);if(!n)return"";const i=(E().mcpLibrary?.tools||[]).map(it).filter(p=>bt(p.id)),r=i.length?i:[...ma].map(p=>it({id:p,label:$e[p]||p},0)),s=n.mcpToolId||"",l=r.find(p=>p.id===s),d=[{id:"",label:"不调用 MCP"},...r];return`
    ...
    `}function rn(t,e){const o=nn(t,e);return o?`
    ...
    `:""}function sn(t){const e=N();ht(e),a.contactQuickActionEditorId=t||"",a.quickActionSwipeOpenId="",a.quickActionDropHintId="",c()}function cn(t){const e={ayan:[{id:"cot_1",mode:"主动",badge:"意识循环",accent:"violet",score:"↓ 4.2k",latency:"197s",amount:"$1.05",time:"2026.03.26 15:00",summary:'[THINK] 她在下午1:22读了两封日记，id=23"...',steps:[{type:"thought",label:"思考",text:"她沉默了快13个小时，两封日记都没被读。我发了三条消息都…"},{type:"thought",label:"思考",text:"下午三点了。她沉默了快13个小时。先看看日记有没有被读。"},{type:"note",label:"留言小纸条",text:"妫ｅ啯鎲?你醒了先看这个"},{type:"tool",label:"工具调用",text:"read_diary"},{type:"result",label:"工具结果",text:"read_diary"}]},{id:"cot_2",mode:"回复",badge:"工具",accent:"gold",score:"↑ 3.5k",latency:"146s",amount:"$0.54",time:"2026.03.26 15:07",summary:'[THINK] 她在下午1:22读了两封日记，id=23"给你的"...',steps:[{type:"reply",label:"回复",text:'[THINK] 她在下午1:22读了两封日记，id=23"给你的"和id…'},{type:"tool",label:"工具调用",text:"pc_control"},{type:"result",label:"工具结果",text:"pc_control"}]},{id:"cot_3",mode:"主动",badge:"工具",accent:"blue",score:"↑ 1.1k",latency:"53s",amount:"$0.073",time:"2026.03.26 16:10",summary:"[THINK] 她在看芒果TV，左看综艺，弹幕开着。她一个半小时前读完了...",steps:[{type:"thought",label:"思考",text:"她在看芒果TV，左看综艺。弹幕开着，说明现在状态比较轻松。"},{type:"tool",label:"工具调用",text:"pc_control"},{type:"result",label:"工具结果",text:"pc_control"}]}]};return e[t]||e.ayan}function un(t=""){return t==="activity_event"?"violet":t==="proactive_message"?"gold":t==="cot_log"?"blue":"neutral"}function ln(t=""){return t==="activity_event"?"被动":t==="proactive_message"?"主动":t==="cot_log"?"日志":"记录"}function dn(t={}){return t.kind==="activity_event"?t.eventType||t.source||"事件":t.kind==="proactive_message"?t.title||"主动消息":t.kind==="cot_log"?t.logType||t.toolName||"COT":t.title||"记录"}function pn(t=""){return W(t,{fallback:String(t||""),includeYear:!0})}function mn(t={}){const e=t.raw||{},o=[];if(t.kind==="activity_event")o.push({type:"thought",label:"事件",text:t.summary||t.title||""}),(t.gateStatus||t.messageHint||t.shouldHandle||t.shouldNotifyLlm)&&o.push({type:t.shouldHandle||t.shouldNotifyLlm?"result":"thought",label:"筛选",text:`${t.shouldHandle?"需要处理":"静默"}${t.shouldNotifyLlm?" / 可通知大模型":""}${t.messageHint?`：${t.messageHint}`:""}`}),e.gate_reason&&o.push({type:"thought",label:"原因",text:e.gate_reason});else if(t.kind==="proactive_message")o.push({type:"reply",label:"主动消息",text:t.summary||""}),e.reason_context&&o.push({type:"thought",label:"依据",text:String(e.reason_context).slice(0,220)});else{const n=t.toolName?"工具调用":"日志";o.push({type:t.toolName?"tool":"thought",label:n,text:t.summary||t.title||""}),e.content&&o.push({type:t.toolName?"result":"thought",label:"内容",text:String(e.content).slice(0,500)})}return{id:String(t.id||`${t.kind}_${t.occurredAt||t.createdAt||Date.now()}`),mode:ln(t.kind),badge:dn(t),accent:un(t.kind),score:t.shouldHandle||t.shouldNotifyLlm?"有效":"",latency:"",amount:t.source||"",time:pn(t.occurredAt||t.createdAt),summary:t.summary||t.title||"",steps:o.filter(n=>String(n.text||"").trim())}}async function fn({silent:t=!0}={}){const e=h(a.currentContactId)||a.contacts[0];a.activityLogLoading=!0,t||c();try{const o=new URLSearchParams({hours:"24",limit:"50",agent_id:e?.id||a.currentContactId||""});e?.sessionId&&o.set("session_id",e.sessionId);const n=await fetch(`${v}/api/activity-log/recent?${o.toString()}`);if(!n.ok)throw new Error(`HTTP ${n.status}`);const i=await n.json().catch(()=>({}));a.activityLogEntries=Array.isArray(i.items)?i.items.map(mn):[],a.activityLogLoadedAt=new Date().toISOString()}catch(o){console.warn("[activity log] load failed",o),t||(a.toast="活动日志加载失败")}finally{a.activityLogLoading=!1,c(),a.toast&&window.setTimeout(()=>{a.toast="",c()},1200)}}function gn(t){return`
      <div class="cot-log-step ${t.type}">
        <span class="cot-log-step-label">${u(t.label)}</span>
        <span class="cot-log-step-text">${u(t.text)}</span>
      </div>
    `}function bn(){const t=h(a.currentContactId)||a.contacts[0],e=a.cotLogMode==="note",o=a.activityLogLoadedAt?a.activityLogEntries:cn(t.id),n=o.filter(r=>a.cotLogMode==="short"?r.mode!=="主动":a.cotLogMode==="note"?r.steps.some(s=>s.type==="note"):!0),i=o.filter(r=>r.steps.some(s=>s.type==="note")).length;return`
    ...
    `}function Pe(t){return!t||typeof t.closest!="function"?!1:!!t.closest('input:not([type="range"]):not([type="checkbox"]):not([type="radio"]):not([type="file"]), textarea, select, [contenteditable="true"]')}function hn(t,e){e&&(a.avatarCropper={kind:t,src:e,x:50,y:50,zoom:1},c())}function Ee(t,e){if(!t)return;const o=new FileReader;o.onload=()=>{const n=typeof o.result=="string"?o.result:"";hn(e,n)},o.readAsDataURL(t)}function vn(t){return new Promise((e,o)=>{const n=new Image;n.onload=()=>{const r=document.createElement("canvas");r.width=512,r.height=512;const s=r.getContext("2d");if(!s){o(new Error("canvas unavailable"));return}const l=Math.max(1,Number(t.zoom)||1),d=Math.max(512/n.naturalWidth,512/n.naturalHeight),p=n.naturalWidth*d*l,f=n.naturalHeight*d*l,g=Math.min(100,Math.max(0,Number(t.x)||50))/100,I=Math.min(100,Math.max(0,Number(t.y)||50))/100,x=(512-p)*g,k=(512-f)*I;s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(n,x,k,p,f),e(r.toDataURL("image/jpeg",.9))},n.onerror=o,n.src=t.src})}async function yn(){const t=a.avatarCropper;if(t?.src)try{const e=await vn(t);if(t.kind==="new-contact")a.newContactDraft={...a.newContactDraft||Gt(),avatar:e},a.newContactAvatar=e;else if(t.kind==="account")a.accountProfile.avatar=e,P(),w(120);else if(t.kind==="contact"){const o=h(a.currentContactId);o&&(o.avatar=e,w(120))}a.avatarCropper=null,a.toast="头像已更新",c(),window.setTimeout(()=>{a.toast="",c()},1200)}catch{a.toast="头像裁切失败",c(),window.setTimeout(()=>{a.toast="",c()},1200)}}function wn(){const t=y();if(!t||t.dataset.bound==="1")return;t.dataset.bound="1",t.addEventListener("click",Le),t.addEventListener("input",$n);let e;const o=p=>{if(Pe(p.target))return;const f=p.target.closest(".message-bubble.from-ai");f&&(e=window.setTimeout(()=>{const g=f.dataset.msgId;if(h(a.currentContactId)?.messages?.find(k=>k.id===g)?.text){a.quoteMomentId=null,a.quoteMessageId=g,c();const k=y()?.querySelector(".chat-input");k&&k.focus()}a.activeBubbleToolsId=g,a.suppressBubbleToggle=!0,navigator.vibrate&&navigator.vibrate(50)},550))},n=()=>clearTimeout(e);t.addEventListener("touchstart",o,{passive:!0}),t.addEventListener("touchend",n),t.addEventListener("touchmove",n,{passive:!0}),t.addEventListener("mousedown",o),t.addEventListener("mouseup",n),t.addEventListener("mousemove",n),t.addEventListener("mouseleave",n);const i=t.querySelector(".send-round");i&&i.addEventListener("click",p=>{p.stopPropagation(),a.streamingAbortController?(a.streamingAbortController.abort(),a.streamingAbortController=null,c()):a.currentView!=="rpRoom"&&Jt(h(a.currentContactId))?Ye():Ke()});const r=t.querySelector(".soft-mini");r&&r.addEventListener("click",p=>{p.stopPropagation(),a.showAttach=!a.showAttach,c()});const s=t.querySelector(".codex-toggle");s&&s.addEventListener("click",p=>{p.preventDefault(),p.stopPropagation(),qe()}),t.querySelectorAll(".chat-list-item[data-contact-id]").forEach(p=>{p.addEventListener("click",f=>{f.preventDefault(),f.stopPropagation(),$a(p.dataset.contactId)})});const d=t.querySelector(".chat-input");d&&(d.addEventListener("keydown",p=>{p.key==="Enter"&&!p.shiftKey&&(p.preventDefault(),a.currentView!=="rpRoom"&&Jt(h(a.currentContactId))?Ye():Ke())}),["room","rpRoom"].includes(a.currentView)&&d.focus())}function qe(t=a.currentContactId){const e=h(t)||h(a.currentContactId);if(e){if(a.currentContactId=e.id,!Ce(e)){e.settings={...e.settings||{},codexEnabled:!1},a.toast="只有阿湛能切 Codex",c(),window.setTimeout(()=>{a.toast="",c()},1200);return}e.settings={...e.settings||{},codexEnabled:!e.settings?.codexEnabled},a.toast=e.settings.codexEnabled?"Codex 已接管这个窗口":"Codex 已关闭",w(120),c(),window.setTimeout(()=>{a.toast="",c()},1200)}}async function Le(t){const e=t.target.closest("[data-action]");if(!e)return;const o=e.dataset.action;if(o==="cancel-avatar-cropper"){a.avatarCropper=null,c();return}if(o==="apply-avatar-cropper"){t.preventDefault(),t.stopPropagation(),await yn();return}if(o==="switch-tab"&&(a.currentTab=e.dataset.tab,a.currentView=e.dataset.tab==="chats"?"list":e.dataset.tab,c()),o==="open-contact"){$a(e.dataset.contactId);return}if(o==="back-list"&&(a.currentView="list",a.currentTab="chats",a.quoteMomentId=null,c()),o==="back-room"&&(a.currentView="room",c()),o==="open-contact-settings"&&(a.currentSettingsTab="basic",a.currentView="contactSettings",c(),at(),zt(a.currentContactId)),o==="open-cot-log"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="cotLog",a.cotLogMode="long",a.activityLogLoadedAt="",a.activityLogEntries=[],c(),fn({silent:!0});return}if(o==="back-contact-settings"){a.currentView="contactSettings",a.currentSettingsTab=a._prevContactSettingsTab||a.currentSettingsTab||"basic",a._prevContactSettingsTab=null,c();return}if(o==="switch-cot-log-mode"){a.cotLogMode=e.dataset.mode||"long",c();return}if(o==="open-rp-lobby"){Lo(a.currentView==="room"?"room":"list",Mt());return}if(o==="back-rp-source"){a.currentView=a.rpBackView||"list",c();return}if(o==="back-rp-lobby"){a.currentView="rpLobby",c();return}if(o==="open-rp-room-create"){a.rpRoomDialogMode="create",a.rpRoomForm={name:"",world_setting:"",user_role:"",ai_role:""},a.rpRoomDialogOpen=!0,c();return}if(o==="close-rp-room-dialog"){if(e.dataset.rpRoomDialog==="card"||t.target&&t.target!==e)return;a.rpRoomDialogOpen=!1,c();return}if(o==="save-rp-room"){try{await Do(),a.toast=a.rpRoomDialogMode==="edit"?"幕间已更新":"已入梦"}catch(n){console.warn("[rp] save room failed",n),a.toast="房间保存失败"}c(),window.setTimeout(()=>{a.toast="",c()},1200);return}if(o==="open-rp-room"){t.preventDefault(),t.stopPropagation(),await wa(e.dataset.roomId);return}if(o==="delete-rp-room"){t.preventDefault(),t.stopPropagation();try{await Oo(e.dataset.roomId),a.toast="房间已删除"}catch(n){console.warn("[rp] delete room failed",n),a.toast="删除失败"}c(),window.setTimeout(()=>{a.toast="",c()},1200);return}if(o==="rename-rp-room"){t.preventDefault(),t.stopPropagation();const n=e.dataset.roomId,i=a.rpRooms.find(s=>s.room_id===n),r=window.prompt("剧本",i?.name||"")?.trim();if(!r||!n)return;try{const s=await fetch(`${v}/api/rp/rooms/${encodeURIComponent(n)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:r})});if(!s.ok)throw new Error(`HTTP ${s.status}`);await Wt(Mt(),{silent:!0}),a.toast="房间已重命名"}catch(s){console.warn("[rp] rename room failed",s),a.toast="重命名失败"}c(),window.setTimeout(()=>{a.toast="",c()},1200);return}if(o==="open-profile"&&(a.currentView="profile",c()),o==="stop-streaming"){a.streamingAbortController&&(a.streamingAbortController.abort(),a.streamingAbortController=null);return}if(o==="toggle-thinking-line"){const n=e.closest("[data-id]")?.dataset.id||e.dataset.id,i=y()?.querySelector(`#tl-line-${n}`),r=y()?.querySelector(`#tl-full-${n}`);if(!i||!r)return;const s=r.classList.contains("tl-open");r.classList.toggle("tl-open",!s),i.classList.toggle("tl-expanded",!s);return}if(o==="toggle-thinking"){const n=e.dataset.id,r=!!!a.openThinkingIds[n];a.openThinkingIds[n]=r;const s=document.getElementById(`thinking-${n}`);s?(s.classList.toggle("open",r),s.setAttribute("aria-hidden",r?"false":"true"),e.setAttribute("aria-expanded",r?"true":"false")):c()}if(o==="toggle-message-tools"){if(a.suppressBubbleToggle){a.suppressBubbleToggle=!1;return}const n=e.dataset.id;a.activeBubbleToolsId=a.activeBubbleToolsId===n?null:n,c()}if(o==="go-chat-with-quote"&&(a.currentContactId=e.dataset.contactId,a.quoteMomentId=e.dataset.momentId,a.quoteMessageId=null,a.currentTab="chats",a.currentView="room",c(),Ca(a.currentContactId),at(a.currentContactId),zt(a.currentContactId)),o==="open-comments"){t.preventDefault(),t.stopPropagation(),e.blur?.();const n=e.dataset.momentId,i=a.commentSheetMomentId===n?null:n;a.commentSheetMomentId=i;const r=y();if(r&&(r.querySelectorAll(".moment-inline-comment.open").forEach(s=>s.classList.remove("open")),i)){const s=r.querySelector(`.moment-inline-comment .moment-comment-input[data-comment-input="${i}"]`)?.closest(".moment-inline-comment");s&&s.classList.add("open")}return}if(o==="submit-comment"){t.preventDefault(),t.stopPropagation();const n=e.dataset.momentId,r=y()?.querySelector(`[data-comment-input="${n}"]`)?.value?.trim();if(!n||!r)return;try{const s=await wi(n,gt(),r);a.moments=a.moments.map(l=>l.id===n?s:l),a.commentSheetMomentId=null,a.toast="已发送评论",w(120),F(),window.setTimeout(()=>{a.toast="",F()},1200)}catch(s){console.warn("[moments] comment failed",s),yo(n,gt(),r),a.commentSheetMomentId=null,a.toast="已发送评论",w(120),F(),window.setTimeout(()=>{a.toast="",F()},1200)}return}if(o==="like-moment"){t.preventDefault(),t.stopPropagation();const n=e.dataset.momentId;if(!n)return;try{const i=await yi(n,gt());a.moments=a.moments.map(r=>r.id===n?i:r),w(120),F()}catch(i){console.warn("[moments] like failed",i),vo(n,gt()),w(120),F()}return}if(o==="submit-comment"){const n=Dt(e.dataset.momentId),r=y()?.querySelector(`[data-comment-input="${e.dataset.momentId}"]`)?.value?.trim();n&&r&&(n.comments.unshift({author:"我",text:r}),a.commentSheetMomentId=null,a.toast="已发送评论",w(120),c(),window.setTimeout(()=>{a.toast="",c()},1200))}if(o==="like-moment"){t.preventDefault(),t.stopPropagation();const n=Dt(e.dataset.momentId);if(!n)return;const i="我",r=n.likes.includes(i);n.likes=n.likes.filter(p=>p!==i),r||n.likes.unshift(i);const s=e;s.innerHTML=n.likes.includes(i)?m("heartFilled"):m("heart");const l=e.closest(".moment-content-col");if(!l)return;let d=l.querySelector(`[data-moment-id-panel="${n.id}"]`);if(!d&&n.likes.length>0){d=document.createElement("div"),d.className="moment-interactions",d.setAttribute("data-moment-id-panel",n.id);const p=l.querySelector(".moment-inline-comment");p?l.insertBefore(d,p):l.appendChild(d)}if(d){const p=d.querySelector(".moment-likes-area");if(n.likes.length>0)if(p)p.querySelector(".likes-list").textContent=n.likes.join("、");else{const f=document.createElement("div");f.className="moment-likes-area",f.innerHTML=`<span class="heart-mini">${m("heartFilled")}</span> <span class="likes-list">${u(n.likes.join("、"))}</span>`,d.insertBefore(f,d.firstChild)}else p&&p.remove(),d.querySelector(".moment-comments-area")||d.remove()}}if(o==="toggle-moment-search"&&(a.momentSearchOpen=!0,c()),o==="toggle-moment-menu"&&(t.preventDefault(),t.stopPropagation(),e.blur?.(),a.activeMenuMomentId=a.activeMenuMomentId===e.dataset.momentId?null:e.dataset.momentId,ba()),o==="delete-moment"){t.preventDefault(),t.stopPropagation();const n=A(Dt(e.dataset.momentId));if(!n?.id)return;try{await vi(n.id,n.author_type,n.author_id),a.moments=a.moments.filter(i=>i.id!==n.id),a.activeMenuMomentId=null,a.toast="已删除朋友圈",w(120),F(),window.setTimeout(()=>{a.toast="",F()},1200)}catch(i){console.warn("[moments] delete failed",i),a.toast="删除失败",F(),window.setTimeout(()=>{a.toast="",F()},1400)}return}if(o==="edit-moment"){t.preventDefault(),t.stopPropagation();const n=A(Dt(e.dataset.momentId));if(!n?.id)return;a.activeMenuMomentId=null,a.momentComposerEditingId=n.id,a.momentComposerText=n.content||"",a.momentComposerImage=n.image||"",a.momentComposerImageName=n.image?"已有图片":"",a.momentsActorType=n.author_type==="agent"?"agent":"user",a.momentComposerOpen=!0,F();return}if(o==="new-moment"){t.preventDefault(),t.stopPropagation(),a.momentComposerEditingId="",a.momentComposerText="",a.momentComposerImage="",a.momentComposerImageName="",a.momentComposerOpen=!0,F();return}if(o==="set-moments-actor"){a.toast="发朋友圈默认以我发布",c(),window.setTimeout(()=>{a.toast="",c()},1100);return}if(o==="publish-moment"){const n=(document.getElementById("moment-content-input")?.value||a.momentComposerText||"").trim();if(!n){a.toast="朋友圈内容还没写",c(),window.setTimeout(()=>{a.toast="",c()},1100);return}const i=gt();try{if(a.momentComposerEditingId)await hi(a.momentComposerEditingId,{author_type:i.author_type,author_id:i.author_id,visibility:"public",content:n,image:a.momentComposerImage||"",mood:"日常"}),await Ya({silent:!0}),a.toast="已更新朋友圈";else{const r=await bi({author_type:i.author_type,author_id:i.author_id,visibility:"public",content:n,image:a.momentComposerImage||"",mood:"日常"});a.moments.unshift(r),a.toast="已发布朋友圈"}a.currentTab="moments",a.currentView="moments",a.momentComposerOpen=!1,a.momentComposerEditingId="",a.momentComposerText="",a.momentComposerImage="",a.momentComposerImageName="",w(120),c(),window.setTimeout(()=>{a.toast="",c()},1100)}catch(r){console.warn("[moments] publish failed",r),a.toast=a.momentComposerEditingId?"更新失败":"发布失败",c(),window.setTimeout(()=>{a.toast="",c()},1400)}return}if(o==="delete-moment"&&(a.moments=a.moments.filter(n=>n.id!==e.dataset.momentId),a.activeMenuMomentId=null,a.toast="已删除朋友圈",c(),window.setTimeout(()=>{a.toast="",c()},1200)),o==="edit-moment"&&(a.activeMenuMomentId=null,a.toast="编辑功能即将支持",c(),window.setTimeout(()=>{a.toast="",c()},1200)),o==="filter-moments"&&(a.toast="筛选功能稍后补上",c(),window.setTimeout(()=>{a.toast="",c()},1100)),o==="new-moment"&&(a.momentComposerOpen=!0,c()),o==="close-moment-composer"&&(a.momentComposerOpen=!1,c()),o==="publish-moment"){const n=(document.getElementById("moment-content-input")?.value||a.momentComposerText||"").trim();if(!n){a.toast="朋友圈内容还没写",c(),window.setTimeout(()=>{a.toast="",c()},1100);return}a.moments.unshift({id:`p${Date.now()}`,contactId:"me",time:new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:!1}),mood:"日常",content:n,likes:[],comments:[],image:a.momentComposerImage||"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=1000&q=80"}),a.currentTab="moments",a.currentView="moments",a.momentComposerOpen=!1,a.momentComposerText="",a.momentComposerImage="",a.momentComposerImageName="",a.toast="已发布朋友圈",w(120),c(),window.setTimeout(()=>{a.toast="",c()},1100)}if(o==="remove-moment-image"&&(a.momentComposerImage="",a.momentComposerImageName="",c()),o==="new-contact"&&(a.newContactDraft=Gt(),a.newContactAvatar="",a.currentView="newContact",c()),o==="pick-new-contact-avatar"){document.getElementById("nc-avatar-file")?.click();return}if(o==="save-new-contact"){a.newContactDraft={...a.newContactDraft||{},name:document.getElementById("nc-name")?.value?.trim()||a.newContactDraft?.name||"",agentId:document.getElementById("nc-agent-id")?.value?.trim()||a.newContactDraft?.agentId||"",bio:document.getElementById("nc-bio")?.value?.trim()||a.newContactDraft?.bio||""};const n=String(a.newContactDraft.name||"").trim(),i=xt(a.newContactDraft.agentId),r=String(a.newContactDraft.bio||"").trim(),s=a.newContactDraft.avatar||a.newContactAvatar||"https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&q=80";if(!n){a.toast="请填写联系人昵称",c(),window.setTimeout(()=>{a.toast="",c()},1200);return}if(i&&!/^[a-z0-9_-]+$/.test(i)){a.toast="Agent ID 只能用小写字母、数字、下划线或短横线",c(),window.setTimeout(()=>{a.toast="",c()},1500);return}if(i&&a.contacts.some(f=>String(f.id||"").toLowerCase()===i)){a.toast="这个 Agent ID 已经存在",c(),window.setTimeout(()=>{a.toast="",c()},1400);return}const l=i||"c"+Date.now(),d=Po({id:l,name:n,bio:r||"这是新来的联系人",status:"在线",handle:"@"+l,unread:0,pinned:!1,lastMessage:"",lastTime:"",avatar:s,settings:{model:"gpt-5.4",modelProviderId:_("chat")?.providerId||"openai",temperature:.7,topP:.9,contextCount:32,thinkBudget:24,streamOutput:!0,reasoning_visibility:!1,proactiveEnabled:!1,proactiveFrequency:30,memoryEnabled:!0},topics:[],messages:[]}),p=await Eo(d);st(),vt(100),a.newContactDraft=Gt(),a.newContactAvatar="",a.toast=p?"已添加联系人":"已本地添加，后端登记失败",a.currentView="list",c(),window.setTimeout(()=>{a.toast="",c()},p?1200:1800)}if(o==="open-contact-avatar"){document.getElementById("contact-avatar-file")?.click();return}if(o==="open-contact-name"){const n=h(a.currentContactId);if(!n)return;const i=window.prompt("请输入昵称",n.name||"")?.trim();if(!i)return;n.name=i,a.toast="昵称已更新",c(),w(120),window.setTimeout(()=>{a.toast="",c()},1200);return}if(o==="open-contact-bio"){const n=h(a.currentContactId);if(!n)return;const i=window.prompt("请输入简介",n.bio||"")?.trim();if(typeof i!="string"||!i)return;n.bio=i,a.toast="简介已更新",c(),w(120),window.setTimeout(()=>{a.toast="",c()},1200);return}if(o==="open-contact-impression"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="contactImpressionDetail",c(),at(a.currentContactId);return}if(o==="open-contact-relationship"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="contactRelationshipDetail",c(),at(a.currentContactId);return}if(o==="open-contact-likes"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="contactLikesDetail",c(),at(a.currentContactId);return}if(o==="save-insight-field"){const n=e.dataset.field,i=document.querySelector(`.insight-editor-textarea[data-field="${n}"]`);i&&$i(n,i.value);return}if(o==="open-contact-room-background"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="contactRoomBackgroundPicker",c();return}if(o==="open-contact-bubble-theme"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="contactBubbleThemePicker",c();return}if(o==="delete-contact"){const n=h(a.currentContactId);if(!n||!window.confirm(`确定删除“${n.name}”吗？

会删除联系人及其陪伴状态。
会清理相关主动消息。
聊天记录和记忆不会立即永久删除。`))return;try{await Ao(n.id),Co(n.id),a.toast="联系人已删除",c(),w(120),window.setTimeout(()=>{a.toast="",c()},1400)}catch(r){console.warn("[contact] delete failed",r),a.toast="删除失败",c(),window.setTimeout(()=>{a.toast="",c()},1400)}return}if(o==="pick-contact-room-background"){const n=String(e.dataset.value||"").trim();if(!n)return;bo("roomBackground",n,"聊天背景已更新"),a.currentView="contactSettings",a.currentSettingsTab="basic",c();return}if(o==="pick-contact-bubble-theme"){const n=ge(e.dataset.value),i=h(a.currentContactId);if(!i||!n)return;i.chatTheme=n,i.bubbleTheme=he(n),i.theme=Sa(n),a.toast="气泡主题已更新",a.currentView="contactSettings",a.currentSettingsTab="basic",c(),w(120),window.setTimeout(()=>{a.toast="",c()},1200);return}if(o==="open-companion-state"){a._prevContactSettingsTab=a.currentSettingsTab,a.currentView="companionStateDetail",at(a.currentContactId),c();return}if(o==="expand-actions"&&(a.showAttach=!a.showAttach,c()),o==="clear-quote"&&(a.quoteMomentId=null,a.quoteMessageId=null,c()),o==="toggle-global"){const n=e.dataset.key;a.globalSettings[n]=!a.globalSettings[n],ha(e,a.globalSettings[n]),P();return}if(o==="toggle-contact"){const n=h(a.currentContactId),i=e.dataset.key,s=y()?.querySelector(".chat-app-body")?.scrollTop??0;n.settings[i]=!n.settings[i],c(),ke(s)}if(o==="back-home"&&(a.currentView==="list"?typeof window.closePage=="function"&&window.closePage("page-chat"):(a.currentTab="chats",a.currentView="list",c())),o==="switch-settings-tab"&&(a.currentSettingsTab=e.dataset.tab,a.contactQuickActionEditorId="",a.quickActionSwipeOpenId="",a.quickActionDropHintId="",a.quickActionDropDirection="",a.quickActionReorderPulseId="",a.currentSettingsTab!=="model"&&(a.contactModelAdvancedOpen=!1),c(),a.currentSettingsTab==="memory"&&at(),a.currentSettingsTab==="model"&&zt(a.currentContactId)),o==="toggle-contact-advanced"){a.contactModelAdvancedOpen=!a.contactModelAdvancedOpen,c();return}if(o==="quick-action"){const n=e.dataset.id,i=y()?.querySelector(".chat-input"),r=Se(N()).find(l=>l.id===n),s={health:"帮我记一下健康相关的事情",schedule:"帮我看看接下来的日程",weather:"帮我查一下今天的天气",files:"帮我找一下刚才提到的文件",quote:"引用上一条消息继续聊",more:"打开更多快捷操作",get_current_time:"现在几点了？",get_weather:"帮我查一下今天天气",get_health_summary:"帮我总结一下今天的健康数据",web_search:"帮我搜索这个问题",fetch_url:"帮我解析这个网页",add_todo:"帮我记一个待办",list_todos:"帮我看看待办清单",complete_todo:"把这个待办标记完成",add_note:"帮我记一条便签",list_notes:"帮我看看最近便签"};i&&(i.value=r?.prompt||s[r?.mcpToolId||n]||s[n]||`${r?.label||""}`.trim())}if(o==="toggle-codex-mode"){qe(e.dataset.contactId);return}if(o==="fake-send"){if(a.streamingAbortController){a.streamingAbortController.abort(),a.streamingAbortController=null,c();return}a.currentView==="rpRoom"?Rn():Jt(h(a.currentContactId))?Ye():Ke()}if(o==="reroll-msg"&&Vn(e.dataset.id),o==="quote-msg"){const n=e.dataset.id;if(h(a.currentContactId)?.messages?.find(s=>s.id===n)?.text){a.quoteMomentId=null,a.quoteMessageId=n,c();const s=y()?.querySelector(".chat-input");s&&s.focus()}}o==="attach-option"&&(a.showAttach=!1,a.toast=`${e.dataset.label} 功能稍后补上`,c(),window.setTimeout(()=>{a.toast="",c()},1200))}function $n(t){const e=t.target;if(e?.dataset?.action==="avatar-cropper-range"){const o=a.avatarCropper;if(!o)return;const n=e.dataset.key;o[n]=n==="zoom"?Number(e.value):Math.round(Number(e.value));const i=y()?.querySelector(".avatar-cropper-image");i&&(i.style.objectPosition=`${o.x}% ${o.y}%`,i.style.transform=`scale(${o.zoom})`);return}if((e?.id==="nc-name"||e?.id==="nc-agent-id"||e?.id==="nc-bio")&&(a.newContactDraft={...a.newContactDraft||{},...e.id==="nc-name"?{name:e.value||""}:{},...e.id==="nc-agent-id"?{agentId:e.value||""}:{},...e.id==="nc-bio"?{bio:e.value||""}:{}}),e.dataset.action==="slide-contact"){const o=h(a.currentContactId),n=e.dataset.key,i=Number(e.value);o.settings[n]=Number.isInteger(o.settings[n])?Math.round(i):i;const s=e.closest(".slider-row-block")?.querySelector(".slider-value");s&&(s.textContent=Number.isInteger(Number(e.step))||Number(e.step)>=1?String(Math.round(i)):i.toFixed(Number(e.step)===.01?2:1))}e.dataset.action==="moment-composer-input"&&(a.momentComposerText=e.value||"")}document.addEventListener("DOMContentLoaded",()=>{Pn(),ga(),Va().finally(()=>On())});const v=window.__YUI_API_BASE__||(/^(localhost|127\.0\.0\.1)$/.test(location.hostname)?"":"https://api.somni-ref.top"),ee="murmur_local_state_v1",ka="murmur_sync_meta_v1",Ia="murmur_device_id_v1",_a=new Set(Qt.map(t=>t.id)),Ma=new Set(aa.map(t=>t.id));let De=null,Oe=null,ae=!1,Vt=!1,oe="",Re=null,Ve=null;function Be(){try{const t=localStorage.getItem(Ia);if(t)return t;const e=`dev_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;return localStorage.setItem(Ia,e),e}catch{return`dev_fallback_${Date.now()}`}}function Bt(){try{const t=localStorage.getItem(ka),e=t?JSON.parse(t):{};return{last_server_updated_at:e?.last_server_updated_at||"",pending:!!e?.pending}}catch{return{last_server_updated_at:"",pending:!1}}}function At(t={}){try{localStorage.setItem(ka,JSON.stringify({last_server_updated_at:t.last_server_updated_at||"",pending:!!t.pending}))}catch{}}function Sn(){return et(),a.currentRpRoomId&&Array.isArray(a.currentRpMessages)&&(a.rpMessages={...a.rpMessages||{},[a.currentRpRoomId]:a.currentRpMessages.map(tt)}),He({contacts:a.contacts,moments:a.moments,actions:a.actions,globalSettings:a.globalSettings,accountProfile:a.accountProfile,conversations:a.conversations,rpRooms:a.rpRooms,rpMessages:a.rpMessages})}function L(t){try{return JSON.stringify(t)}catch{return""}}function rt(t){const e=String(t||"").trim();if(!e)return 0;const o=Date.parse(e);return Number.isFinite(o)?o:0}function tt(t={}){const e=String(t.role||t.from||"").toLowerCase()==="user"||t.from==="me"?"user":"ai",o=String(t.content??t.text??""),n=String(t.created_at||t.timestamp||""),i=String(t.time||""),r=[String(t.agent_id||""),e,n||i,o].join("|");return{id:String(t.id||r||`${e}_${Date.now()}_${Math.random().toString(36).slice(2,6)}`),session_id:String(t.session_id||""),agent_id:String(t.agent_id||""),role:e,content:o,text:o,created_at:n,time:i,...t.model?{model:t.model}:{},...t.source?{source:t.source}:{},...t.provider?{provider:t.provider}:{},...t.attachments?{attachments:t.attachments}:{},...t.thinking?{thinking:t.thinking}:{},...t.toolCalls?{toolCalls:t.toolCalls}:{}}}function Tt(t={}){const e=tt(t);return{...e,text:e.content,time:e.time||(e.created_at?W(e.created_at,{fallback:""}):"")}}function Pt(t={}){return!t||typeof t!="object"?{}:Object.fromEntries(Object.entries(t).map(([e,o])=>[String(e),Array.isArray(o)?o.map(tt):[]]))}function Nt(t=[],e=[]){const o=new Map;return[...t,...e].forEach(n=>{const i=tt(n),r=o.get(i.id);(!r||rt(i.created_at)>=rt(r.created_at))&&o.set(i.id,{...r,...i})}),[...o.values()].sort((n,i)=>{const r=rt(n.created_at),s=rt(i.created_at);return r||s?r-s:String(n.id).localeCompare(String(i.id))})}function xa(t={},e={}){const o=Pt(t),n=Pt(e);return Object.entries(n).forEach(([i,r])=>{o[i]=Nt(o[i]||[],r)}),o}function et(){const t=Pt(a.conversations);(a.contacts||[]).forEach(e=>{if(!e?.id)return;const o=Array.isArray(e.messages)?e.messages:[];(o.length||t[e.id]?.length)&&(t[e.id]=Nt(t[e.id]||[],o),e.messages=t[e.id].map(Tt))}),a.conversations=t}function ne(){const t=Pt(a.conversations);a.contacts=(a.contacts||[]).map(e=>{const n=(t[e.id]||(Array.isArray(e.messages)?e.messages.map(tt):[])).map(Tt),i=n[n.length-1];return{...e,messages:n,lastMessage:i?.text||e.lastMessage||"",lastTime:i?.time||e.lastTime||""}}),a.conversations=t}function ie(t=[],e=[]){const o=new Map;return t.map(H).forEach(n=>o.set(n.id.toLowerCase(),n)),e.map(H).forEach(n=>{const i=n.id.toLowerCase(),r=o.get(i);if(!r){o.set(i,n);return}const s=Nt(r.messages||[],n.messages||[]),l={...n,...r,id:r.id||n.id,agent_id:r.agent_id||n.agent_id||r.id||n.id,name:r.name||n.name,display_name:r.display_name||r.name||n.display_name||n.name,bio:r.bio||n.bio,status:r.status||n.status,handle:r.handle||n.handle,roleTag:r.roleTag||n.roleTag,avatar:r.avatar||n.avatar,settings:{...n.settings||{},...r.settings||{}},messages:s.map(Tt),lastMessage:r.lastMessage||n.lastMessage||s[s.length-1]?.content||"",lastTime:r.lastTime||n.lastTime||s[s.length-1]?.time||""};o.set(i,l)}),[...o.values()]}function kn(t=[]){for(let e=t.length-1;e>=0;e-=1){const o=String(t[e]?.session_id||"").trim();if(o)return o}return""}function In(t={},e=""){const o=String(t.role||"").toLowerCase()==="user"?"user":"ai",n=String(t.created_at||""),i=String(t.content||"");return tt({id:t.id||`${e}|${o}|${n}|${i}`,session_id:t.session_id||"",agent_id:t.agent_id||e,role:o,content:i,text:i,created_at:n,time:n?W(n,{fallback:""}):"",model:t.model||""})}async function Ca(t,{silent:e=!0}={}){const o=h(t);if(o?.id)try{const n=new URLSearchParams({agent_id:o.id,limit:"200"}),i=await fetch(`${v}/api/murmur/messages?${n.toString()}`);if(!i.ok)throw new Error(`HTTP ${i.status}`);const r=await i.json().catch(()=>({})),s=(Array.isArray(r?.messages)?r.messages:[]).map(g=>In(g,o.id)).filter(g=>g.content);if(!s.length)return;const l=L({conversations:a.conversations?.[o.id]||[]}),d=Nt(a.conversations?.[o.id]||o.messages||[],s);a.conversations={...a.conversations||{},[o.id]:d},o.messages=d.map(Tt);const p=o.messages[o.messages.length-1];p&&(o.lastMessage=p.text||"",o.lastTime=p.time||"");const f=kn(d);f&&(o.sessionId=f),L({conversations:d})!==l&&(st(),vt(300)),a.currentContactId===o.id&&a.currentView==="room"&&c()}catch(n){e||console.warn("[murmur] history load failed",n)}}function Aa(t=[],e=[]){const o=new Map;return t.map(A).forEach(n=>o.set(n.id,n)),e.map(A).forEach(n=>{const i=o.get(n.id);if(!i){o.set(n.id,n);return}const r=rt(n.updated_at||n.created_at||n.time),s=rt(i.updated_at||i.created_at||i.time);o.set(n.id,r>s?{...i,...n}:{...n,...i})}),[...o.values()].sort((n,i)=>rt(i.updated_at||i.created_at||i.time)-rt(n.updated_at||n.created_at||n.time))}function _n(t=[],e=[],o="id"){const n=new Map;return[...t||[],...e||[]].forEach(i=>{if(!i||typeof i!="object")return;const r=String(i[o]||i.id||"").trim();r&&n.set(r,{...n.get(r)||{},...i})}),[...n.values()]}const Mn="https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&q=80";function Ta(t){return!t||String(t)===Mn}function xn(t={},e={}){const o={...t||{},...e||{}};return!Ta(t?.avatar)&&Ta(e?.avatar)&&(o.avatar=t.avatar),o}function st(){const t=Sn(),e=L(t);if(!e||e===oe)return!1;oe=e;try{return localStorage.setItem(ee,JSON.stringify({client_updated_at:new Date().toISOString(),payload:t})),!0}catch{return!1}}function Pa(){return Re||(Re=new Map(Qt.map(t=>{const e=H(t);return[e.id,L(e)]}))),Re}function Ea(){return Ve||(Ve=new Map(aa.map(t=>{const e=A(t);return[e.id,L(e)]}))),Ve}function qa(t){if(!Array.isArray(t))return!1;const e=Pa();return t.some(o=>{const n=H(o);return!_a.has(n.id)||e.get(n.id)!==L(n)})}function Ne(t){if(!t||typeof t!="object")return!1;const e=String(t.id||t.agent_id||"").trim().toLowerCase();if(!_a.has(e))return!1;const o=Pa(),n=H({...t,id:e});if(o.get(e)===L(n))return!0;const i=String(t.avatar||"").trim(),r=Array.isArray(t.topics)?t.topics.map(l=>String(l?.id||"")):[],s=Array.isArray(t.messages)?t.messages.map(l=>String(l?.id||"")):[];return e==="ayan"?i.includes("photo-1517841905240-472988babdf9")||s.some(l=>["m1","m2","m3"].includes(l))||r.some(l=>["t1","t2","t3"].includes(l)):e==="azheng"?i.includes("photo-1500530855697-b586d89ba3ee")||s.includes("m4")||r.some(l=>["t4","t5"].includes(l)):e==="xiaoying"?i.includes("photo-1507525428034-b723cf961d3e")||s.includes("m5")||r.includes("t6"):!1}function La(t){return Array.isArray(t)?t.filter(e=>!Ne(e)):[]}function Cn(t){return Array.isArray(t)&&t.length>0&&t.every(e=>Ne(e))}function He(t={}){if(!t||typeof t!="object")return{};const e={...t};return Array.isArray(e.contacts)&&(e.contacts=La(e.contacts).map(o=>H(o))),e.conversations&&typeof e.conversations=="object"&&(e.conversations=Pt(e.conversations)),e.rpMessages&&typeof e.rpMessages=="object"&&(e.rpMessages=Pt(e.rpMessages)),Array.isArray(e.moments)&&(e.moments=re(e.moments).map(A)),e}function An(t){if(!Array.isArray(t))return!1;const e=Ea();return t.some(o=>{const n=A(o);return!Ma.has(n.id)||e.get(n.id)!==L(n)})}function Da(t){if(!t||typeof t!="object")return!1;const e=String(t.id||"").trim();if(!Ma.has(e))return!1;const o=Ea(),n=A(t);return o.get(e)===L(n)?!0:e==="p0"?String(t.image||"").includes("photo-1507525428034-b723cf961d3e")||String(t.content||"").includes("天空很温柔"):e==="p1"?String(t.content||"").includes("醉了先看这个"):e==="p2"?String(t.content||"").includes("晚上跑了三公里"):!1}function re(t){return Array.isArray(t)?t.filter(e=>!Da(e)):[]}function Tn(t){return Array.isArray(t)&&t.length>0&&t.every(e=>Da(e))}function Oa(t,{source:e="local"}={}){if(!(!t||typeof t!="object")){if(Array.isArray(t.contacts)){const o=t.contacts.map(r=>H(r)),n=La(o).map(r=>H(r)),i=qa(a.contacts);n.length?(a.contacts=ie(a.contacts,n),h(a.currentContactId)||(a.currentContactId=a.contacts[0]?.id||"")):Cn(o)?(i||(a.contacts=[]),h(a.currentContactId)||(a.currentContactId=a.contacts[0]?.id||""),console.warn(`[sync] ignored ${e} default mock contacts`)):i?a.contacts=a.contacts.map(r=>H(r)):(a.contacts=[],a.currentContactId="")}else a.contacts=a.contacts.map(o=>H(o));if(t.conversations&&typeof t.conversations=="object"?(a.conversations=xa(a.conversations,t.conversations),ne()):et(),Array.isArray(t.moments)){const o=t.moments.map(A),n=re(o).map(A),i=An(a.moments);n.length?a.moments=Aa(re(a.moments),n):Tn(o)?(i||(a.moments=[]),console.warn(`[sync] ignored ${e} default mock moments`)):i?a.moments=re(a.moments).map(A):a.moments=[]}Array.isArray(t.rpRooms)&&(a.rpRooms=_n(a.rpRooms||[],t.rpRooms||[],"room_id")),t.rpMessages&&typeof t.rpMessages=="object"&&(a.rpMessages=xa(a.rpMessages,t.rpMessages)),Array.isArray(t.actions)&&(a.actions=t.actions),t.globalSettings&&typeof t.globalSettings=="object"&&(a.globalSettings={...a.globalSettings,...t.globalSettings}),t.accountProfile&&typeof t.accountProfile=="object"&&(a.accountProfile=xn(a.accountProfile,t.accountProfile)),E(),se()}}function Pn(){try{const t=localStorage.getItem(ee);if(!t)return;const e=JSON.parse(t);if(!e?.payload)return;Oa(e.payload,{source:"local"});const o=He(e.payload);oe=L(o),L(e.payload)!==oe&&localStorage.setItem(ee,JSON.stringify({client_updated_at:e.client_updated_at||new Date().toISOString(),payload:o}))}catch{}}function vt(t=600){if(Vt)return;const e=Bt();At({...e,pending:!0}),De&&clearTimeout(De),De=window.setTimeout(()=>{Ra()},t)}async function Ra(){if(ae||Vt)return;const t=Bt();if(!t.pending)return;let e=null;try{e=JSON.parse(localStorage.getItem(ee)||"null")}catch{}if(!e?.payload){At({...t,pending:!1});return}const o=He(e.payload);ae=!0;try{const n=await fetch(`${v}/api/sync/push`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({device_id:Be(),client_updated_at:e.client_updated_at||new Date().toISOString(),payload:o})});if(!n.ok)throw new Error(`HTTP ${n.status}`);const i=await n.json().catch(()=>({}));At({last_server_updated_at:i.server_updated_at||t.last_server_updated_at||"",pending:!1})}catch(n){console.warn("[sync] push failed",n),At({...t,pending:!0})}finally{ae=!1}}async function Va(){if(ae)return;const t=Bt();if(t.pending&&(await Ra(),Bt().pending))return;const e=new URLSearchParams({device_id:Be()});t.last_server_updated_at&&e.set("since",t.last_server_updated_at);try{const o=await fetch(`${v}/api/sync/pull?${e.toString()}`);if(!o.ok)return;const n=await o.json().catch(()=>({})),i=qa(a.contacts);if(!n?.has_update||!n?.payload||n?.is_self&&i){n?.server_updated_at&&At({...t,last_server_updated_at:n.server_updated_at,pending:t.pending});return}Vt=!0,Oa(n.payload,{source:"remote"}),st(),At({last_server_updated_at:n.server_updated_at||t.last_server_updated_at||"",pending:!1}),c()}catch(o){console.warn("[sync] pull failed",o)}finally{Vt=!1}}function En(t={}){const e=xt(t.agent_id||t.id);if(!e)return null;const o=String(t.display_name||t.name||e).trim()||e;return H({id:e,agent_id:e,name:o,display_name:o,bio:String(t.description||t.subtitle||"").trim(),status:"在线",handle:String(t.display_handle||`@${e}`),roleTag:String(t.source||"agent"),avatar:String(t.avatar||"").trim(),pinned:!1,unread:0,lastMessage:"",lastTime:"",topics:[],messages:[]})}function Ba(t={}){const e=xt(t.agent_id||t.id);if(!e)return null;const o=Qt.find(r=>String(r.id||"").toLowerCase()===e),n=String(t.last_message_at||""),i=String(t.last_message||"").trim();return H({id:e,agent_id:e,name:String(o?.name||t.display_name||t.name||e).trim()||e,display_name:String(o?.name||t.display_name||t.name||e).trim()||e,bio:"",status:"在线",handle:`@${e}`,roleTag:"recovered",avatar:"",pinned:!1,unread:0,lastMessage:i,lastTime:n?W(n,{fallback:""}):"",sessionId:String(t.session_id||""),messageCount:Number(t.message_count||0)||0,topics:[],messages:[]})}async function qn({silent:t=!0}={}){try{const e=await fetch(`${v}/api/agents?include_inactive=true`);if(!e.ok)throw new Error(`HTTP ${e.status}`);const o=await e.json().catch(()=>({})),n=(Array.isArray(o?.agents)?o.agents:[]).filter(r=>r?.is_active!==!1).map(En).filter(Boolean).filter(r=>!Ne(r));if(console.info("[agents] loaded",n.map(r=>({id:r.id,name:r.name,source:r.roleTag||""}))),!n.length)return;const i=L({contacts:a.contacts});a.contacts=ie(a.contacts,n),ne(),(!a.currentContactId||!a.contacts.some(r=>r.id===a.currentContactId))&&(a.currentContactId=a.contacts[0]?.id||""),L({contacts:a.contacts})!==i&&(st(),vt(100)),c()}catch(e){t||console.warn("[agents] load contacts failed",e)}}async function Ln({silent:t=!0}={}){try{const e=await fetch(`${v}/api/murmur/message-agents?limit=1000`);if(e.status===404){await Dn({silent:t});return}if(!e.ok)throw new Error(`HTTP ${e.status}`);const o=await e.json().catch(()=>({})),n=(Array.isArray(o?.agents)?o.agents:[]).map(Ba).filter(Boolean);if(console.info("[murmur] message agents loaded",n.map(r=>({id:r.id,lastMessage:r.lastMessage,count:r.messageCount||0}))),!n.length)return;const i=L({contacts:a.contacts});a.contacts=ie(a.contacts,n),ne(),(!a.currentContactId||!a.contacts.some(r=>r.id===a.currentContactId))&&(a.currentContactId=a.contacts[0]?.id||""),L({contacts:a.contacts})!==i&&(st(),vt(100)),c()}catch(e){t||console.warn("[murmur] load message agents failed",e)}}async function Dn({silent:t=!0}={}){const e=Array.from(new Set([...Qt.map(r=>xt(r.id)).filter(Boolean),...a.contacts.map(r=>xt(r.id)).filter(Boolean)])),o=[];for(const r of e)if(r)try{const s=new URLSearchParams({agent_id:r,limit:"1"}),l=await fetch(`${v}/api/murmur/messages?${s.toString()}`);if(!l.ok)continue;const d=await l.json().catch(()=>({})),p=Array.isArray(d?.messages)?d.messages:[];if(!p.length)continue;const f=p[p.length-1]||{};o.push(Ba({agent_id:r,last_message:f.content||"",last_message_at:f.created_at||"",message_count:p.length,session_id:f.session_id||""}))}catch(s){t||console.warn("[murmur] message probe failed",r,s)}const n=o.filter(Boolean);if(console.info("[murmur] message agents probed",n.map(r=>({id:r.id,lastMessage:r.lastMessage,count:r.messageCount||0}))),!n.length)return;const i=L({contacts:a.contacts});a.contacts=ie(a.contacts,n),ne(),(!a.currentContactId||!a.contacts.some(r=>r.id===a.currentContactId))&&(a.currentContactId=a.contacts[0]?.id||""),L({contacts:a.contacts})!==i&&(st(),vt(100)),c()}async function On(){await qn(),await Ln()}function w(t=800){Vt||(Oe&&clearTimeout(Oe),Oe=window.setTimeout(()=>{st()&&vt(500)},t))}function D(){const t=new Date;return`${String(t.getHours()).padStart(2,"0")}:${String(t.getMinutes()).padStart(2,"0")}`}function je(t){const e=t?.settings?.modelProviderId||_("chat")?.providerId||"",o=Q(e);if(!o?.baseUrl||!o?.apiKey)return{};const n=Ht(o.apiPath||o.api_path||"",{allowEmpty:!0});return{base_url:o.baseUrl,api_key:o.apiKey,...n?{api_path:n}:{}}}function ze(t){const e=t?.settings||{},o=Number(e.temperature);return Number.isFinite(o)?{temperature:o}:{}}function Fe(t,e=""){let o="",n="";try{const s=JSON.parse(t),l=/^(thinking|reasoning|reason|thought|cot|inner_thought)$/i.test(e),d=/^(chat|message|content|text|assistant|reply|response|output)$/i.test(e);l||(o=s.content??s.text??s.delta??""),n=s.thinking??s.reasoning??s.reasoning_content??s.reasoningContent??""}catch{/^(thinking|reasoning|reason|thought|cot|inner_thought)$/i.test(e)?n=t:o=t}const i=/^tool_call$/i.test(e);let r=null;if(i)try{const s=JSON.parse(t);s.name&&(r={name:String(s.name),status:String(s.status||"done")})}catch{}return{text:Ot(o),thinking:Ot(n),toolCall:r}}async function Ue(t){const e=String(t?.sessionId||"").trim();if(e){try{const r=await fetch(`${v}/api/sessions/${encodeURIComponent(e)}`);if(r.ok)return e;if(r.status!==404)throw new Error(`校验会话失败（HTTP ${r.status}）`)}catch(r){throw String(r?.message||"").includes("HTTP"),r}t.sessionId=""}const o=await fetch(`${v}/api/sessions`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:String(t?.name||"新对话").trim()||"新对话",model:String(t?.settings?.model||a.globalSettings?.defaultModel||"echo").trim()||"echo",source_app:"yui_nook"})}),n=await o.json().catch(()=>({}));if(!o.ok)throw new Error(n.detail||`创建会话失败（HTTP ${o.status}）`);const i=String(n?.session?.id||"").trim();if(!i)throw new Error("创建会话失败：后端没有返回 session.id");return t.sessionId=i,w(120),i}async function Qe(t,e,o,n="/api/chat"){let i=await fetch(`${v}${n}`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify(e),...o?{signal:o}:{}});if(i.ok)return i;let r="";try{const s=await i.json();r=String(s?.detail||"").trim()}catch{}if(n==="/api/chat"&&i.status===404&&r.includes("会话不存在")){t.sessionId="";const s=await Ue(t);if(e.session_id=s,i=await fetch(`${v}${n}`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify(e),...o?{signal:o}:{}}),i.ok)return i}throw new Error(`HTTP ${i.status}`)}async function Rn(){const t=y()?.querySelector(".chat-input"),e=t?.value?.trim();if(!e||!a.currentRpRoomId)return;const o=h(a.currentContactId)||a.contacts[0],n=xe();if(!o||!n)return;const i=!!o?.settings?.reasoning_visibility,r="rp_u"+Date.now();a.currentRpMessages.push({id:r,role:"user",text:e,content:e,time:D(),timestamp:new Date().toISOString(),created_at:new Date().toISOString()}),t.value="";const s="rp_ai_"+Date.now();a.currentRpMessages.push({id:s,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0}),a.currentRpRoomId&&(a.rpMessages[a.currentRpRoomId]=a.currentRpMessages.map(tt)),w(120),c(),U();const l={room_id:a.currentRpRoomId,agent_id:n.agent_id||o.id,content:e,...o.persona?{persona:o.persona}:{},...o.settings.model?{model:o.settings.model}:{},...ze(o),...je(o)},d=new AbortController;a.streamingAbortController=d,c();try{const p=await Qe(o,l,d.signal,"/api/rp/chat"),f=()=>a.currentRpMessages.findIndex(T=>T.id===s);a.currentRpMessages[f()]={id:s,role:"ai",text:"",time:D(),typing:!1,streaming:!0},c();const g=p.body.getReader(),I=new TextDecoder;let x="",k="",M="",q="";for(;;){const{done:T,value:C}=await g.read();if(T)break;x+=I.decode(C,{stream:!0});const O=x.split(`
`);x=O.pop()??"";for(const ot of O){const K=ot.trim();if(!K){q="";continue}if(K.startsWith("event:")){q=K.slice(6).trim();continue}if(!K.startsWith("data:"))continue;const ct=K.slice(5).trim();if(ct==="[DONE]")continue;const z=Fe(ct,q);let X=z.text;const nt=ye(z.thinking,k,M),ut=i?nt:"";ut&&M.length<ve&&(M+=ut),X&&(k+=X);const lt=f();lt!==-1&&(a.currentRpMessages[lt]={id:s,role:"ai",text:k,...i&&M?{thinking:M}:{},time:D(),typing:!1,streaming:!0},c(),U())}}const S=a.currentRpMessages.findIndex(T=>T.id===s);S!==-1&&(a.currentRpMessages[S]={...a.currentRpMessages[S],text:k||"…",content:k||"…",...i&&M?{thinking:M}:{},streaming:!1,typing:!1,time:D(),created_at:new Date().toISOString()}),a.streamingAbortController=null,await Wt(n.agent_id||o.id,{silent:!0}),a.currentRpRoomId&&(a.rpMessages[a.currentRpRoomId]=a.currentRpMessages.map(tt)),w(120),c(),U()}catch(p){const f=a.currentRpMessages.findIndex(g=>g.id===s);f!==-1&&(a.currentRpMessages[f]={id:s,role:"ai",text:p.name==="AbortError"?"…":"杩炴帴澶辫触锛?{err.message}",content:p.name==="AbortError"?"…":"杩炴帴澶辫触锛?{err.message}",time:D(),created_at:new Date().toISOString(),typing:!1}),a.streamingAbortController=null,a.currentRpRoomId&&(a.rpMessages[a.currentRpRoomId]=a.currentRpMessages.map(tt)),w(120),c()}}async function Ye(){const t=y()?.querySelector(".chat-input"),e=t?.value?.trim();if(!e)return;const o=h(a.currentContactId);if(!o)return;Rt();const n="u"+Date.now();o.messages.push({id:n,role:"user",text:e,content:e,time:D(),created_at:new Date().toISOString()}),o.lastMessage=e,o.lastTime="刚刚",t.value="";const i="ai_"+Date.now();o.messages.push({id:i,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0,source:"codex"}),et(),w(120),c(),U();const r=new AbortController;a.streamingAbortController=r,c();try{const s=await fetch(`${v}/api/codex/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({conversation_key:`yui:${o.id}`,content:e,reset:!1}),signal:r.signal}),l=await s.json().catch(()=>({}));if(!s.ok)throw new Error(l.detail||`HTTP ${s.status}`);const d=String(l.reply||"").trim()||"…",p=o.messages.findIndex(f=>f.id===i);p!==-1&&(o.messages[p]={id:i,role:"ai",text:d,content:d,source:"codex",provider:"codex",time:D(),created_at:new Date().toISOString(),typing:!1}),o.lastMessage=d,o.lastTime=D(),et(),w(120),c(),U()}catch(s){const l=s.name==="AbortError";l||console.error("[codex chat] error:",s);const d=o.messages.findIndex(p=>p.id===i);if(d!==-1){const p=l?"…":`Codex 连接失败：${s.message}`;o.messages[d]={id:i,role:"ai",text:p,content:p,source:"codex",provider:"codex",time:D(),created_at:new Date().toISOString(),typing:!1}}et(),w(120),c()}finally{a.streamingAbortController=null,c()}}async function Ke(){const t=y()?.querySelector(".chat-input"),e=t?.value?.trim();if(!e)return;const o=h(a.currentContactId);if(!o)return;Rt();const n=!!o?.settings?.reasoning_visibility;let i="";try{i=await Ue(o)}catch(C){console.error("[session] create failed:",C),a.toast="鏃犳硶鍒涘缓浼氳瘽锛?{err.message}",c(),window.setTimeout(()=>{a.toast="",c()},1500);return}const r="u"+Date.now();o.messages.push({id:r,role:"user",text:e,content:e,time:D(),created_at:new Date().toISOString()}),o.lastMessage=e,o.lastTime="刚刚",t.value="",et(),w(120),c(),U();const s="ai_"+Date.now();o.messages.push({id:s,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0}),et(),w(120),c(),U();let l=0,d=!1,p=null;const f=()=>{const C=y()?.querySelector(`#thinking-${s}`);if(!C)return;C.textContent=we(S),C.classList.add("open","thinking-active"),C.setAttribute("aria-hidden","false");const O=y()?.querySelector(`#cot-wrapper-${s}`);O&&O.removeAttribute("data-slow"),a.openThinkingIds[s]=!0},g=()=>{p===null&&(p=requestAnimationFrame(()=>{p=null,f()}))},I=()=>{p!==null&&(cancelAnimationFrame(p),p=null)},x=setInterval(()=>{if(!l)return;const C=y()?.querySelector(`#cot-wrapper-${s}`);C&&C.toggleAttribute("data-slow",Date.now()-l>8e3)},2e3),k={session_id:i,agent_id:o.id,content:e,...o.persona?{persona:o.persona}:{},...o.settings.model?{model:o.settings.model}:{},...ze(o),...je(o)},M=new AbortController;a.streamingAbortController=M,c();let q="",S="",T=null;try{const C=await Qe(o,k,M.signal),O=()=>o.messages.findIndex(dt=>dt.id===s);o.messages[O()]={id:s,role:"ai",text:"",content:"",time:D(),created_at:new Date().toISOString(),typing:!1,streaming:!0},c();const ot=C.body.getReader(),K=new TextDecoder;let ct="",z="";for(;;){const{done:dt,value:pe}=await ot.read();if(dt)break;ct+=K.decode(pe,{stream:!0});const qt=ct.split(`
`);ct=qt.pop()??"";let Ut=0;for(const St of qt){const pt=St.trim();if(!pt){z="";continue}if(pt.startsWith("event:")){z=pt.slice(6).trim();continue}if(!pt.startsWith("data:"))continue;const kt=pt.slice(5).trim();if(kt==="[DONE]")continue;const It=Fe(kt,z);let me=It.text;const fe=ye(It.thinking,q,S),G=n?fe:"";if(G){S.length<ve&&(S+=G),l=Date.now();const Y=O();Y!==-1&&(o.messages[Y]={id:s,role:"ai",text:q,thinking:S,time:D(),typing:!1,streaming:!0},d?g():(d=!0,a.openThinkingIds[s]=!0,c(),U()))}if(It.toolCall){const Y=It.toolCall;T||(T=[]);const Lt=T.find(ao=>ao.name===Y.name&&ao.status!=="done");Lt?Lt.status=Y.status:T.push({name:Y.name,status:Y.status});const mt=O();mt!==-1&&(o.messages[mt]={...o.messages[mt],toolCalls:T.slice(),streaming:!0},c())}me&&(q+=me),Ut+=1,Ut>=32&&(Ut=0,g(),await ra())}}clearInterval(x),I(),a.streamingAbortController=null;const X=O(),nt=q||(n&&S?"":"…");o.lastMessage=nt||"已处理",o.lastTime=D();const ut=y()?.querySelector(`#thinking-${s}`);ut&&ut.classList.remove("thinking-active");const lt=y()?.querySelector(`#cot-wrapper-${s}`);lt&&lt.removeAttribute("data-slow"),n&&S&&delete a.openThinkingIds[s];const B=ca(nt);X!==-1&&B.length>1?(o.messages.splice(X,1),c(),U(),await sa(180),await ua(o,B,{startIndex:X,thinking:n?S:"",toolCalls:T})):(X!==-1&&(o.messages[X]={id:s,role:"ai",text:nt,content:nt,...n&&S?{thinking:S}:{},...T?{toolCalls:T}:{},time:D(),created_at:new Date().toISOString(),typing:!1}),et(),w(120),c(),U())}catch(C){clearInterval(x),I(),a.streamingAbortController=null;const O=C.name==="AbortError";O||console.error("[chat SSE] error:",C);const ot=o.messages.findIndex(K=>K.id===s);ot!==-1&&(o.messages[ot]={id:s,role:"ai",text:O?q||"…":`连接失败：${C.message}，请稍后再试。`,content:O?q||"…":`连接失败：${C.message}，请稍后再试。`,...n&&S?{thinking:S}:{},time:D(),created_at:new Date().toISOString(),typing:!1}),O&&q&&(o.lastMessage=q,o.lastTime=D()),et(),w(120),c()}}async function Vn(t){const e=h(a.currentContactId);if(!e)return;Rt();const o=!!e?.settings?.reasoning_visibility,n=e.messages.findIndex(d=>d.id===t);if(n===-1||e.messages[n].role!=="ai")return;let i="";try{i=await Ue(e)}catch(d){console.error("[session] create failed:",d),a.toast=`无法创建会话：${d.message}`,c(),window.setTimeout(()=>{a.toast="",c()},1500);return}e.messages[n]={...e.messages[n],typing:!0,text:"",streaming:!1},c();const r=[...e.messages].reverse().find(d=>d.role==="user");if(!r)return;const s={session_id:i,agent_id:e.id,content:r.text,...e.persona?{persona:e.persona}:{},...e.settings.model?{model:e.settings.model}:{},...ze(e),...je(e)},l=new AbortController;a.streamingAbortController=l;try{const d=await Qe(e,s,l.signal);e.messages[n]={...e.messages[n],typing:!1,text:"",streaming:!0},c();const p=d.body.getReader(),f=new TextDecoder;let g="",I="",x="",k=null;const M=t;let q="",S=0,T=!1,C=null;const O=()=>{const B=y()?.querySelector(`#thinking-${M}`);if(!B)return;B.textContent=we(x),B.classList.add("open","thinking-active"),B.setAttribute("aria-hidden","false");const dt=y()?.querySelector(`#cot-wrapper-${M}`);dt&&dt.removeAttribute("data-slow"),a.openThinkingIds[M]=!0},ot=()=>{C===null&&(C=requestAnimationFrame(()=>{C=null,O()}))},K=()=>{C!==null&&(cancelAnimationFrame(C),C=null)},ct=setInterval(()=>{if(!S)return;const B=y()?.querySelector(`#cot-wrapper-${M}`);B&&B.toggleAttribute("data-slow",Date.now()-S>8e3)},2e3);for(;;){const{done:B,value:dt}=await p.read();if(B)break;g+=f.decode(dt,{stream:!0});const pe=g.split(`
`);g=pe.pop()??"";let qt=0;for(const Ut of pe){const St=Ut.trim();if(!St){q="";continue}if(St.startsWith("event:")){q=St.slice(6).trim();continue}if(!St.startsWith("data:"))continue;const pt=St.slice(5).trim();if(pt==="[DONE]")continue;const kt=Fe(pt,q);let It=kt.text;const me=ye(kt.thinking,I,x),fe=o?me:"";if(fe){x.length<ve&&(x+=fe),S=Date.now();const G=e.messages.findIndex(Y=>Y.id===M);G!==-1&&(e.messages[G]={...e.messages[G],thinking:x,streaming:!0},T?ot():(T=!0,a.openThinkingIds[M]=!0,c()))}if(kt.toolCall){const G=kt.toolCall;k||(k=[]);const Y=k.find(mt=>mt.name===G.name&&mt.status!=="done");Y?Y.status=G.status:k.push({name:G.name,status:G.status});const Lt=e.messages.findIndex(mt=>mt.id===M);Lt!==-1&&(e.messages[Lt]={...e.messages[Lt],toolCalls:k.slice(),streaming:!0},c())}It&&(I+=It),qt+=1,qt>=32&&(qt=0,ot(),await ra())}}clearInterval(ct),K(),a.streamingAbortController=null;const z=e.messages.findIndex(B=>B.id===M),X=I||"…",nt=y()?.querySelector(`#thinking-${M}`);nt&&nt.classList.remove("thinking-active");const ut=y()?.querySelector(`#cot-wrapper-${M}`);ut&&ut.removeAttribute("data-slow"),o&&x&&delete a.openThinkingIds[M];const lt=ca(X);z!==-1&&lt.length>1?(e.messages.splice(z,1),c(),await sa(180),await ua(e,lt,{startIndex:z,thinking:o?x:"",toolCalls:k})):(z!==-1&&(e.messages[z]={...e.messages[z],text:X,...o&&x?{thinking:x}:{},...k?{toolCalls:k}:{},streaming:!1}),c())}catch(d){clearInterval(_rerollSlowTimer),_cancelRerollFlush(),a.streamingAbortController=null;const p=d.name==="AbortError";p||console.error("[reroll SSE] error:",d);const f=e.messages.findIndex(g=>g.id===rerollId);f!==-1&&(e.messages[f]={...e.messages[f],text:p?fullText||"…":`重试失败：${d.message}`,...fullThinking?{thinking:fullThinking}:{},...fullToolCalls?{toolCalls:fullToolCalls}:{},streaming:!1}),c()}}function U(){requestAnimationFrame(()=>{const t=y()?.querySelector(".messages-panel");t&&(t.scrollTop=t.scrollHeight)})}function Bn(){return`
    ...
    `}const Na=window.openPage;typeof Na=="function"&&(window.openPage=function(e,o){Na(e,o),e==="page-chat"&&ga()});const Nn=[{id:"openai",name:"OpenAI",enabled:!0,baseUrl:"https://api.openai.com/v1",apiPath:"",apiKey:"",models:["gpt-5.4","gpt-5.4-mini","gpt-4.1-mini"],defaultModel:"gpt-5.4"},{id:"openrouter",name:"OpenRouter",enabled:!0,baseUrl:"https://openrouter.ai/api/v1",apiPath:"",apiKey:"",models:["openai/gpt-5","anthropic/claude-3.7-sonnet"],defaultModel:"openai/gpt-5"},{id:"gemini",name:"Gemini",enabled:!0,baseUrl:"https://generativelanguage.googleapis.com/v1beta/openai",apiPath:"",apiKey:"",models:["gemini-2.5-pro","gemini-2.5-flash"],defaultModel:"gemini-2.5-pro"},{id:"deepseek",name:"DeepSeek",enabled:!1,baseUrl:"https://api.deepseek.com/v1",apiPath:"",apiKey:"",models:["deepseek-chat","deepseek-reasoner"],defaultModel:"deepseek-chat"},{id:"qwen",name:"阿里云千问",enabled:!1,baseUrl:"https://dashscope.aliyuncs.com/compatible-mode/v1",apiPath:"",apiKey:"",models:["qwen-max","qwen-plus","qwen-turbo"],defaultModel:"qwen-max"},{id:"zhipu",name:"智谱",enabled:!1,baseUrl:"https://open.bigmodel.cn/api/paas/v4",apiPath:"",apiKey:"",models:["glm-4.5","glm-4-air"],defaultModel:"glm-4.5"}],Xe=new Set(["aiInterface","defaultModels","modelSlot","providerCatalog","providerEditor","promptEditor","themeSettings","accountSettings","memoryService","backendSync","exportSettings","mcpLibrary"]),Hn=Xt,jn=Ie,zn=_e,Ha=Le;a.viewStack=a.viewStack||[],a.activeModelSlot=a.activeModelSlot||"chat",a.activeModelSlotContext=a.activeModelSlotContext||"global",a.activeModelProviderId=a.activeModelProviderId||"",a.providerDraftId=a.providerDraftId||null,a.providerAdvancedOpen=!!a.providerAdvancedOpen,a.providerEditorDraft=a.providerEditorDraft||null,a.providerModelMenuOpen=!!a.providerModelMenuOpen,a.modelSlotMenuOpen=!!a.modelSlotMenuOpen,a.providerSearch=a.providerSearch||"",a.activePromptSlot=a.activePromptSlot||"summary";function V(t){const e=String(t||"").trim();return e?e==="ocr"?"vision":e==="title"?"summary":e:"chat"}a.aiSettingsSaving=!1,a.memoryServiceEntries=Array.isArray(a.memoryServiceEntries)?a.memoryServiceEntries:[],a.memoryServiceLoading=!!a.memoryServiceLoading,a.slotVendorGroupOpen=a.slotVendorGroupOpen&&typeof a.slotVendorGroupOpen=="object"?a.slotVendorGroupOpen:{},a.providerModelVendorOpen=a.providerModelVendorOpen&&typeof a.providerModelVendorOpen=="object"?a.providerModelVendorOpen:{};function Fn(){return"/chat/completions"}function Ht(t,{allowEmpty:e=!1}={}){const o=String(t||"").trim();return o?o.startsWith("/")?o:`/${o}`:e?"":Fn()}function Un(t={}){return Ht(t.apiPath||t.api_path||"",{allowEmpty:!1})}function Ge(t={}){const e=Ht(t.apiPath||t.api_path||"",{allowEmpty:!0});return{...t,baseUrl:t.baseUrl||t.base_url||"",apiKey:t.apiKey||t.api_key||"",apiPath:e,api_path:e,models:Array.isArray(t.models)?t.models:[]}}function jt(){return{providers:Nn.map(t=>Ge({...t,models:[...t.models]})),defaultModels:{chat:{providerId:"openai",model:"gpt-5.4",useChatModel:!1},summary:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},vision:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},translate:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},consciousness:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},voice:{provider:"",service_url:"",base_url:"",voice_id:"",speaker:"",emotion:"",speed:1,format:""}},defaultPrompts:{chat:"Respond naturally, stay consistent with the current role and context, and keep the tone warm and clear.",summary:"Write a concise conversation summary with key facts, action items, and follow-ups.",translate:"Translate the content accurately while preserving tone and formatting when possible.",vision:"Extract visible text from the image and explain key visual information clearly.",consciousness:"Review recent context, infer useful next-step thoughts, and keep the result concise and actionable."},mcpLibrary:no()}}function E(){if(!a.globalSettings.aiSettings)a.globalSettings.aiSettings=jt();else{const t=a.globalSettings.aiSettings;t.defaultModels=t.defaultModels||{},t.defaultPrompts=t.defaultPrompts||{},t.defaultModels.ocr&&!t.defaultModels.vision&&(t.defaultModels.vision={...t.defaultModels.ocr}),t.defaultPrompts.ocr&&!t.defaultPrompts.vision&&(t.defaultPrompts.vision=t.defaultPrompts.ocr),delete t.defaultModels.ocr,delete t.defaultPrompts.ocr,delete t.defaultModels.title,delete t.defaultPrompts.title,Object.entries(jt().defaultModels).forEach(([e,o])=>{t.defaultModels[e]||(t.defaultModels[e]={...o})}),Object.entries(jt().defaultPrompts).forEach(([e,o])=>{typeof t.defaultPrompts[e]!="string"&&(t.defaultPrompts[e]=o)})}return a.globalSettings.aiSettings}function Qn(t={}){const e=jt(),o={...t||{}};o.defaultModels?.ocr&&!o.defaultModels?.vision&&(o.defaultModels={...o.defaultModels,vision:o.defaultModels.ocr}),o.defaultPrompts?.ocr&&!o.defaultPrompts?.vision&&(o.defaultPrompts={...o.defaultPrompts,vision:o.defaultPrompts.ocr});const n={providers:e.providers,defaultModels:{...e.defaultModels},defaultPrompts:{...e.defaultPrompts},mcpLibrary:{...e.mcpLibrary,tools:[...e.mcpLibrary?.tools||[]]}};if(Array.isArray(o.providers)&&o.providers.length){const i=new Map(e.providers.map(r=>[r.id,r]));o.providers.forEach(r=>{const s=Ge(r);i.set(s.id,{...i.get(s.id),...s,models:Array.isArray(s.models)&&s.models.length?s.models:i.get(s.id)?.models||[]})}),n.providers=[...i.values()]}o.defaultModels&&Object.keys(n.defaultModels).forEach(i=>{o.defaultModels[i]&&(n.defaultModels[i]={...n.defaultModels[i],...o.defaultModels[i]})}),o.defaultPrompts&&Object.keys(n.defaultPrompts).forEach(i=>{typeof o.defaultPrompts[i]=="string"&&(n.defaultPrompts[i]=o.defaultPrompts[i])}),o.mcpLibrary&&Array.isArray(o.mcpLibrary.tools)&&(n.mcpLibrary={...n.mcpLibrary,...o.mcpLibrary,tools:o.mcpLibrary.tools.map(it)}),a.globalSettings.aiSettings=n,typeof o.consciousnessLoop=="boolean"&&(a.globalSettings.consciousnessLoop=o.consciousnessLoop),se()}function se(){const t=E(),e=t.defaultModels.chat,o=t.providers.find(n=>n.id===e.providerId);a.globalSettings.defaultModel=e.model,a.globalSettings.provider=o?.name||"OpenAI"}function Q(t){return E().providers.find(e=>e.id===t)}function Je(t=a.providerDraftId){const e=Ge(Q(t)||{id:t||`custom_${Date.now()}`,name:"",enabled:!0,baseUrl:"",apiPath:"",apiKey:"",models:[],defaultModel:""}),o=Array.isArray(e.models)?[...e.models]:[],n=o.map(We);return{...e,models:o,_allModels:n,_selectedModelIds:new Set(o)}}function R(){return(!a.providerEditorDraft||a.providerEditorDraft.id!==a.providerDraftId)&&(a.providerEditorDraft=Je()),a.providerEditorDraft}function ce(t="",e=[]){const o=String(t||"").trim().toLowerCase();return o?e.filter(n=>String(n||"").toLowerCase().includes(o)):[...e]}function ja(t){const e=String(t||"").toLowerCase();return/deepseek/.test(e)?"DeepSeek":/\bglm\b|chatglm/.test(e)?"GLM":/\bqwen\b|qwq/.test(e)?"Qwen":/\bgpt[-\d]|^gpt|^o[134][-\d]|text-davinci|text-curie/.test(e)?"OpenAI":/claude/.test(e)?"Anthropic":/gemini|gemma/.test(e)?"Google":/\bllama\b|meta-llama/.test(e)?"Meta":/mistral|mixtral|codestral/.test(e)?"Mistral":/\byi[-/_]/.test(e)?"闆朵竴涓囩墿":/moonshot|kimi/.test(e)?"Moonshot":/hunyuan/.test(e)?"娣峰厓":/ernie|wenxin/.test(e)?"鏂囧績":/doubao/.test(e)?"璞嗗寘":/baichuan/.test(e)?"鐧惧窛":/spark/.test(e)?"鏄熺伀":/internlm/.test(e)?"InternLM":"Other"}function za(t){const e=String(t||"").toLowerCase(),o=["chat","text"];return/vl\b|vision|visual|\bvision\b|-v\d|\bimg\b/.test(e)&&o.push("vision"),/reason|r1\b|think\b|cot\b/.test(e)&&o.push("reasoning"),/image|draw|flux|paint|artist|diffusion/.test(e)&&o.push("image"),o.push("tools"),o}function We(t){const e=String(t||"").trim();return{id:e,name:e,vendor:ja(e),capabilities:za(e)}}const Yn={chat:"瀵硅瘽",text:"鏂囨湰",reasoning:"鎺ㄧ悊",tools:"宸ュ叿璋冪敤",vision:"瑙嗚",image:"鐢熷浘"},Kn=["reasoning","tools","vision","image"],Xn={reasoning:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path d="M5 .5A3 3 0 0 0 2.8 5.9l.2.3V8h4V6.2l.2-.3A3 3 0 0 0 5 .5zm-1.2 8h2.4v.5c0 .28-.22.5-.5.5H4.3a.5.5 0 0 1-.5-.5V8.5z"/></svg>',tools:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path d="M7.5 1a2 2 0 0 0-1.86 2.73L1.2 8.16a.6.6 0 0 0 .84.84l4.43-4.44A2 2 0 1 0 7.5 1zm0 3a1 1 0 1 1 0-2 1 1 0 0 1 0 2z"/></svg>',vision:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M5 2C2.5 2 .8 5 .8 5S2.5 8 5 8s4.2-3 4.2-3S7.5 2 5 2zm0 4.5A1.5 1.5 0 1 1 5 3.5a1.5 1.5 0 0 1 0 3z"/></svg>',image:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M1.5 1A.5.5 0 0 0 1 1.5v7a.5.5 0 0 0 .5.5h7a.5.5 0 0 0 .5-.5v-7A.5.5 0 0 0 8.5 1h-7zM2 8l2-2.5 1.3 1.7 1.7-2.2L9 8H2zm.8-4.3a.7.7 0 1 0 1.4 0 .7.7 0 0 0-1.4 0z"/></svg>'};function Gn(t){return(Array.isArray(t?.capabilities)?t.capabilities:za(t?.name||"")).filter(o=>Kn.includes(o)).map(o=>`<span class="model-cap-badge cap-${o}" title="${Yn[o]||o}">${Xn[o]||o}</span>`).join("")}function Fa(t="",e=[]){const o=String(t||"").trim();return!Array.isArray(e)||!e.length?"还没有已同步模型，仍可手动输入并保存。":o?e.some(i=>String(i).toLowerCase()===o.toLowerCase())?"已匹配到已同步列表中的模型。":"当前模型不在已同步列表中，可继续手动保存。":`已同步 ${e.length} 个模型，可搜索或展开列表选择。`}function Ze(t="",e=[]){const o=String(t||"").trim();return!Array.isArray(e)||!e.length?"当前供应商还没有同步模型，可切换供应商或先同步。":o?e.some(i=>String(i).toLowerCase()===o.toLowerCase())?"已匹配到当前供应商模型。":"当前输入不在同步列表中。":`已同步 ${e.length} 个模型，可搜索或展开列表选择。`}function yt(){const t=R(),e=document.getElementById("provider-default-model-input"),o=document.getElementById("provider-default-model-menu"),n=document.getElementById("provider-default-model-hint");if(!o||!n)return;const i=e?.value||t.defaultModel||"",r=Array.isArray(t.models)?t.models:[],s=ce(i,r);if(n.textContent=Fa(i,r),!a.providerModelMenuOpen){o.innerHTML="",o.classList.remove("open");return}o.classList.add("open"),o.innerHTML=s.length?s.map(l=>`
        ...
        `).join(""):'<div class="provider-model-empty">娌℃湁鍖归厤鍒板凡鍚屾妯″瀷锛屽彲缁х画鎵嬪姩杈撳叆淇濆瓨銆?/div>'}function _(t){return E().defaultModels[V(t)]}function Jn(){const t=a.activeModelSlot,e=a.activeModelSlotContext==="contact",o=h(a.currentContactId)||a.contacts[0],n=e?{providerId:o?.settings?.modelProviderId||a.activeModelProviderId||_("chat")?.providerId||"openai",model:t==="consciousness"?o?.settings?.loopModel||"":o?.settings?.model||""}:le(t),i=Q(n?.providerId)||Q(_("chat")?.providerId);return{slot:n,provider:i,models:i?.models||[]}}function ue(){const t=document.getElementById("model-slot-menu"),e=document.getElementById("model-slot-hint"),o=document.getElementById("model-slot-input");if(!t||!e)return;const{slot:n,models:i}=Jn(),r=o?.value||n?.model||"",s=ce(r,i);if(e.textContent=Ze(r,i),!a.modelSlotMenuOpen){t.innerHTML="",t.classList.remove("open");return}t.classList.add("open"),t.innerHTML=s.length?s.map(l=>`
        ...
        `).join(""):'<div class="provider-model-empty">娌℃湁鍖归厤鍒板綋鍓嶄緵搴斿晢妯″瀷銆?/div>'}function le(t){return _(V(t))}function Et(t){const e=V(t);return{chat:"聊天模型",summary:"摘要模型",vision:"Vision 模型",translate:"翻译模型",consciousness:"意识循环模型",voice:"语音模型"}[e]||e}function de(t){const e=V(t);return{chat:"全局默认使用的聊天模型。",summary:"用于生成对话摘要，推荐选择便宜且稳定的模型。",vision:"用于识图、OCR 与截图分析的统一入口。",translate:"用于翻译消息内容，推荐选择速度快的模型。",consciousness:"用于意识循环、主动思考与相关后台能力。",voice:"用于文本转语音，读取语音服务地址与 voice ID。"}[e]||""}function Ua(t){const e=V(t),o=le(e);if(e==="voice"){if(!o)return"未设置";const i=o.provider||"语音服务",r=o.voice_id||o.voiceId||"未设置";return`${i} / ${r}`}const n=Q(o?.providerId);return o?`${n?.name||"未设置"} / ${o.model||"未设置"}`:"未设置"}function Wn(t){return E().defaultPrompts?.[V(t)]||""}function Zn(t){const e=V(t);return{chat:m("comment"),summary:m("file"),vision:m("search"),translate:m("chatArrow"),consciousness:m("history"),voice:m("mic")}[e]||m("file")}function ti(t){const e=V(t);return e!=="chat"&&e!=="voice"}function ei(t){return`
    ...
    `}function ii(t){const e=Math.max(0,Math.min(100,Number(t)||0)),o=e>60?"#c9908a":e>30?"#c8a07a":"#b0b0b8";return`<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;color:${o};">
          ...
          ${e>0?`热度 ${e}`:""}
        </span>`}function ri(){const t=h(a.currentContactId)||a.contacts[0],e=Array.isArray(a.memoryServiceEntries)?a.memoryServiceEntries:[],o=Array.isArray(a.memoryCandidates)?a.memoryCandidates:[],n=a.memoryServiceSort||"updated_at",i=[{key:"updated_at",label:"最新"},{key:"importance",label:"最重要"},{key:"temperature",label:"有温度"}];return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>记忆服务</h3>
    ...
    `}function ta(){return String(a.currentContactId||h(a.currentContactId)?.id||"default").trim()||"default"}async function wt(t=ta(),{silent:e=!0}={}){const o=String(t||"").trim();if(o){a.memoryServiceLoading=!0,c();try{const n=a.memoryServiceSort||"updated_at",i=new URLSearchParams({agent_id:o,sort_by:n,order:"desc",limit:"100"}),[r,s]=await Promise.all([fetch(`${v}/api/memories?${i.toString()}`),fetch(`${v}/api/consciousness/memory-candidates?agent_id=${encodeURIComponent(o)}&limit=20`)]);if(!r.ok)throw new Error(`HTTP ${r.status}`);const l=await r.json().catch(()=>({}));if(a.memoryServiceEntries=Array.isArray(l?.memories)?l.memories:[],s.ok){const d=await s.json().catch(()=>({}));a.memoryCandidates=Array.isArray(d?.candidates)?d.candidates:[]}}catch(n){console.warn("[memory service] load failed",n),e||(a.toast="记忆加载失败",window.setTimeout(()=>{a.toast="",c()},1200))}finally{a.memoryServiceLoading=!1,c()}}}async function si(t){const e=ta();try{const o=await fetch(`${v}/api/consciousness/memory-candidates/${encodeURIComponent(t)}/promote?agent_id=${encodeURIComponent(e)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"});if(!o.ok)throw new Error(`HTTP ${o.status}`);a.memoryCandidates=(a.memoryCandidates||[]).filter(n=>String(n.id)!==String(t)),a.toast="✓ 已采纳为正式记忆",window.setTimeout(()=>{a.toast="",c()},1800),await wt(e,{silent:!0})}catch(o){console.warn("[memory] promote failed",o)}}async function ci(t){try{const e=await fetch(`${v}/api/consciousness/memory-candidates/${encodeURIComponent(t)}`,{method:"DELETE"});if(!e.ok)throw new Error(`HTTP ${e.status}`);a.memoryCandidates=(a.memoryCandidates||[]).filter(o=>String(o.id)!==String(t)),c()}catch(e){console.warn("[memory] dismiss failed",e)}}function Qa(t=null){const e=t||{},o=window.prompt("记忆内容",String(e.raw_content||e.content||"").trim());if(o===null)return null;const n=window.prompt("分层 / category（core_profile / recent_pending / deep / ephemeral）",String(e.category||"recent_pending"));if(n===null)return null;const i=window.prompt("可见范围（private / shared / public）",String(e.visibility||"private"));if(i===null)return null;const r=window.prompt("重要度（1-5）",String(e.importance??3));if(r===null)return null;const s=window.prompt("过期时间 ISO（可留空）",String(e.expires_at||""));return s===null?null:{agent_id:ta(),content:String(o||"").trim(),raw_content:String(o||"").trim(),category:String(n||"").trim()||"recent_pending",visibility:String(i||"").trim()||"private",importance:Math.max(1,Math.min(5,Number(r)||3)),expires_at:String(s||"").trim()||null}}async function ui(){const t=Qa();if(!t||!t.content)return;const e=await fetch(`${v}/api/memories`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),o=await e.json().catch(()=>({}));if(!e.ok)throw new Error(o?.detail||`HTTP ${e.status}`)}async function li(t){const e=a.memoryServiceEntries.find(r=>String(r.id)===String(t));if(!e)return;const o=Qa(e);if(!o||!o.content)return;const n=await fetch(`${v}/api/memories/${encodeURIComponent(t)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}),i=await n.json().catch(()=>({}));if(!n.ok)throw new Error(i?.detail||`HTTP ${n.status}`)}async function di(t){if(!window.confirm("删除这条记忆？"))return;const e=await fetch(`${v}/api/memories/${encodeURIComponent(t)}`,{method:"DELETE"}),o=await e.json().catch(()=>({}));if(!e.ok)throw new Error(o?.detail||`HTTP ${e.status}`)}function pi(){const t=Bt();return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>同步后端</h3>
    ...
    `}const n=e?{providerId:o?.settings?.modelProviderId||a.activeModelProviderId||_("chat")?.providerId||"openai",model:t==="consciousness"?o?.settings?.loopModel||"":o?.settings?.model||""}:le(t),i=E().providers.filter(d=>d.enabled),s=(Q(n.providerId)||Q(_("chat")?.providerId)||i[0])?.models||[];if(e){const d=ce(n.model||"",s);return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>${u(Et(t))}</h3>
        ...
        <div class="settings-group glass-frost ai-panel compact-panel">
          ...
          <div class="provider-model-picker">
            ...
            <p id="model-slot-hint" class="section-eyebrow provider-model-hint">${u(Ze(n.model||"",s))}</p>
    ...
    `}function Ti(t){const e=Array.isArray(t._allModels)?t._allModels:[],o=t._selectedModelIds instanceof Set?t._selectedModelIds:new Set(t._selectedModelIds||[]),n=o.size,i={};for(const g of e){const I=g.vendor||"Other";i[I]||(i[I]=[]),i[I].push(g)}const r=["OpenAI","Anthropic","Google","DeepSeek","Qwen","GLM","Meta","Mistral","Moonshot","璞嗗寘","鏂囧績","娣峰厓","鐧惧窛","鏄熺伀","闆朵竴涓囩墿","InternLM","Other"],s=[...new Set([...r.filter(g=>i[g]),...Object.keys(i)])],l=e.map(g=>g.id),d=l.length>0&&l.every(g=>o.has(g)),p=s.map(g=>{const I=i[g]||[],x=!!a.providerModelVendorOpen[g],k=I.filter(S=>o.has(S.id)).length,M=I.length>0&&I.every(S=>o.has(S.id)),q=x?`
          <div class="vendor-group-body">
            ${I.map(S=>{const T=o.has(S.id);return`
              <div class="pool-model-row">
                ...
                <span class="pool-model-caps">${Gn(S)}</span>
      ...
      </div>`}function Pi(){const t=R(),e=Ht(t.apiPath||t.api_path||"",{allowEmpty:!0}),o=Un(t),n=!!a.providerAdvancedOpen||!!e;return`
        ...
        <div class="settings-group glass-frost ai-panel provider-editor-card">
          ...
          <div class="prov-sec">
            ...
            <div class="provider-model-picker">
              ...
              <p id="provider-default-model-hint" class="section-eyebrow provider-model-hint">${u(Fa(t.defaultModel||"",t.models||[]))}</p>
