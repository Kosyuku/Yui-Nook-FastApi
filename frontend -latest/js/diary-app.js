  ...
  */
  ...
  function renderHeader() {
    ...
    const title = state.view === 'board'
      ? 'Daydream'
      : state.view === 'list'
        ? (userNotebook ? '我的本子' : `${person.name}的日记本`)
        : (userNotebook ? '我的本子' : '角色日记');
    const subtitle = state.view === 'board'
      ? '白昼梦'
      : state.view === 'list'
        ? (userNotebook ? '我的日记' : '角色日记')
        : (userNotebook ? '写给自己看的页角' : '翻页');
              ...
              >
                ...
                <div class="diary-book-inner">
                  ...
                  <div class="diary-book-meta">
                    ${String(person.authorType || '').toLowerCase() === 'user' ? '<span>[我的]</span>' : ''}
                    <span>${Number.isFinite(person.entryCount) ? person.entryCount : person.diaries.length} 篇</span>
                  </div>
                </div>
              </button>
            `,
        ...
        <div class="diary-list-stage accent-${person.accent}">
          <div class="diary-list-fog-card">
            <div class="diary-list-minihead">
              ...
              <span class="diary-list-pill">${userNotebook ? '我的本子' : `${person.name}的本子`}</span>
            </div>
            ...
            <div class="diary-list-fog-meta">
              ${userNotebook ? '我的本子 · 只给自己看的日记' : `${person.name}的日记本 · 角色日记`} · ${Number.isFinite(person.entryCount) ? person.entryCount : person.diaries.length}篇
              ${person.canRename ? '<button class="ambs-add-btn" data-action="rename-notebook">改名</button>' : ''}
              ${person.canCreateEntry ? '<button class="ambs-add-btn" data-action="create-diary-entry">＋ 新建</button>' : ''}
            </div>
          </div>
          ...
          <div class="diary-entry-list">
            ${person.diaries
              .map(
                (diary) => `
                  <article class="diary-entry-row">
                    ...
                    >
                      <h3>${diary.title}</h3>
                      <p class="diary-entry-song">${diary.song ? `${diary.song}${diary.mood ? `（${diary.mood}）` : ''}` : (diary.preview || '日记条目')}</p>
              ...
              .join('')}
            ${person.diaries.length ? '' : '<p class="diary-note-empty">这本日记还没有内容。</p>'}
          </div>
        </div>
      </section>
    `;
      ...
      <section class="diary-detail-view">
        <article class="diary-paper accent-${person.accent}">
          ...
          <div class="diary-paper-foot">
            ...
            ${diary.canEdit ? '<button class="ambs-add-btn" data-action="edit-diary-entry">编辑</button>' : ''}
            ${diary.canDelete ? '<button class="ambs-add-btn" data-action="delete-diary-entry">删除</button>' : ''}
          </div>
        </article>

        <aside class="diary-side-notes">
          <section class="diary-note-panel">
            <h3>${userNotebook ? '我的日记' : '角色留言'}</h3>
            ...
            ${diary.canComment ? '<button class="ambs-add-btn" data-action="comment-diary-entry">写评论</button>' : ''}
          </section>
        </aside>
      </section>
    `;
