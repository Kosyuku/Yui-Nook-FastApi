(() => {
    ...
    function renderHeader() {
        ...
        <header class="chat-page-header simple-header">
          ...
          <div class="chat-page-title">Mirage路骞绘ゼ</div>
      ...
      <header class="room-hero room-theme-${c.theme}">
        <div class="room-hero-inner">
          ...
          <div class="room-actions">
            <button class="icon-btn icon-circle" data-action="open-rp-lobby" aria-label="Mirage路骞绘ゼ">${icon('history')}</button>
            ...
            <button type="button" class="chat-list-item" data-action="open-rp-lobby" style="min-height:44px;padding:8px 14px;">
              <div class="chat-list-content" style="min-width:0;">
                <div class="chat-list-head">
                  <strong class="chat-list-name" style="font-size:14px;color:rgba(92,76,84,.72);font-weight:700;">Mirage路骞绘ゼ</strong>
                </div>
              </div>
            </button>
      ...
      <article class="moment-row">
        ...
        <div class="moment-content-col">
          ...
          <div class="moment-footer">
            <time class="moment-time">${escapeHtml(moment.time || moment.created_at || '')}</time>
        ...
        <div class="moment-content-col">
          ...
          ${(moment.likes.length > 0 || moment.comments.length > 0) ? `
            <div class="moment-interactions" data-moment-id-panel="${moment.id}">
              ...
              ${moment.comments.length > 0 ? `
                <div class="moment-comments-area">
                  ${moment.comments.map((comment) => `<div class="moment-comment-line"><span class="comment-author">${escapeHtml(comment.author_name || '璁垮')}</span>: <span class="comment-text">${escapeHtml(comment.text)}</span></div>`).join('')}
                </div>
              ` : ''}
            </div>
          ` : ''}
          ...
          <div class="moment-inline-comment ${state.commentSheetMomentId === moment.id ? 'open' : ''}">
            <input class="moment-comment-input" data-comment-input="${moment.id}" placeholder="鍐欎笅浣犵殑璇勮" />
      ...
      <div class="topic-confirm-overlay" data-action="close-rp-room-dialog">
        <section class="topic-confirm-card glass-frost" data-rp-room-dialog="card" role="dialog" aria-modal="true" aria-label="${isEdit ? '编辑房间' : '新建房间'}" style="max-width:440px;">
          ...
          <div style="display:grid;gap:10px;text-align:left;">
            <input id="rp-room-name" class="search-input glass-frost" style="width:100%;padding:12px 14px;border-radius:16px;" placeholder="剧本" value="${escapeHtml(form.name || '')}" />
            <textarea id="rp-room-world" class="persona-textarea glass-frost" rows="3" placeholder="世界观">${escapeHtml(form.world_setting || '')}</textarea>
            <input id="rp-room-user-role" class="search-input glass-frost" style="width:100%;padding:12px 14px;border-radius:16px;" placeholder="你的角色" value="${escapeHtml(form.user_role || '')}" />
            <input id="rp-room-ai-role" class="search-input glass-frost" style="width:100%;padding:12px 14px;border-radius:16px;" placeholder="AI 角色" value="${escapeHtml(form.ai_role || '')}" />
          </div>
          <div class="topic-confirm-actions">
    ...
    function renderGlobalSettings() {
      ...
      <section class="settings-page page-block ai-settings-page">
        ...
        <div class="settings-group glass-frost ai-panel">
          ...
          ${navRow('AI 鎺ュ彛', `${s.provider || 'OpenAI'} / ${s.defaultModel || 'gpt-5.4'}`, 'open-ai-interface')}
        </div>
        <div class="settings-group glass-frost ai-panel">
          <h3>数据与存储</h3>
    ...
    function renderContactSettingsV2() {
      ...
      <section class="contact-settings-page page-block">
        ...
        ${state.currentSettingsTab === 'model' ? `
          ...
          <div class="settings-group glass-frost ai-panel">
            <h3>\u89d2\u8272\u8bbe\u5b9a</h3>
            <textarea class="persona-textarea glass-frost" data-contact-field="persona" rows="5" placeholder="\u5728\u8fd9\u91cc\u8f93\u5165 AI \u7684\u4eba\u8bbe\u3001\u89d2\u8272\u8bf4\u660e\u3001\u884c\u4e3a\u6307\u4ee4\u3002">${escapeHtml(c.persona || '')}</textarea>
  ...
  function renderCompanionStateDetail() {
    ...
    const cooldown = cs.proactive_cooldown_until || '还没有东西';
    const updatedAt = cs.updated_at || '还没有东西';
    return `
      <section class="topics-page page-block">
        <div class="settings-group glass-frost ai-panel topic-detail-card">
          <h3>当前状态</h3>
      ...
      <section class="new-contact-page page-block">
        <div class="settings-group glass-frost ai-panel new-contact-card">
          ...
          <div class="new-contact-field">
            ...
            <input id="nc-name" class="new-contact-input" placeholder="\u65b0\u8054\u7cfb\u4eba\u79f0\u547c" />
          </div>
          <div class="new-contact-field">
            <label for="nc-bio">\u8054\u7cfb\u4eba\u7b80\u4ecb</label>
            <input id="nc-bio" class="new-contact-input" placeholder="\u4e00\u53e5\u7b80\u77ed\u7684\u63cf\u8ff0" />
          </div>
    ...
    function renderContactQuickActionEditor(contact, actionId) {
      ...
      <div class="qae-fields">
        <div class="qae-field-group">
          ...
          <input id="contact-quick-label" class="qae-input" value="${escapeHtml(action.label || '')}" placeholder="\u4f8b\u5982\uff1a\u5929\u6c14" autocomplete="off" />
        </div>
        <div class="qae-field-group">
          <label class="qae-label">MCP \u8c03\u7528\uff08\u53ef\u9009\uff09</label>
          <select id="contact-quick-mcp" class="qae-select">
            <option value="">\u4e0d\u8c03\u7528 MCP</option>
            ${fallbackTools.map((item) => `<option value="${escapeHtml(item.id)}" ${mcpValue === item.id ? 'selected' : ''}>${escapeHtml(item.label)}</option>`).join('')}
          </select>
        </div>
        <div class="qae-field-group">
          <label class="qae-label">\u70b9\u51fb\u540e\u53d1\u9001\u7684\u8bdd\u672f</label>
          <textarea id="contact-quick-prompt" class="qae-textarea" placeholder="\u8f93\u5165\u9ed8\u8ba4\u8bdd\u672f\uff0c\u4e0d\u8bbe\u7f6e\u5219\u4e0d\u4f1a\u81ea\u52a8\u53d1\u9001">${escapeHtml(action.prompt || '')}</textarea>
        </div>
      </div>
    `;
    }
    ...
    function renderCotLogPage() {
      ...
      <section class="cot-log-page page-block">
        ...
        <div class="cot-log-stack">
          ${state.activityLogLoading ? '<div class="cot-log-empty glass-frost">正在加载活动日志...</div>' : ''}
          ${!state.activityLogLoading && state.activityLogLoadedAt && !logs.length ? '<div class="cot-log-empty glass-frost">还没有活动日志。</div>' : ''}
          ${logs.map((item) => {
    ...
    function applyLocalPayload(payload) {
        ...
        if (!chatProfileLoaded && payload.accountProfile && typeof payload.accountProfile === 'object') {
    ...
    function renderBackendSyncPage() {
      ...
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          ...
          ${navRow('\u4e0a\u6b21\u540c\u6b65', meta.last_server_updated_at || '\u6682\u65e0')}
          <div class="ai-inline-actions" style="margin-top:10px;">
    ...
    async function loadChatProfile({ silent = true } = {}) {
        try {
            const resp = await fetch(`${API_BASE}/api/chat/profile`);
            if (!resp.ok) {
            ...
            if (data?.profile && typeof data.profile === 'object') {
                if (!data.updated_at && state.accountProfile && typeof state.accountProfile === 'object') {
                    state.accountProfile = {
                        ...data.profile,
                        ...state.accountProfile,
                    };
                    chatProfileLoaded = true;
                    render();
                    saveChatProfile(state.accountProfile).catch(() => {});
    ...
    async function saveChatProfile(patch = {}, { silent = true } = {}) {
        const cleanPatch = {};
        ['avatar', 'nickname', 'signature'].forEach((key) => {
            if (Object.prototype.hasOwnProperty.call(patch, key)) {
                cleanPatch[key] = patch[key] == null ? '' : String(patch[key]);
            }
        });
        if (Object.keys(cleanPatch).length === 0) return state.accountProfile;
        ...
        try {
            ...
            console.warn('[chat profile] save failed', error);
            if (!silent) {
                state.toast = '账号资料保存失败';
                render();
                window.setTimeout(() => { state.toast = ''; render(); }, 1400);
            }
            throw error;
    ...
    async function syncProviderModelsFromEditor() {
        ...
        try {
            ...
            if (message.includes('Failed to fetch')) {
                alert('\u540c\u6b65\u5931\u8d25\uff1a\u5f53\u524d\u524d\u7aef\u8fde\u4e0d\u4e0a\u540e\u7aef\u63a5\u53e3\u3002\u8bf7\u786e\u8ba4\u540e\u7aef\u5df2\u542f\u52a8\uff0c\u5e76\u4e14 http://localhost:8000 \u53ef\u4ee5\u8bbf\u95ee\u3002');
                return;
            }
            alert(`\u540c\u6b65\u5931\u8d25\uff1a${message}`);
        }
    ...
    handleClick = function patchedHandleClick(event) {
        ...
        if (action === 'open-account-nickname') {
            ...
            state.toast = '\u6635\u79f0\u5df2\u66f4\u65b0';
            render();
            saveChatProfile({ nickname: nextName }, { silent: false }).catch(() => {});
            window.setTimeout(() => { state.toast = ''; render(); }, 1200);
            return;
        }
        if (action === 'open-account-signature') {
            ...
            state.toast = '\u4e2a\u6027\u7b7e\u540d\u5df2\u66f4\u65b0';
            render();
            saveChatProfile({ signature: nextSign }, { silent: false }).catch(() => {});
            window.setTimeout(() => { state.toast = ''; render(); }, 1200);
            return;
        }
    ...
    document.addEventListener('change', (event) => {
        ...
        if (target?.id === 'account-avatar-file') {
            ...
            reader.onload = () => {
                ...
                state.toast = '\u5934\u50cf\u5df2\u66f4\u65b0';
                render();
                saveChatProfile({ avatar: state.accountProfile.avatar }, { silent: false }).catch(() => {});
                window.setTimeout(() => { state.toast = ''; render(); }, 1200);
            };
            reader.readAsDataURL(file);
            return;
