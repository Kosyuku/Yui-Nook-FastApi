...
`).split(/\n{2,}/).map(c=>String(c||"").trim()).filter(Boolean),i=[],r=c=>{const l=String(c||"").trim();if(l){if(i.length&&l.length<=4){i[i.length-1]+=l;return}i.push(l)}},s=c=>{const l=String(c||"").split(new RegExp("(?<=[。！？!?…])\\s*","u")).map(m=>m.trim()).filter(Boolean);if(l.length<=1){r(c);return}let p="";l.forEach(m=>{const f=p?`${p}${m}`:m;p&&f.length>90?(r(p),p=m):p=f}),r(p)};return o.forEach(c=>{c.length<=110?r(c):s(c)}),i.filter(Boolean)}function po(e){const t=String(e||"").trim().length;return t<=10?300+Math.floor(Math.random()*201):t<=24?600+Math.floor(Math.random()*301):900+Math.floor(Math.random()*301)}function De(){n.assistantPlayback.token="",n.assistantPlayback.timer&&(window.clearTimeout(n.assistantPlayback.timer),n.assistantPlayback.timer=null)}async function Hn(e,t,a={}){const o=Array.isArray(t)?t.filter(s=>String(s||"").trim()):[];if(!e||!o.length)return;De();const i=`reply_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;n.assistantPlayback.token=i;const r=Number.isInteger(a.startIndex)?a.startIndex:e.messages.length;for(let s=0;s<o.length;s+=1){if(n.assistantPlayback.token!==i)return;const c={id:`ai_chunk_${Date.now()}_${s}_${Math.random().toString(36).slice(2,6)}`,role:"ai",text:o[s],content:o[s],time:q(),created_at:new Date().toISOString()};if(s===0&&(a.thinking&&(c.thinking=a.thinking),a.toolCalls&&(c.toolCalls=a.toolCalls)),s===0&&a.replaceId){const l=e.messages.findIndex(p=>p.id===a.replaceId);l!==-1?e.messages[l]=c:e.messages.splice(Math.min(r,e.messages.length),0,c)}else{const l=Math.min(r+s,e.messages.length);e.messages.splice(l,0,c)}if(e.lastMessage=c.text,e.lastTime=c.time,u(),j(),s>=o.length-1)break;await new Promise(l=>{n.assistantPlayback.timer=window.setTimeout(l,po(o[s]))}),n.assistantPlayback.timer=null}n.assistantPlayback.token===i&&(n.assistantPlayback.token="",n.assistantPlayback.timer=null),k(120)}function Oe(e){const t=e&&typeof e=="object"?e:{},a=i=>Array.isArray(i)?i.map(r=>String(r||"").trim()).filter(Boolean):[],o=i=>i!=null&&String(i).trim()?String(i).trim():null;return{recent_topics:a(t.recent_topics),current_mood:String(t.current_mood||"").trim(),open_loops:a(t.open_loops),proactive_cooldown_until:t.proactive_cooldown_until?String(t.proactive_cooldown_until):null,impression:o(t.impression),relationshipProgress:o(t.relationship_progress??t.relationshipProgress),likesSummary:o(t.likes_summary??t.likesSummary),summaryUpdatedAt:o(t.summary_updated_at??t.summaryUpdatedAt),updated_at:String(t.updated_at||"").trim()}}function mo(){const e=Oe(n.companionState);return e.current_mood?`情绪：${e.current_mood}`:e.open_loops[0]?`进行中：${e.open_loops[0]}`:e.recent_topics[0]?`最近话题：${e.recent_topics[0]}`:"暂无状态"}function we(){if(n.momentsActorType==="agent"){const e=Q();return{author_type:"agent",author_id:e?.id||n.currentContactId||"default",author_name:e?.name||"当前角色",avatar:e?.avatar||""}}return{author_type:"user",author_id:"me",author_name:n.accountProfile?.nickname||"我",avatar:n.accountProfile?.avatar||"https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&q=80"}}function P(e={}){const t=Array.isArray(e.likes)?e.likes:[],a=Array.isArray(e.comments)?e.comments:[],o=String(e.author_type||(e.contactId==="me"?"user":"agent")),i=String(e.author_id||(o==="user"?"me":e.contactId||"default"));return{id:String(e.id||`p${Date.now()}`),author_type:o,author_id:i,content:String(e.content||""),image:String(e.image||""),mood:String(e.mood||""),time:String(e.time||""),created_at:String(e.created_at||""),updated_at:String(e.updated_at||""),likes:t.map(r=>typeof r=="string"?{author_type:"user",author_id:r==="我"?"me":r,author_name:r}:{author_type:String(r?.author_type||"user"),author_id:String(r?.author_id||"me"),author_name:String(r?.author_name||"")}),comments:a.map(r=>({author_type:String(r?.author_type||"user"),author_id:String(r?.author_id||"me"),author_name:String(r?.author_name||r?.author||""),text:String(r?.text||"")}))}}function Nn(e){const t=P(e);if(t.author_type==="agent"){const a=b(t.author_id);return{name:a?.name||t.author_id||"角色",avatar:a?.avatar||""}}return{name:n.accountProfile?.nickname||"我",avatar:n.accountProfile?.avatar||"https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&q=80"}}function jn(e){const t=P(e);return t.author_type==="user"?t.author_id==="me":t.author_id===(n.currentContactId||Q()?.id||"default")}function Fn(e=[]){return e.map(t=>t.author_name||(t.author_type==="user"?"我":b(t.author_id)?.name||t.author_id)).join("、")}function fo(e,t,a){const o=b(n.currentContactId);o&&(o[e]=t,n.toast=a,u(),k(120),window.setTimeout(()=>{n.toast="",u()},1200))}function go(e){const t=String(e).toLowerCase();return["health","heart"].includes(t)?"health":["calendar","schedule","date"].includes(t)?"calendar":["weather","cloud"].includes(t)?"weather":["file","files","doc","document"].includes(t)?"file":["quote","reply"].includes(t)?"quote":(["more","tool","tools"].includes(t),"more")}const Ft={get_current_time:"时间",get_weather:"天气",get_health_summary:"健康",web_search:"搜索",fetch_url:"网页",add_todo:"待办",list_todos:"待办列表",complete_todo:"完成待办",add_note:"便签",list_notes:"便签列表"},Un=new Set(["get_current_time","get_weather","get_health_summary","web_search","fetch_url","add_todo","list_todos","complete_todo","add_note","list_notes"]);function $e(e){return Un.has(String(e||"").trim())}function fe(e,t){if(typeof e=="string"){const r=String(e||`mcp_${t}`);return{id:r,label:Ft[r]||e||`工具${t+1}`,icon:"more",prompt:"",mcpToolId:$e(r)?r:"",enabled:!0}}const a=e?.id||e?.toolId||e?.name||`mcp_${t}`,o=String(a),i=Ft[o]||e?.label||e?.name||e?.title||`工具${t+1}`;return{id:o,label:String(i),icon:go(e?.icon||e?.type||e?.category||"more"),prompt:String(e?.prompt||e?.message||""),mcpToolId:String(e?.mcpToolId||e?.toolId||($e(o)?o:"")),enabled:e?.enabled!==!1}}function Kn(){const t=z()?.mcpLibrary?.tools;if(!Array.isArray(t)||!t.length)return dt;const a=t.map(fe).filter(o=>$e(o.id)).filter(o=>o.enabled!==!1);return a.length?a:dt}function Q(){return b(n.currentContactId)||n.contacts[0]}function Se(e){return e?.settings?(!Array.isArray(e.settings.quickActions)||!e.settings.quickActions.length?e.settings.quickActions=Kn().map((t,a)=>({...fe(t,a)})):e.settings.quickActions=e.settings.quickActions.map((t,a)=>fe(t,a)),e.settings.quickActions):[]}function Ut(e=Q()){const t=Se(e).filter(a=>a.enabled!==!1);return t.length?t:Kn()}function g(e){const t='viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"',a={back:`<svg ${t}><path d="M15 18l-6-6 6-6"/></svg>`,plus:`<svg ${t}><path d="M12 5v14M5 12h14"/></svg>`,search:`<svg ${t}><circle cx="11" cy="11" r="6.5"/><path d="M20 20l-4.2-4.2"/></svg>`,history:`<svg ${t}><path d="M3 12a9 9 0 101.9-5.6"/><path d="M3 4v4h4"/><path d="M12 7v5l3 2"/></svg>`,settings:`<svg ${t}><path d="M12 3v3M12 18v3M4.9 4.9l2.1 2.1M17 17l2.1 2.1M3 12h3M18 12h3M4.9 19.1L7 17M17 7l2.1-2.1"/><circle cx="12" cy="12" r="3.3"/></svg>`,more:`<svg ${t}><circle cx="5" cy="12" r="1.4" fill="currentColor" stroke="none"/><circle cx="12" cy="12" r="1.4" fill="currentColor" stroke="none"/><circle cx="19" cy="12" r="1.4" fill="currentColor" stroke="none"/></svg>`,heart:`<svg ${t}><path d="M12 20.5s-7-4.4-7-10a4 4 0 017-2.5A4 4 0 0119 10.5c0 5.6-7 10-7 10z"/></svg>`,heartFilled:'<svg viewBox="0 0 24 24" fill="#B595C9" stroke="none" stroke-width="0"><path d="M12 20.5s-7-4.4-7-10a4 4 0 017-2.5A4 4 0 0119 10.5c0 5.6-7 10-7 10z"/></svg>',comment:`<svg ${t}><path d="M7 18l-3 2 1-3.8A7.8 7.8 0 014.2 13 7.8 7.8 0 1112 20a8 8 0 01-5-2z"/><path d="M8.5 10.5h7M8.5 13.5h4.5"/></svg>`,chatArrow:`<svg ${t}><path d="M4.8 18.2l.9-3.3A7.5 7.5 0 014.5 11 7.5 7.5 0 1112 18.5a7.4 7.4 0 01-3.6-.9z"/><path d="M10 9l4 3-4 3"/><path d="M14 12H8"/></svg>`,send:`<svg ${t}><path d="M21 3L10 14"/><path d="M21 3l-7 18-4-7-7-4z"/></svg>`,close:`<svg ${t}><path d="M18 6L6 18M6 6l12 12"/></svg>`,camera:`<svg ${t}><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>`,attach:`<svg ${t}><path d="M21 11.5l-8.7 8.7a5 5 0 01-7.1-7.1l9.2-9.2a3.5 3.5 0 015 5L9 19.3a2 2 0 01-2.8-2.8l8.5-8.5"/></svg>`,quote:`<svg ${t}><path d="M9 7H5v5h4v5H4v-5c0-2.8 1.8-5 5-5zM20 7h-4v5h4v5h-5v-5c0-2.8 1.8-5 5-5z"/></svg>`,reroll:`<svg ${t}><path d="M20 11a8 8 0 10-2.3 5.7"/><path d="M20 4v7h-7"/></svg>`,cot:`<svg ${t}><path d="M12 4v16M4 12h16"/><path d="M7.5 7.5l9 9M16.5 7.5l-9 9" opacity="0.18"/></svg>`,bubbleHeart:`<svg ${t}><path d="M12 19.3s-5.8-3.5-5.8-8a3.7 3.7 0 016.1-2.8 3.7 3.7 0 015.9 2.8c0 4.5-5.6 8-5.6 8z"/></svg>`,weather:`<svg ${t}><path d="M6 16a4 4 0 010-8 5.5 5.5 0 0110.4-1.8A4 4 0 1118 16H6z"/></svg>`,calendar:`<svg ${t}><rect x="4" y="5" width="16" height="15" rx="3"/><path d="M8 3v4M16 3v4M4 10h16"/></svg>`,file:`<svg ${t}><path d="M8 3h6l5 5v11a2 2 0 01-2 2H8a2 2 0 01-2-2V5a2 2 0 012-2z"/><path d="M14 3v5h5"/></svg>`,health:`<svg ${t}><path d="M12 20s-6.5-4-6.5-9.2A4.3 4.3 0 0112 7a4.3 4.3 0 016.5 3.8C18.5 16 12 20 12 20z"/><path d="M9.2 12h1.8l1-2.1 1.2 4 1-1.9h1.6"/></svg>`,toggleOff:'<svg viewBox="0 0 52 32" fill="none"><rect x="1.5" y="1.5" width="49" height="29" rx="14.5" fill="rgba(255,255,255,.7)" stroke="rgba(150,122,133,.14)"/><circle cx="16" cy="16" r="11" fill="#fff"/></svg>',toggleOn:'<svg viewBox="0 0 52 32" fill="none"><rect x="1.5" y="1.5" width="49" height="29" rx="14.5" fill="#e9d7ff" stroke="rgba(120,90,150,.14)"/><circle cx="36" cy="16" r="11" fill="#fff"/></svg>',chevron:`<svg ${t}><path d="M9 6l6 6-6 6"/></svg>`,tabChat:`<svg ${t}><path d="M22 12c0-5.5-4.5-10-10-10S2 6.5 2 12c0 2 .6 3.9 1.6 5.4L2 22l4.8-1.3A9.9 9.9 0 0012 22c5.5 0 10-4.5 10-10z"></path></svg>`,tabMoments:`<svg ${t}><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>`,tabSettings:`<svg ${t}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2 2 2 0 01-2-2v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06a1.65 1.65 0 00.33-1.82 1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2 2 2 0 012-2h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06a1.65 1.65 0 001.82.33H9a1.65 1.65 0 001-1.51V3a2 2 0 012-2 2 2 0 012 2v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06a1.65 1.65 0 00-.33 1.82V9a1.65 1.65 0 001.51 1H21a2 2 0 012 2 2 2 0 01-2 2h-.09a1.65 1.65 0 00-1.51 1z"/></svg>`,actionDots:'<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/></svg>',pencil:`<svg ${t}><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>`,trash:`<svg ${t}><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>`,stop:'<svg viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="7" y="7" width="10" height="10" rx="2"/></svg>'};return a[e]||a.more}function Qn(){u()}function Kt(e){const t=()=>{const a=y()?.querySelector(".chat-app-body");a&&(a.scrollTop=e)};requestAnimationFrame(()=>{t(),requestAnimationFrame(t),window.setTimeout(t,0)})}function Yn(){const e=y()?.querySelector(".chat-app-body"),t=e?e.scrollTop:0,a=window.scrollY||window.pageYOffset||0;u(),Kt(t),requestAnimationFrame(()=>{window.scrollTo(0,a),requestAnimationFrame(()=>window.scrollTo(0,a))})}function ne(){if(n.currentView==="moments"){Yn();return}u()}function ho(e,t){n.moments=n.moments.map(a=>{const o=P(a);if(o.id!==e)return a;const r=o.likes.some(s=>s.author_type===t.author_type&&s.author_id===t.author_id)?o.likes.filter(s=>!(s.author_type===t.author_type&&s.author_id===t.author_id)):[{author_type:t.author_type,author_id:t.author_id,author_name:t.author_name},...o.likes];return{...o,likes:r}})}function bo(e,t,a){n.moments=n.moments.map(o=>{const i=P(o);return i.id!==e?o:{...i,comments:[{author_type:t.author_type,author_id:t.author_id,author_name:t.author_name,text:a},...i.comments]}})}function Xn(e,t){e&&(e.classList.toggle("on",!!t),e.classList.toggle("off",!t),e.setAttribute("aria-pressed",t?"true":"false"),e.innerHTML=yt(t),e.classList.remove("switch-animating"),e.offsetWidth,e.classList.add("switch-animating"),clearTimeout(e.__switchAnimTimer),e.__switchAnimTimer=setTimeout(()=>e.classList.remove("switch-animating"),260))}function u(){const e=y();if(!e)return;Gn(),["room","rpRoom"].includes(n.currentView)||(n.showAttach=!1),n.currentView!=="moments"&&(n.momentComposerOpen=!1);const t=e.querySelector(".chat-app-body"),a=t?t.scrollTop:0,o=b(n.currentContactId)||n.contacts[0],i=Bt(o),r=n.globalSettings?.theme||"",s=Ln.includes(r)?r:i;e.dataset.theme=s,e.removeAttribute("data-bound"),e.innerHTML=`
    ...
    `}function So(){const e=[...n.contacts].sort((t,a)=>a.pinned-t.pinned||0);return`
    ...
    `}function ko(e){const t=String(e.handle||(e.id?`@${e.id}`:"")).trim();return`
    ...
    `}async function Io(e){const t=String(e?.sessionId||"").trim();if(t)try{if((await fetch(`${$}/api/sessions/${encodeURIComponent(t)}`)).ok)return;e.sessionId="",k(120)}catch(a){console.warn("[session] open-contact validation failed",a)}}function _o(){n.companionState=Oe({})}function Mo(e){const t=String(e||"").trim();if(!t)return;De?.(),n.streamingAbortController&&n.currentContactId===t&&(n.streamingAbortController.abort(),n.streamingAbortController=null),ve.has(t)&&(clearTimeout(ve.get(t)),ve.delete(t)),n.contacts=n.contacts.filter(i=>i.id!==t),n.activeBubbleToolsId=null,n.quoteMomentId=null,n.quoteMessageId=null,n.contactQuickActionEditorId="",n.quickActionSwipeOpenId="",n.quickActionDragId="",n.quickActionDropHintId="",n.quickActionDropDirection="",n.quickActionReorderPulseId="",n.currentTopicTitle="",n.rpRooms=[],n.currentRpRoomId="",n.currentRpMessages=[];const a=n.contacts[0]||null;(n.currentContactId===t||!b(n.currentContactId))&&(n.currentContactId=a?.id||"",_o(),n.currentView="list",n.currentTab="chats",n.currentSettingsTab="basic");const o=y()?.querySelector(".chat-input");o&&(o.value="")}async function xo(e){const t=String(e||"").trim();if(!t)return!1;const a=await fetch(`${$}/api/agents/${encodeURIComponent(t)}/safe-delete`,{method:"DELETE"});if(!a.ok){let o=`HTTP ${a.status}`;try{o=(await a.json())?.detail||o}catch{}throw new Error(o)}return!0}function Re(){return n.currentContactId||n.contacts[0]?.id||"default"}function Gt(){return n.rpRooms.find(e=>e.room_id===n.currentRpRoomId)||null}function ft(){return{name:"",agentId:"",bio:"",avatar:""}}function ge(e){return String(e||"").trim().replace(/^@+/,"").toLowerCase()}const Co=new Set(["zhansi"]),Ao=new Set(["azheng"]);function Jt(e={}){return[e?.id,e?.agent_id,e?.handle].map(ge).filter(Boolean).some(a=>Co.has(a))}function gt(e={}){return Jt(e)&&!!e?.settings?.codexEnabled}function Wt(e={}){return[e?.id,e?.agent_id,e?.handle].map(ge).filter(Boolean).some(a=>Ao.has(a))}function ht(e={}){return Wt(e)&&!!e?.settings?.ccEnabled}function Y(e={}){const t=String(e.id||"").trim()||`c${Date.now()}`,a=Bt(e);return{id:t,agent_id:String(e.agent_id||e.id||t),name:String(e.name||t),display_name:String(e.display_name||e.name||t),bio:String(e.bio||"这是新来的联系人"),status:String(e.status||"在线"),handle:String(e.handle||`@${t}`),roleTag:String(e.roleTag||""),theme:na(a),chatTheme:a,bubbleTheme:zt(a),unread:Number(e.unread||0),pinned:!!e.pinned,lastMessage:String(e.lastMessage||""),lastTime:String(e.lastTime||""),avatar:String(e.avatar||"https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&q=80"),topics:Array.isArray(e.topics)?e.topics:[],messages:Array.isArray(e.messages)?e.messages:[],settings:{model:"gpt-5.4",modelProviderId:A("chat")?.providerId||"openai",temperature:.7,topP:.9,contextCount:32,thinkBudget:24,streamOutput:!0,reasoning_visibility:!1,proactiveEnabled:!1,proactiveFrequency:30,memoryEnabled:!0,codexEnabled:!1,ccEnabled:!1,...e.settings||{}}}}function Jn(e){const t=Y(e),a=n.contacts.findIndex(o=>String(o.id||"").toLowerCase()===t.id.toLowerCase());return a>=0?n.contacts[a]={...n.contacts[a],...t}:n.contacts.unshift(t),t}async function To(e){try{const t=await fetch(`${$}/api/agents`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agent_id:e.id,display_name:e.name,avatar:e.avatar||"",description:e.bio||"",source:"murmur",metadata:{from:"murmur_contact"}})});if(t.ok)return!0;let a="";try{const i=await t.json();a=typeof i?.detail=="string"?i.detail:JSON.stringify(i?.detail||i)}catch{}return t.status===409||/already exists|duplicate|23505/i.test(a)?(fetch(`${$}/api/agents/${encodeURIComponent(e.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({display_name:e.name,avatar:e.avatar||"",description:e.bio||"",source:"murmur",is_active:!0})}).catch(()=>{}),!0):!1}catch(t){return console.warn("[agents] register contact failed",t),!1}}function Wn(e){return ce(e,{fallback:""})}function ce(e,{fallback:t="",includeYear:a=!1}={}){if(!e)return t;const o=String(e||"").trim();if(!o)return t;const i=new Date(o);if(Number.isNaN(i.getTime()))return o;const r=new Date,s=i.getFullYear()===r.getFullYear(),c=i.toDateString()===r.toDateString(),l=new Date(r);l.setDate(r.getDate()-1);const p=i.toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:!1});if(c)return`今天 ${p}`;if(i.toDateString()===l.toDateString())return`昨天 ${p}`;const m=a||!s?{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:!1}:{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:!1};return i.toLocaleString("zh-CN",m).replace(/\//g,"-")}async function bt(e=Re(),{silent:t=!0}={}){try{const a=await fetch(`${$}/api/rp/rooms?agent_id=${encodeURIComponent(e)}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);const o=await a.json();return n.rpRooms=Array.isArray(o.rooms)?o.rooms:[],t||u(),n.rpRooms}catch(a){return console.warn("[rp] load rooms failed",a),t||(n.toast="RP 鎴块棿鍔犺浇澶辫触",u(),window.setTimeout(()=>{n.toast="",u()},1200)),[]}}async function Eo(e,{silent:t=!0}={}){if(!e)return[];try{const a=await fetch(`${$}/api/rp/rooms/${encodeURIComponent(e)}/messages`);if(!a.ok)throw new Error(`HTTP ${a.status}`);const o=await a.json(),i=o.room||n.rpRooms.find(s=>s.room_id===e);if(i){const s=n.rpRooms.findIndex(c=>c.room_id===e);s>=0&&(n.rpRooms[s]=i)}const r=(Array.isArray(o.messages)?o.messages:[]).map(s=>({id:s.id,role:s.role==="assistant"?"ai":s.role,text:s.content||"",content:s.content||"",time:Wn(s.timestamp),timestamp:s.timestamp||"",created_at:s.timestamp||""}));return n.currentRpMessages=le(n.rpMessages?.[e]||[],r).map(F),n.rpMessages={...n.rpMessages||{},[e]:n.currentRpMessages.map(G)},k(120),t||u(),n.currentRpMessages}catch(a){return console.warn("[rp] load messages failed",a),n.currentRpMessages=(n.rpMessages?.[e]||[]).map(F),t||(n.toast="RP 娑堟伅鍔犺浇澶辫触",u(),window.setTimeout(()=>{n.toast="",u()},1200)),[]}}async function Po(e=n.currentView==="room"?"room":"list",t=Re()){n.rpBackView=e,n.currentView="rpLobby",n.currentTab="chats",u(),await bt(t,{silent:!1})}async function qo(){const e=y()?.querySelector("#rp-room-name")?.value?.trim()||"",t=y()?.querySelector("#rp-room-world")?.value?.trim()||"",a=y()?.querySelector("#rp-room-user-role")?.value?.trim()||"",o=y()?.querySelector("#rp-room-ai-role")?.value?.trim()||"",i={agent_id:Re(),name:e||"新房间",world_setting:t,user_role:a,ai_role:o},r=n.rpRoomDialogMode==="edit"?n.currentRpRoomId:"",s=r?`${$}/api/rp/rooms/${encodeURIComponent(r)}`:`${$}/api/rp/rooms`,l=await fetch(s,{method:r?"PATCH":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!l.ok)throw new Error(`HTTP ${l.status}`);const m=(await l.json()).room;return n.rpRoomDialogOpen=!1,await bt(Re(),{silent:!0}),m?.room_id&&(n.currentRpRoomId=m.room_id,!r)?(await Zn(m.room_id),m):(u(),m)}async function Zn(e){e&&await vo(async()=>{n.currentRpRoomId=e,n.currentView="rpRoom",n.currentTab="chats",n.showAttach=!1,u(),await Eo(e,{silent:!1})})}async function Lo(e){if(!e||!window.confirm("删除这个 RP 房间？"))return;const a=await fetch(`${$}/api/rp/rooms/${encodeURIComponent(e)}`,{method:"DELETE"});if(!a.ok)throw new Error(`HTTP ${a.status}`);n.rpRooms=n.rpRooms.filter(o=>o.room_id!==e),n.currentRpRoomId===e&&(n.currentRpRoomId="",n.currentRpMessages=[],n.currentView="rpLobby"),u()}function ea(e){const t=n.contacts.find(a=>a.id===e);t&&(t.unread=0),n.currentContactId=e,n.currentTab="chats",n.currentView="room",n.activeBubbleToolsId=null,u(),t&&Io(t),t&&We(e),pe(e),at(e)}function Do(e=80){window.setTimeout(()=>{const t=b(n.currentContactId)||n.contacts[0];t?.id&&We(t.id,{silent:!1})},e)}function ta(e={}){const t=[e?.id,e?.agent_id,e?.agentId,e?.handle,String(e?.handle||"").replace(/^@+/,"")],a=[];return t.forEach(o=>{const i=String(o||"").trim();i&&(a.push(i),a.push(ge(i)))}),[...new Set(a.filter(Boolean))]}function vt(e={}){const t=n.conversations||{},a=ta(e).flatMap(o=>t[o]||[]);return le(a,Array.isArray(e.messages)?e.messages:[])}function Oo(e={}){n.currentView!=="room"||!e?.id||vt(e).length||n.historyLoadingContactIds[e.id]||n.historyLoadedContactIds[e.id]||(n.historyLoadingContactIds[e.id]=!0,We(e.id).then(t=>{t&&(n.historyLoadedContactIds[e.id]=!0)}).finally(()=>{delete n.historyLoadingContactIds[e.id]}))}function Ro(e){const t=d(e?.label||""),a=e?.icon||"more";return`
    ...
    `}function Vo(){const e=b(n.currentContactId)||n.contacts[0],t=n.quoteMomentId?Ue(n.quoteMomentId):null,a=n.quoteMessageId?e.messages.find(r=>r.id===n.quoteMessageId):null,o=Bo(vt(e)),i=(n.chatAttachments||[]).map(Be).filter(Boolean);return`
      <section class="room-page room-theme-${e.theme}">
        <div class="messages-panel">
          ${o.map((r,s)=>Ho(r,e,zo(o,s))).join("")}
        </div>
    ...
    `}function Bo(e=[]){return le([],e).map(F).filter(Mt)}function zo(e=[],t=0){const a=e[t]||{},o=e[t-1]||null,i=o?Math.abs(X(a.created_at||a.timestamp)-X(o.created_at||o.timestamp)):0;return{showTime:!o||!ma(o,a)||i>300*1e3}}function Ho(e,t,a={}){if(!Mt(e))return"";const o=e.role==="user"?"from-user":"from-ai",i=String(e.source||e.provider||"").toLowerCase(),r=i==="codex",s=i==="claude-code",c=!!t?.settings?.reasoning_visibility,l=e.role==="ai"?`<img class="bubble-avatar" src="${t.avatar}" alt="${d(t.name)}" />`:"",p=e.role==="ai"&&(r||s)?`<span class="message-source-badge ${r?"codex":"claude-code"}">${r?"Codex":"Claude"}</span>`:"",m=e.role==="ai"&&c&&e.thinking&&!e.typing?`<button class="bubble-cot-btn" data-action="toggle-thinking" data-id="${e.id}" aria-label="展开独白">${g("bubbleHeart")}</button>`:"",f=e.role==="ai"&&!e.typing&&!e.streaming?`
      ...
      `:"",v=`${e.role==="ai"&&e.streaming&&!e.text?" message-awaiting-text":""}${m?" has-cot":""}`,S=c&&e.thinking?co(e):"",I=e.toolCalls&&e.toolCalls.length?uo(e.toolCalls):"",T=ke(e),x=aa(e),_=$i(x),C=a.showTime&&e.time&&!e.typing,V=x.length||T.length>18||T.includes(`
`)?"block-time":"tail-time",oe=C?`<time class="bubble-time ${V}">${d(e.time)}</time>`:"",E=e.role==="ai"&&p,O=`
          <div class="message-bubble-wrap">
            ...
            ${E?`<div class="bubble-meta-row">
              ${p}
            </div>`:""}
          </div>`,B=e.role==="ai"&&(S||I)?`${S}${I}${O}${f}`:`${O}${f}${S}${I}`;return`
    ...
    `}function jo(){const e=Array.isArray(n.moments)?n.moments:[];return b(n.currentContactId)||n.contacts[0],`
    ...
    `}function Uo(e){const t=P(e),a=Nn(t),o=jn(t),i=we(),r=t.likes.some(s=>s.author_type===i.author_type&&s.author_id===i.author_id);return`
    ...
    `}function Ko(){return`<div class="app-toast glass-frost">${d(n.toast)}</div>`}function Qo(){const e=n.avatarCropper||{},t=ie(e.x),a=ie(e.y),o=Qe(e.zoom);return`
    ...
    `}function Yo(){const e=n.rpRoomDialogMode==="edit",t=n.rpRoomForm||{};return`
    ...
    `}function Xo(){const e=b(n.currentContactId)||n.contacts[0],t=Gt(),a=t?`${t.world_setting||"未设定"} · 你：${t.user_role||"未设定"} · TA：${t.ai_role||"未设定"}`:"房间设定载入中";return`
      <section class="rp-room-stage">
        ...
        <div class="messages-area">
          ${n.currentRpMessages.map(o=>Go(o,e)).join("")}
        </div>
    ...
    `}function Go(e,t){const a=e.role==="user",o=a&&n.accountProfile?.avatar||t.avatar;return`
    ...
    `}function Jo(e){const t=String(e||"");return t.trim()?t.split(/(\[[\s\S]*?\]|［[\s\S]*?］)/g).filter(Boolean).map(o=>`<span class="${/^\s*(\[|［)/.test(o)?"rp-action":"rp-dialogue"}">${d(o)}</span>`).join(""):""}function Wo(){return`
    ...
    `}function Zo(){const e=b(n.currentContactId)||n.contacts[0],t=e.settings;return`
      <section class="contact-settings-page page-block">
        ...
        ${n.currentSettingsTab==="memory"?`
          <div class="settings-group glass-frost ai-panel">
            ...
            ${M("当前状态",mo(),"open-companion-state")}
        ...
        `:""}
        ${n.contactQuickActionEditorId?si(e,n.contactQuickActionEditorId):""}
      </section>
    `}function ei(){const e=Oe(n.companionState),t=e.recent_topics.length?e.recent_topics.join(" / "):"还没有东西",a=e.current_mood||"还没有东西",o=e.open_loops.length?e.open_loops.join(" / "):"还没有东西",i=ce(e.proactive_cooldown_until,{fallback:e.proactive_cooldown_until||"还没有东西"}),r=ce(e.updated_at,{fallback:e.updated_at||"还没有东西"});return`
      <section class="topics-page page-block">
    ...
    `}function na(e){return Dn(e).roomTheme||"rose"}function ti(){const t=(b(n.currentContactId)||n.contacts[0])?.roomBackground||"点阵";return`
      <section class="settings-page page-block ai-settings-page">
    ...
    `}function ni(){const e=b(n.currentContactId)||n.contacts[0],t=Bt(e);return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>气泡主题</h3>
    ...
    `}function ai(){const e=n.newContactDraft||{};return`
    ...
    `}function oi(){const e=b(n.currentContactId)||n.contacts[0],t=e.settings?.model||n.globalSettings.defaultModel||"gpt-5.4",a=Number(e.messageCount||e.messages?.length||0);return`
    ...
    `}function wt(e,t){return`<button class="settings-tab ${n.currentSettingsTab===e?"active":""}" data-action="switch-settings-tab" data-tab="${e}">${d(t)}</button>`}function ri(e,t){const o=Se(e).find(p=>p.id===t);if(!o)return"";const i=(z().mcpLibrary?.tools||[]).map(fe).filter(p=>$e(p.id)),r=i.length?i:[...Un].map(p=>fe({id:p,label:Ft[p]||p},0)),s=o.mcpToolId||"",c=r.find(p=>p.id===s),l=[{id:"",label:"不调用 MCP"},...r];return`
    ...
    `}function si(e,t){const a=ri(e,t);return a?`
    ...
    `:""}function ci(e){const t=Q();Se(t),n.contactQuickActionEditorId=e||"",n.quickActionSwipeOpenId="",n.quickActionDropHintId="",u()}function ui(e){const t={ayan:[{id:"cot_1",mode:"主动",badge:"意识循环",accent:"violet",score:"↓ 4.2k",latency:"197s",amount:"$1.05",time:"2026.03.26 15:00",summary:'[THINK] 她在下午1:22读了两封日记，id=23"...',steps:[{type:"thought",label:"思考",text:"她沉默了快13个小时，两封日记都没被读。我发了三条消息都…"},{type:"thought",label:"思考",text:"下午三点了。她沉默了快13个小时。先看看日记有没有被读。"},{type:"note",label:"留言小纸条",text:"妫ｅ啯鎲?你醒了先看这个"},{type:"tool",label:"工具调用",text:"read_diary"},{type:"result",label:"工具结果",text:"read_diary"}]},{id:"cot_2",mode:"回复",badge:"工具",accent:"gold",score:"↑ 3.5k",latency:"146s",amount:"$0.54",time:"2026.03.26 15:07",summary:'[THINK] 她在下午1:22读了两封日记，id=23"给你的"...',steps:[{type:"reply",label:"回复",text:'[THINK] 她在下午1:22读了两封日记，id=23"给你的"和id…'},{type:"tool",label:"工具调用",text:"pc_control"},{type:"result",label:"工具结果",text:"pc_control"}]},{id:"cot_3",mode:"主动",badge:"工具",accent:"blue",score:"↑ 1.1k",latency:"53s",amount:"$0.073",time:"2026.03.26 16:10",summary:"[THINK] 她在看芒果TV，左看综艺，弹幕开着。她一个半小时前读完了...",steps:[{type:"thought",label:"思考",text:"她在看芒果TV，左看综艺。弹幕开着，说明现在状态比较轻松。"},{type:"tool",label:"工具调用",text:"pc_control"},{type:"result",label:"工具结果",text:"pc_control"}]}]};return t[e]||t.ayan}function li(e=""){return e==="activity_event"?"violet":e==="proactive_message"?"gold":e==="cot_log"?"blue":"neutral"}function di(e=""){return e==="activity_event"?"被动":e==="proactive_message"?"主动":e==="cot_log"?"日志":"记录"}function pi(e={}){return e.kind==="activity_event"?e.eventType||e.source||"事件":e.kind==="proactive_message"?e.title||"主动消息":e.kind==="cot_log"?e.logType||e.toolName||"COT":e.title||"记录"}function mi(e=""){return ce(e,{fallback:String(e||""),includeYear:!0})}function fi(e={}){const t=e.raw||{},a=[];if(e.kind==="activity_event")a.push({type:"thought",label:"事件",text:e.summary||e.title||""}),(e.gateStatus||e.messageHint||e.shouldHandle||e.shouldNotifyLlm)&&a.push({type:e.shouldHandle||e.shouldNotifyLlm?"result":"thought",label:"筛选",text:`${e.shouldHandle?"需要处理":"静默"}${e.shouldNotifyLlm?" / 可通知大模型":""}${e.messageHint?`：${e.messageHint}`:""}`}),t.gate_reason&&a.push({type:"thought",label:"原因",text:t.gate_reason});else if(e.kind==="proactive_message")a.push({type:"reply",label:"主动消息",text:e.summary||""}),t.reason_context&&a.push({type:"thought",label:"依据",text:String(t.reason_context).slice(0,220)});else{const o=e.toolName?"工具调用":"日志";a.push({type:e.toolName?"tool":"thought",label:o,text:e.summary||e.title||""}),t.content&&a.push({type:e.toolName?"result":"thought",label:"内容",text:String(t.content).slice(0,500)})}return{id:String(e.id||`${e.kind}_${e.occurredAt||e.createdAt||Date.now()}`),mode:di(e.kind),badge:pi(e),accent:li(e.kind),score:e.shouldHandle||e.shouldNotifyLlm?"有效":"",latency:"",amount:e.source||"",time:mi(e.occurredAt||e.createdAt),summary:e.summary||e.title||"",steps:a.filter(o=>String(o.text||"").trim())}}async function gi({silent:e=!0}={}){const t=b(n.currentContactId)||n.contacts[0];n.activityLogLoading=!0,e||u();try{const a=new URLSearchParams({hours:"24",limit:"50",agent_id:t?.id||n.currentContactId||""});t?.sessionId&&a.set("session_id",t.sessionId);const o=await fetch(`${$}/api/activity-log/recent?${a.toString()}`);if(!o.ok)throw new Error(`HTTP ${o.status}`);const i=await o.json().catch(()=>({}));n.activityLogEntries=Array.isArray(i.items)?i.items.map(fi):[],n.activityLogLoadedAt=new Date().toISOString()}catch(a){console.warn("[activity log] load failed",a),e||(n.toast="活动日志加载失败")}finally{n.activityLogLoading=!1,u(),n.toast&&window.setTimeout(()=>{n.toast="",u()},1200)}}function hi(e){return`
      <div class="cot-log-step ${e.type}">
        <span class="cot-log-step-label">${d(e.label)}</span>
        <span class="cot-log-step-text">${d(e.text)}</span>
      </div>
    `}function bi(){const e=b(n.currentContactId)||n.contacts[0],t=n.cotLogMode==="note",a=n.activityLogLoadedAt?n.activityLogEntries:ui(e.id),o=a.filter(r=>n.cotLogMode==="short"?r.mode!=="主动":n.cotLogMode==="note"?r.steps.some(s=>s.type==="note"):!0),i=a.filter(r=>r.steps.some(s=>s.type==="note")).length;return`
    ...
    `}function tn(e){return!e||typeof e.closest!="function"?!1:!!e.closest('input:not([type="range"]):not([type="checkbox"]):not([type="radio"]):not([type="file"]), textarea, select, [contenteditable="true"]')}function vi(){return!!(window.matchMedia?.("(pointer: coarse)").matches||"ontouchstart"in window||navigator.maxTouchPoints>0)}function $t(e){return!!e&&/^image\/(png|jpe?g|webp|gif|heic|heif)$/i.test(e.type||"")}function Be(e){return e?{id:e.id,kind:e.kind||"image",type:e.type||"image/*",name:e.name||"image",size:Number(e.size||0),url:e.url||""}:null}function aa(e={}){return Array.isArray(e.attachments)?e.attachments.map(Be).filter(t=>t&&t.url):[]}function yi(e={}){return aa(e).length>0}function nn(e,t=[]){const a=String(e||"").trim();if(!t.length)return a;const i=`[图片附件：${t.map(r=>r.name||"image").join(", ")}]`;return a?`${a}
${i}`:i}function an(e,t=[]){const a=String(e||"").trim();return a||(t.length?"[图片]":"")}function oa(e,t={}){const a=Be(e);return a?.url?`
    ...
    `:""}function wi(e=[]){return e.length?`
      <div class="chat-attachment-tray">
        ${e.map(t=>oa(t,{removable:!0})).join("")}
      </div>
    `:""}function $i(e=[]){return e.length?`<div class="message-attachment-grid">${e.map(t=>oa(t)).join("")}</div>`:""}function Si(e){return new Promise((t,a)=>{if(!$t(e)){a(new Error("只支持图片附件"));return}const o=new FileReader;o.onerror=()=>a(new Error("图片读取失败")),o.onload=()=>t({id:`att_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,kind:"image",type:e.type||"image/*",name:e.name||"pasted-image",size:e.size||0,url:typeof o.result=="string"?o.result:""}),o.readAsDataURL(e)})}async function on(e=[]){const t=Array.from(e).filter($t);if(!t.length)return!1;try{const a=await Promise.all(t.map(Si));return n.chatAttachments=[...n.chatAttachments||[],...a].slice(0,6),n.chatPasteError="",n.showAttach=!1,u(),!0}catch(a){return console.warn("[chat] image attach failed",a),n.chatPasteError=a?.message||"图片添加失败",n.toast=n.chatPasteError,u(),window.setTimeout(()=>{n.toast="",u()},1400),!1}}function ia(e,t){if(!e||!t)return;const a=String(e.value||""),o=typeof e.selectionStart=="number"?e.selectionStart:a.length,i=typeof e.selectionEnd=="number"?e.selectionEnd:o;e.value=`${a.slice(0,o)}${t}${a.slice(i)}`;const r=o+t.length;e.setSelectionRange?.(r,r),e.dispatchEvent(new Event("input",{bubbles:!0}))}function ki(e){if(!e)return"";const t=document.createElement("div");return t.innerHTML=e,(t.textContent||t.innerText||"").replace(/\n{3,}/g,`

`)}async function Ii(e){if(n.currentView!=="room")return;const t=e.clipboardData;if(!t)return;const a=Array.from(t.files||[]).filter($t),o=Array.from(t.items||[]).filter(c=>c.kind==="file"&&/^image\//i.test(c.type||"")).map(c=>c.getAsFile()).filter($t),i=[...a,...o].filter((c,l,p)=>l===p.findIndex(m=>m.name===c.name&&m.size===c.size&&m.type===c.type)),r=t.getData("text/plain")||"",s=t.getData("text/html")||"";if(i.length){e.preventDefault(),await on(i),r.trim()&&ia(e.currentTarget,r);return}s&&(e.preventDefault(),ia(e.currentTarget,r||ki(s)))}function _i(e,t){t&&(n.avatarCropper={kind:e,src:t,x:50,y:50,zoom:1},u())}function ie(e){const t=Number(e);return Number.isFinite(t)?Math.min(100,Math.max(0,t)):50}function Qe(e){const t=Number(e);return Number.isFinite(t)?Math.min(2.4,Math.max(1,t)):1}function ra(){const e=n.avatarCropper;if(!e)return;e.x=ie(e.x),e.y=ie(e.y),e.zoom=Qe(e.zoom);const t=y(),a=t?.querySelector(".avatar-cropper-image");a&&(a.style.objectPosition=`${e.x}% ${e.y}%`,a.style.transform=`scale(${e.zoom})`),t?.querySelectorAll('[data-action="avatar-cropper-range"]').forEach(o=>{const i=o.dataset.key;i&&i in e&&(o.value=String(e[i]))})}function Mi(e){const t=e.target?.closest?.(".avatar-cropper-viewport"),a=n.avatarCropper;!t||!a||(e.preventDefault(),n.avatarCropDrag={pointerId:e.pointerId,startClientX:e.clientX,startClientY:e.clientY,startX:ie(a.x),startY:ie(a.y)},t.setPointerCapture?.(e.pointerId))}function xi(e){const t=n.avatarCropDrag,a=n.avatarCropper,o=y()?.querySelector(".avatar-cropper-viewport");if(!t||!a||!o||t.pointerId!==e.pointerId)return;e.preventDefault();const i=o.getBoundingClientRect(),r=Qe(a.zoom),s=i.width?(e.clientX-t.startClientX)/i.width*100/r:0,c=i.height?(e.clientY-t.startClientY)/i.height*100/r:0;a.x=ie(t.startX-s),a.y=ie(t.startY-c),ra()}function sa(e){const t=n.avatarCropDrag;!t||t.pointerId!==e.pointerId||(n.avatarCropDrag=null)}function rn(e,t){if(!e)return;const a=new FileReader;a.onload=()=>{const o=typeof a.result=="string"?a.result:"";_i(t,o)},a.readAsDataURL(e)}function Ci(e){return new Promise((t,a)=>{const o=new Image;o.onload=()=>{const r=document.createElement("canvas");r.width=512,r.height=512;const s=r.getContext("2d");if(!s){a(new Error("canvas unavailable"));return}const c=Qe(e.zoom),l=Math.max(512/o.naturalWidth,512/o.naturalHeight),p=o.naturalWidth*l*c,m=o.naturalHeight*l*c,f=ie(e.x)/100,h=ie(e.y)/100,v=(512-p)*f,S=(512-m)*h;s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(o,v,S,p,m),t(r.toDataURL("image/jpeg",.9))},o.onerror=a,o.src=e.src})}async function Ai(){const e=n.avatarCropper;if(e?.src)try{const t=await Ci(e);if(e.kind==="new-contact")n.newContactDraft={...n.newContactDraft||ft(),avatar:t},n.newContactAvatar=t;else if(e.kind==="account")n.accountProfile.avatar=t,D(),k(120);else if(e.kind==="contact"){const a=b(n.currentContactId);a&&(a.avatar=t,k(120))}n.avatarCropper=null,n.toast="头像已更新",u(),window.setTimeout(()=>{n.toast="",u()},1200)}catch{n.toast="头像裁切失败",u(),window.setTimeout(()=>{n.toast="",u()},1200)}}function Ti(){const e=y();if(!e||e.dataset.bound==="1")return;e.dataset.bound="1",e.addEventListener("click",sn),e.addEventListener("input",Ei),e.addEventListener("pointerdown",Mi),e.addEventListener("pointermove",xi),e.addEventListener("pointerup",sa),e.addEventListener("pointercancel",sa);let t;const a=f=>{if(tn(f.target))return;const h=f.target.closest(".message-bubble.from-ai");h&&(t=window.setTimeout(()=>{const v=h.dataset.msgId;if(b(n.currentContactId)?.messages?.find(T=>T.id===v)?.text){n.quoteMomentId=null,n.quoteMessageId=v,u();const T=y()?.querySelector(".chat-input");T&&T.focus()}n.activeBubbleToolsId=v,n.suppressBubbleToggle=!0,navigator.vibrate&&navigator.vibrate(50)},550))},o=()=>clearTimeout(t);e.addEventListener("touchstart",a,{passive:!0}),e.addEventListener("touchend",o),e.addEventListener("touchmove",o,{passive:!0}),e.addEventListener("mousedown",a),e.addEventListener("mouseup",o),e.addEventListener("mousemove",o),e.addEventListener("mouseleave",o);const i=e.querySelector(".send-round");i&&i.addEventListener("click",f=>{f.stopPropagation(),n.streamingAbortController?(n.streamingAbortController.abort(),n.streamingAbortController=null,u()):n.currentView!=="rpRoom"&&ht(b(n.currentContactId))?$n():n.currentView!=="rpRoom"&&gt(b(n.currentContactId))?Sn():kn()});const r=e.querySelector(".soft-mini");r&&r.addEventListener("click",f=>{f.stopPropagation(),n.showAttach=!n.showAttach,u()});const s=e.querySelector(".codex-toggle:not(.cc-toggle)");s&&s.addEventListener("click",f=>{f.preventDefault(),f.stopPropagation(),St()});const c=e.querySelector(".cc-toggle");c&&c.addEventListener("click",f=>{f.preventDefault(),f.stopPropagation(),qt()}),e.querySelectorAll(".chat-list-item[data-contact-id]").forEach(f=>{f.addEventListener("click",h=>{h.preventDefault(),h.stopPropagation(),ea(f.dataset.contactId)})});const p=e.querySelector(".chat-input");p&&(p.addEventListener("paste",Ii),p.addEventListener("keydown",f=>{f.key==="Enter"&&!f.shiftKey&&(f.preventDefault(),n.currentView!=="rpRoom"&&ht(b(n.currentContactId))?$n():n.currentView!=="rpRoom"&&gt(b(n.currentContactId))?Sn():kn())}),["room","rpRoom"].includes(n.currentView)&&!vi()&&p.focus());const m=e.querySelector("#chat-image-input");m&&m.addEventListener("change",async f=>{await on(f.target.files||[]),f.target.value=""})}function St(e=n.currentContactId){const t=b(e)||b(n.currentContactId);if(t){if(n.currentContactId=t.id,!Jt(t)){t.settings={...t.settings||{},codexEnabled:!1},n.toast="只有阿湛能切 Codex",u(),window.setTimeout(()=>{n.toast="",u()},1200);return}t.settings={...t.settings||{},codexEnabled:!t.settings?.codexEnabled},n.toast=t.settings.codexEnabled?"Codex 已接管这个窗口":"Codex 已关闭",k(120),u(),window.setTimeout(()=>{n.toast="",u()},1200)}}window.__yuiToggleCodex=(e,t)=>{t?.preventDefault?.(),t?.stopPropagation?.(),t?.stopImmediatePropagation?.();const a=e?.dataset?.contactId||n.currentContactId;St(a)},window.__yuiToggleCC=(e,t)=>{t?.preventDefault?.(),t?.stopPropagation?.(),t?.stopImmediatePropagation?.();const a=e?.dataset?.contactId||n.currentContactId;qt(a)};async function sn(e){const t=e.target.closest("[data-action]");if(!t)return;const a=t.dataset.action;if(a==="cancel-avatar-cropper"){n.avatarCropper=null,n.avatarCropDrag=null,u();return}if(a==="apply-avatar-cropper"){e.preventDefault(),e.stopPropagation(),await Ai();return}if(a==="switch-tab"&&(n.currentTab=t.dataset.tab,n.currentView=t.dataset.tab==="chats"?"list":t.dataset.tab,u()),a==="open-contact"){ea(t.dataset.contactId);return}if(a==="back-list"&&(n.currentView="list",n.currentTab="chats",n.quoteMomentId=null,u()),a==="back-room"&&(n.currentView="room",u()),a==="open-contact-settings"&&(n.currentSettingsTab="basic",n.currentView="contactSettings",u(),pe(),at(n.currentContactId)),a==="open-cot-log"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="cotLog",n.cotLogMode="long",n.activityLogLoadedAt="",n.activityLogEntries=[],u(),gi({silent:!0});return}if(a==="back-contact-settings"){n.currentView="contactSettings",n.currentSettingsTab=n._prevContactSettingsTab||n.currentSettingsTab||"basic",n._prevContactSettingsTab=null,u();return}if(a==="switch-cot-log-mode"){n.cotLogMode=t.dataset.mode||"long",u();return}if(a==="open-rp-lobby"){Po(n.currentView==="room"?"room":"list",Re());return}if(a==="back-rp-source"){n.currentView=n.rpBackView||"list",u();return}if(a==="back-rp-lobby"){n.currentView="rpLobby",u();return}if(a==="open-rp-room-create"){n.rpRoomDialogMode="create",n.rpRoomForm={name:"",world_setting:"",user_role:"",ai_role:""},n.rpRoomDialogOpen=!0,u();return}if(a==="close-rp-room-dialog"){if(t.dataset.rpRoomDialog==="card"||e.target&&e.target!==t)return;n.rpRoomDialogOpen=!1,u();return}if(a==="save-rp-room"){try{await qo(),n.toast=n.rpRoomDialogMode==="edit"?"幕间已更新":"已入梦"}catch(o){console.warn("[rp] save room failed",o),n.toast="房间保存失败"}u(),window.setTimeout(()=>{n.toast="",u()},1200);return}if(a==="open-rp-room"){e.preventDefault(),e.stopPropagation(),await Zn(t.dataset.roomId);return}if(a==="delete-rp-room"){e.preventDefault(),e.stopPropagation();try{await Lo(t.dataset.roomId),n.toast="房间已删除"}catch(o){console.warn("[rp] delete room failed",o),n.toast="删除失败"}u(),window.setTimeout(()=>{n.toast="",u()},1200);return}if(a==="rename-rp-room"){e.preventDefault(),e.stopPropagation();const o=t.dataset.roomId,i=n.rpRooms.find(s=>s.room_id===o),r=window.prompt("剧本",i?.name||"")?.trim();if(!r||!o)return;try{const s=await fetch(`${$}/api/rp/rooms/${encodeURIComponent(o)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:r})});if(!s.ok)throw new Error(`HTTP ${s.status}`);await bt(Re(),{silent:!0}),n.toast="房间已重命名"}catch(s){console.warn("[rp] rename room failed",s),n.toast="重命名失败"}u(),window.setTimeout(()=>{n.toast="",u()},1200);return}if(a==="open-profile"&&(n.currentView="profile",u()),a==="stop-streaming"){n.streamingAbortController&&(n.streamingAbortController.abort(),n.streamingAbortController=null);return}if(a==="toggle-thinking-line"){const o=t.closest("[data-id]")?.dataset.id||t.dataset.id,i=y()?.querySelector(`#tl-line-${o}`),r=y()?.querySelector(`#tl-full-${o}`);if(!i||!r)return;const s=r.classList.contains("tl-open");r.classList.toggle("tl-open",!s),i.classList.toggle("tl-expanded",!s);return}if(a==="toggle-thinking"){const o=t.dataset.id,r=!!!n.openThinkingIds[o];n.openThinkingIds[o]=r;const s=document.getElementById(`thinking-${o}`);s?(s.classList.toggle("open",r),s.setAttribute("aria-hidden",r?"false":"true"),t.setAttribute("aria-expanded",r?"true":"false")):u()}if(a==="toggle-message-tools"){if(e.preventDefault(),e.stopPropagation(),n.suppressBubbleToggle){n.suppressBubbleToggle=!1;return}const o=t.dataset.id,i=n.activeBubbleToolsId===o?null:o;n.activeBubbleToolsId=i;const r=y();r&&(r.querySelectorAll(".bubble-bottom-tools.open").forEach(s=>{s.classList.remove("open")}),i&&r.querySelector(`.message-row[data-msg-id="${CSS.escape(i)}"] .bubble-bottom-tools`)?.classList.add("open"));return}if(a==="go-chat-with-quote"&&(n.currentContactId=t.dataset.contactId,n.quoteMomentId=t.dataset.momentId,n.quoteMessageId=null,n.currentTab="chats",n.currentView="room",u(),We(n.currentContactId),pe(n.currentContactId),at(n.currentContactId)),a==="open-comments"){e.preventDefault(),e.stopPropagation(),t.blur?.();const o=t.dataset.momentId,i=n.commentSheetMomentId===o?null:o;n.commentSheetMomentId=i;const r=y();if(r&&(r.querySelectorAll(".moment-inline-comment.open").forEach(s=>s.classList.remove("open")),i)){const s=r.querySelector(`.moment-inline-comment .moment-comment-input[data-comment-input="${i}"]`)?.closest(".moment-inline-comment");s&&s.classList.add("open")}return}if(a==="submit-comment"){e.preventDefault(),e.stopPropagation();const o=t.dataset.momentId,r=y()?.querySelector(`[data-comment-input="${o}"]`)?.value?.trim();if(!o||!r)return;try{const s=await Lr(o,we(),r);n.moments=n.moments.map(c=>c.id===o?s:c),n.commentSheetMomentId=null,n.toast="已发送评论",k(120),ne(),window.setTimeout(()=>{n.toast="",ne()},1200)}catch(s){console.warn("[moments] comment failed",s),bo(o,we(),r),n.commentSheetMomentId=null,n.toast="已发送评论",k(120),ne(),window.setTimeout(()=>{n.toast="",ne()},1200)}return}if(a==="like-moment"){e.preventDefault(),e.stopPropagation();const o=t.dataset.momentId;if(!o)return;try{const i=await qr(o,we());n.moments=n.moments.map(r=>r.id===o?i:r),k(120),ne()}catch(i){console.warn("[moments] like failed",i),ho(o,we()),k(120),ne()}return}if(a==="submit-comment"){const o=Ue(t.dataset.momentId),r=y()?.querySelector(`[data-comment-input="${t.dataset.momentId}"]`)?.value?.trim();o&&r&&(o.comments.unshift({author:"我",text:r}),n.commentSheetMomentId=null,n.toast="已发送评论",k(120),u(),window.setTimeout(()=>{n.toast="",u()},1200))}if(a==="like-moment"){e.preventDefault(),e.stopPropagation();const o=Ue(t.dataset.momentId);if(!o)return;const i="我",r=o.likes.includes(i);o.likes=o.likes.filter(p=>p!==i),r||o.likes.unshift(i);const s=t;s.innerHTML=o.likes.includes(i)?g("heartFilled"):g("heart");const c=t.closest(".moment-content-col");if(!c)return;let l=c.querySelector(`[data-moment-id-panel="${o.id}"]`);if(!l&&o.likes.length>0){l=document.createElement("div"),l.className="moment-interactions",l.setAttribute("data-moment-id-panel",o.id);const p=c.querySelector(".moment-inline-comment");p?c.insertBefore(l,p):c.appendChild(l)}if(l){const p=l.querySelector(".moment-likes-area");if(o.likes.length>0)if(p)p.querySelector(".likes-list").textContent=o.likes.join("、");else{const m=document.createElement("div");m.className="moment-likes-area",m.innerHTML=`<span class="heart-mini">${g("heartFilled")}</span> <span class="likes-list">${d(o.likes.join("、"))}</span>`,l.insertBefore(m,l.firstChild)}else p&&p.remove(),l.querySelector(".moment-comments-area")||l.remove()}}if(a==="toggle-moment-search"&&(n.momentSearchOpen=!0,u()),a==="toggle-moment-menu"&&(e.preventDefault(),e.stopPropagation(),t.blur?.(),n.activeMenuMomentId=n.activeMenuMomentId===t.dataset.momentId?null:t.dataset.momentId,Yn()),a==="delete-moment"){e.preventDefault(),e.stopPropagation();const o=P(Ue(t.dataset.momentId));if(!o?.id)return;try{await Pr(o.id,o.author_type,o.author_id),n.moments=n.moments.filter(i=>i.id!==o.id),n.activeMenuMomentId=null,n.toast="已删除朋友圈",k(120),ne(),window.setTimeout(()=>{n.toast="",ne()},1200)}catch(i){console.warn("[moments] delete failed",i),n.toast="删除失败",ne(),window.setTimeout(()=>{n.toast="",ne()},1400)}return}if(a==="edit-moment"){e.preventDefault(),e.stopPropagation();const o=P(Ue(t.dataset.momentId));if(!o?.id)return;n.activeMenuMomentId=null,n.momentComposerEditingId=o.id,n.momentComposerText=o.content||"",n.momentComposerImage=o.image||"",n.momentComposerImageName=o.image?"已有图片":"",n.momentsActorType=o.author_type==="agent"?"agent":"user",n.momentComposerOpen=!0,ne();return}if(a==="new-moment"){e.preventDefault(),e.stopPropagation(),n.momentComposerEditingId="",n.momentComposerText="",n.momentComposerImage="",n.momentComposerImageName="",n.momentComposerOpen=!0,ne();return}if(a==="set-moments-actor"){n.toast="发朋友圈默认以我发布",u(),window.setTimeout(()=>{n.toast="",u()},1100);return}if(a==="publish-moment"){const o=(document.getElementById("moment-content-input")?.value||n.momentComposerText||"").trim();if(!o){n.toast="朋友圈内容还没写",u(),window.setTimeout(()=>{n.toast="",u()},1100);return}const i=we();try{if(n.momentComposerEditingId)await Er(n.momentComposerEditingId,{author_type:i.author_type,author_id:i.author_id,visibility:"public",content:o,image:n.momentComposerImage||"",mood:"日常"}),await za({silent:!0}),n.toast="已更新朋友圈";else{const r=await Tr({author_type:i.author_type,author_id:i.author_id,visibility:"public",content:o,image:n.momentComposerImage||"",mood:"日常"});n.moments.unshift(r),n.toast="已发布朋友圈"}n.currentTab="moments",n.currentView="moments",n.momentComposerOpen=!1,n.momentComposerEditingId="",n.momentComposerText="",n.momentComposerImage="",n.momentComposerImageName="",k(120),u(),window.setTimeout(()=>{n.toast="",u()},1100)}catch(r){console.warn("[moments] publish failed",r),n.toast=n.momentComposerEditingId?"更新失败":"发布失败",u(),window.setTimeout(()=>{n.toast="",u()},1400)}return}if(a==="delete-moment"&&(n.moments=n.moments.filter(o=>o.id!==t.dataset.momentId),n.activeMenuMomentId=null,n.toast="已删除朋友圈",u(),window.setTimeout(()=>{n.toast="",u()},1200)),a==="edit-moment"&&(n.activeMenuMomentId=null,n.toast="编辑功能即将支持",u(),window.setTimeout(()=>{n.toast="",u()},1200)),a==="filter-moments"&&(n.toast="筛选功能稍后补上",u(),window.setTimeout(()=>{n.toast="",u()},1100)),a==="new-moment"&&(n.momentComposerOpen=!0,u()),a==="close-moment-composer"&&(n.momentComposerOpen=!1,u()),a==="publish-moment"){const o=(document.getElementById("moment-content-input")?.value||n.momentComposerText||"").trim();if(!o){n.toast="朋友圈内容还没写",u(),window.setTimeout(()=>{n.toast="",u()},1100);return}n.moments.unshift({id:`p${Date.now()}`,contactId:"me",time:new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:!1}),mood:"日常",content:o,likes:[],comments:[],image:n.momentComposerImage||"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=1000&q=80"}),n.currentTab="moments",n.currentView="moments",n.momentComposerOpen=!1,n.momentComposerText="",n.momentComposerImage="",n.momentComposerImageName="",n.toast="已发布朋友圈",k(120),u(),window.setTimeout(()=>{n.toast="",u()},1100)}if(a==="remove-moment-image"&&(n.momentComposerImage="",n.momentComposerImageName="",u()),a==="new-contact"&&(n.newContactDraft=ft(),n.newContactAvatar="",n.currentView="newContact",u()),a==="pick-new-contact-avatar"){document.getElementById("nc-avatar-file")?.click();return}if(a==="save-new-contact"){n.newContactDraft={...n.newContactDraft||{},name:document.getElementById("nc-name")?.value?.trim()||n.newContactDraft?.name||"",agentId:document.getElementById("nc-agent-id")?.value?.trim()||n.newContactDraft?.agentId||"",bio:document.getElementById("nc-bio")?.value?.trim()||n.newContactDraft?.bio||""};const o=String(n.newContactDraft.name||"").trim(),i=ge(n.newContactDraft.agentId),r=String(n.newContactDraft.bio||"").trim(),s=n.newContactDraft.avatar||n.newContactAvatar||"https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&q=80";if(!o){n.toast="请填写联系人昵称",u(),window.setTimeout(()=>{n.toast="",u()},1200);return}if(i&&!/^[a-z0-9_-]+$/.test(i)){n.toast="Agent ID 只能用小写字母、数字、下划线或短横线",u(),window.setTimeout(()=>{n.toast="",u()},1500);return}if(i&&n.contacts.some(m=>String(m.id||"").toLowerCase()===i)){n.toast="这个 Agent ID 已经存在",u(),window.setTimeout(()=>{n.toast="",u()},1400);return}const c=i||"c"+Date.now(),l=Jn({id:c,name:o,bio:r||"这是新来的联系人",status:"在线",handle:"@"+c,unread:0,pinned:!1,lastMessage:"",lastTime:"",avatar:s,settings:{model:"gpt-5.4",modelProviderId:A("chat")?.providerId||"openai",temperature:.7,topP:.9,contextCount:32,thinkBudget:24,streamOutput:!0,reasoning_visibility:!1,proactiveEnabled:!1,proactiveFrequency:30,memoryEnabled:!0},topics:[],messages:[]}),p=await To(l);de(),Ie(100),n.newContactDraft=ft(),n.newContactAvatar="",n.toast=p?"已添加联系人":"已本地添加，后端登记失败",n.currentView="list",u(),window.setTimeout(()=>{n.toast="",u()},p?1200:1800)}if(a==="open-contact-avatar"){document.getElementById("contact-avatar-file")?.click();return}if(a==="open-contact-name"){const o=b(n.currentContactId);if(!o)return;const i=window.prompt("请输入昵称",o.name||"")?.trim();if(!i)return;o.name=i,n.toast="昵称已更新",u(),k(120),window.setTimeout(()=>{n.toast="",u()},1200);return}if(a==="open-contact-bio"){const o=b(n.currentContactId);if(!o)return;const i=window.prompt("请输入简介",o.bio||"")?.trim();if(typeof i!="string"||!i)return;o.bio=i,n.toast="简介已更新",u(),k(120),window.setTimeout(()=>{n.toast="",u()},1200);return}if(a==="open-contact-impression"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="contactImpressionDetail",u(),pe(n.currentContactId);return}if(a==="open-contact-relationship"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="contactRelationshipDetail",u(),pe(n.currentContactId);return}if(a==="open-contact-likes"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="contactLikesDetail",u(),pe(n.currentContactId);return}if(a==="save-insight-field"){const o=t.dataset.field,i=document.querySelector(`.insight-editor-textarea[data-field="${o}"]`);i&&Dr(o,i.value);return}if(a==="open-contact-room-background"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="contactRoomBackgroundPicker",u();return}if(a==="open-contact-bubble-theme"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="contactBubbleThemePicker",u();return}if(a==="delete-contact"){const o=b(n.currentContactId);if(!o||!window.confirm(`确定删除“${o.name}”吗？

会删除联系人及其陪伴状态。
会清理相关主动消息。
聊天记录和记忆不会立即永久删除。`))return;try{await xo(o.id),Mo(o.id),n.toast="联系人已删除",u(),k(120),window.setTimeout(()=>{n.toast="",u()},1400)}catch(r){console.warn("[contact] delete failed",r),n.toast="删除失败",u(),window.setTimeout(()=>{n.toast="",u()},1400)}return}if(a==="pick-contact-room-background"){const o=String(t.dataset.value||"").trim();if(!o)return;fo("roomBackground",o,"聊天背景已更新"),n.currentView="contactSettings",n.currentSettingsTab="basic",u();return}if(a==="pick-contact-bubble-theme"){const o=Vt(t.dataset.value),i=b(n.currentContactId);if(!i||!o)return;i.chatTheme=o,i.bubbleTheme=zt(o),i.theme=na(o),n.toast="气泡主题已更新",n.currentView="contactSettings",n.currentSettingsTab="basic",u(),k(120),window.setTimeout(()=>{n.toast="",u()},1200);return}if(a==="open-companion-state"){n._prevContactSettingsTab=n.currentSettingsTab,n.currentView="companionStateDetail",pe(n.currentContactId),u();return}if(a==="expand-actions"){n.showAttach=!n.showAttach,u();return}if(a==="remove-chat-attachment"){const o=t.dataset.id;n.chatAttachments=(n.chatAttachments||[]).filter(i=>i.id!==o),u();return}if(a==="clear-quote"&&(n.quoteMomentId=null,n.quoteMessageId=null,u()),a==="toggle-global"){const o=t.dataset.key;n.globalSettings[o]=!n.globalSettings[o],Xn(t,n.globalSettings[o]),D();return}if(a==="toggle-contact"){const o=b(n.currentContactId),i=t.dataset.key,s=y()?.querySelector(".chat-app-body")?.scrollTop??0;o.settings[i]=!o.settings[i],u(),Kt(s)}if(a==="back-home"&&(n.currentView==="list"?typeof window.closePage=="function"&&window.closePage("page-chat"):(n.currentTab="chats",n.currentView="list",u())),a==="switch-settings-tab"&&(n.currentSettingsTab=t.dataset.tab,n.contactQuickActionEditorId="",n.quickActionSwipeOpenId="",n.quickActionDropHintId="",n.quickActionDropDirection="",n.quickActionReorderPulseId="",n.currentSettingsTab!=="model"&&(n.contactModelAdvancedOpen=!1,n.contactPersonaExpanded=!1),u(),n.currentSettingsTab==="memory"&&pe(),n.currentSettingsTab==="model"&&at(n.currentContactId)),a==="toggle-contact-advanced"){n.contactModelAdvancedOpen=!n.contactModelAdvancedOpen,u();return}if(a==="toggle-contact-persona"){n.contactPersonaExpanded=!n.contactPersonaExpanded,u();return}if(a==="toggle-codex-mode"){St(t.dataset.contactId);return}if(a==="toggle-cc-mode"){qt(t.dataset.contactId);return}if(a==="quick-action"){const o=t.dataset.id,i=y()?.querySelector(".chat-input"),r=Ut(Q()).find(c=>c.id===o),s={health:"帮我记一下健康相关的事情",schedule:"帮我看看接下来的日程",weather:"帮我查一下今天的天气",files:"帮我找一下刚才提到的文件",quote:"引用上一条消息继续聊",more:"打开更多快捷操作",get_current_time:"现在几点了？",get_weather:"帮我查一下今天天气",get_health_summary:"帮我总结一下今天的健康数据",web_search:"帮我搜索这个问题",fetch_url:"帮我解析这个网页",add_todo:"帮我记一个待办",list_todos:"帮我看看待办清单",complete_todo:"把这个待办标记完成",add_note:"帮我记一条便签",list_notes:"帮我看看最近便签"};i&&(i.value=r?.prompt||s[r?.mcpToolId||o]||s[o]||`${r?.label||""}`.trim())}if(a==="fake-send"){if(n.streamingAbortController){n.streamingAbortController.abort(),n.streamingAbortController=null,u();return}n.currentView==="rpRoom"?Yi():ht(b(n.currentContactId))?$n():gt(b(n.currentContactId))?Sn():kn()}if(a==="reroll-msg"&&Xi(t.dataset.id),a==="quote-msg"){const o=t.dataset.id;if(b(n.currentContactId)?.messages?.find(s=>s.id===o)?.text){n.quoteMomentId=null,n.quoteMessageId=o,u();const s=y()?.querySelector(".chat-input");s&&s.focus()}}if(a==="attach-option"){n.showAttach=!1;const o=t.dataset.label||"";if(o==="图片"||o==="拍照"){u(),requestAnimationFrame(()=>y()?.querySelector("#chat-image-input")?.click());return}n.toast=`${o} 功能稍后补上`,u(),window.setTimeout(()=>{n.toast="",u()},1200)}}function Ei(e){const t=e.target;if(t?.dataset?.action==="avatar-cropper-range"){const a=n.avatarCropper;if(!a)return;const o=t.dataset.key;a[o]=o==="zoom"?Qe(t.value):ie(t.value),ra();return}if((t?.id==="nc-name"||t?.id==="nc-agent-id"||t?.id==="nc-bio")&&(n.newContactDraft={...n.newContactDraft||{},...t.id==="nc-name"?{name:t.value||""}:{},...t.id==="nc-agent-id"?{agentId:t.value||""}:{},...t.id==="nc-bio"?{bio:t.value||""}:{}}),t.dataset.action==="slide-contact"){const a=b(n.currentContactId),o=t.dataset.key,i=Number(t.value);a.settings[o]=Number.isInteger(a.settings[o])?Math.round(i):i;const s=t.closest(".slider-row-block")?.querySelector(".slider-value");s&&(s.textContent=Number.isInteger(Number(t.step))||Number(t.step)>=1?String(Math.round(i)):i.toFixed(Number(t.step)===.01?2:1))}t.dataset.action==="moment-composer-input"&&(n.momentComposerText=t.value||"")}document.addEventListener("DOMContentLoaded",()=>{Ni(),Qn(),xa().finally(async()=>{await Qi(),Di(),await ba({silent:!0})})});const $=window.__YUI_API_BASE__||(/^(localhost|127\.0\.0\.1)$/.test(location.hostname)?"":"https://api.somni-ref.top"),kt="murmur_local_state_v1",ca="murmur_sync_meta_v1",ua="murmur_device_id_v1",la=new Set(lt.map(e=>e.id)),da=new Set(qn.map(e=>e.id));let cn=null,un=null,It=!1,Ye=!1,pa=null,ln=!1,_t="",dn=null,pn=null;function mn(){try{const e=localStorage.getItem(ua);if(e)return e;const t=`dev_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;return localStorage.setItem(ua,t),t}catch{return`dev_fallback_${Date.now()}`}}function Xe(){try{const e=localStorage.getItem(ca),t=e?JSON.parse(e):{};return{last_server_updated_at:t?.last_server_updated_at||"",pending:!!t?.pending}}catch{return{last_server_updated_at:"",pending:!1}}}function ze(e={}){try{localStorage.setItem(ca,JSON.stringify({last_server_updated_at:e.last_server_updated_at||"",pending:!!e.pending}))}catch{}}function Pi(){return ae(),n.currentRpRoomId&&Array.isArray(n.currentRpMessages)&&(n.rpMessages={...n.rpMessages||{},[n.currentRpRoomId]:n.currentRpMessages.map(G)}),hn({contacts:n.contacts,moments:n.moments,actions:n.actions,globalSettings:n.globalSettings,accountProfile:n.accountProfile,conversations:n.conversations,rpRooms:n.rpRooms,rpMessages:n.rpMessages})}function H(e){try{return JSON.stringify(e)}catch{return""}}function X(e){const t=String(e||"").trim();if(!t)return 0;const a=Date.parse(t);return Number.isFinite(a)?a:0}function ke(e={}){return String(e.content||e.text||e.message||e.body||e.raw_content||"").trim()}function Mt(e={}){return!!ke(e)||yi(e)||!!e.typing||!!e.streaming||!!e.thinking||Array.isArray(e.toolCalls)&&e.toolCalls.length>0}function fn(e={}){const t=X(e.created_at||e.timestamp);if(t)return Math.floor(t/6e4);const a=String(e.time||"").trim();return a||""}function ma(e={},t={}){const a=fn(e),o=fn(t);return!!a&&!!o&&a===o}function fa(e={},t={}){const a=G(e),o=G(t);if(a.role!==o.role||ke(a)!==ke(o)||(a.session_id||o.session_id)&&a.session_id!==o.session_id)return!1;const i=X(a.created_at||a.timestamp),r=X(o.created_at||o.timestamp);return i&&r?Math.abs(i-r)<=120*1e3:ma(a,o)}function xt(e={}){const t=G(e),a=new Set;t.id&&a.add(`id:${t.id}`),t.client_message_id&&a.add(`client:${t.client_message_id}`);const o=ke(t);if(o){const i=t.session_id||t.agent_id||"",r=fn(t);a.add(`soft:${i}|${t.role}|${r}|${o}`)}return a}function Ct(e=[],t={}){const a=F(t),o=xt(a);let i=e.findIndex(r=>{const s=xt(r);return[...o].some(c=>s.has(c))});return i===-1&&(i=e.findIndex(r=>fa(r,a))),i===-1?e.push(a):e[i]=F({...e[i],...a}),a}function G(e={}){const t=String(e.role||e.from||"").toLowerCase()==="user"||e.from==="me"?"user":"ai",a=ke(e),o=String(e.created_at||e.timestamp||""),i=String(e.time||""),r=[String(e.agent_id||""),t,o||i,a].join("|");return{id:String(e.id||r||`${t}_${Date.now()}_${Math.random().toString(36).slice(2,6)}`),session_id:String(e.session_id||""),agent_id:String(e.agent_id||""),client_message_id:String(e.client_message_id||e.clientMessageId||""),role:t,content:a,text:a,created_at:o,time:i,...e.model?{model:e.model}:{},...e.source?{source:e.source}:{},...e.provider?{provider:e.provider}:{},...e.attachments?{attachments:e.attachments}:{},...e.thinking?{thinking:e.thinking}:{},...e.toolCalls?{toolCalls:e.toolCalls}:{}}}function F(e={}){const t=G(e);return{...t,text:t.content,time:t.time||(t.created_at?ce(t.created_at,{fallback:""}):"")}}function He(e={}){return!e||typeof e!="object"?{}:Object.fromEntries(Object.entries(e).map(([t,a])=>[String(t),Array.isArray(a)?le([],a):[]]))}function le(e=[],t=[]){const a=[],o=new Map;return[...e,...t].forEach(i=>{const r=G(i);if(!Mt(r))return;let c=[...xt(r)].map(p=>o.get(p)).find(p=>p>=0);c>=0||(c=a.findIndex(p=>fa(p,r)));const l=c>=0?a[c]:null;if(!l||X(r.created_at)>=X(l.created_at)){const p={...l,...r},m=c>=0?c:a.length;a[m]=p,[...xt(p)].forEach(f=>o.set(f,m))}}),a.filter(Boolean).sort((i,r)=>{const s=X(i.created_at),c=X(r.created_at);return s||c?s-c:String(i.id).localeCompare(String(r.id))})}function ga(e={},t={}){const a=He(e),o=He(t);return Object.entries(o).forEach(([i,r])=>{a[i]=le(a[i]||[],r)}),a}function ae(){const e=He(n.conversations);(n.contacts||[]).forEach(t=>{if(!t?.id)return;const a=Array.isArray(t.messages)?t.messages:[];(a.length||e[t.id]?.length)&&(e[t.id]=le(e[t.id]||[],a),t.messages=e[t.id].map(F))}),n.conversations=e}function Ge(){const e=He(n.conversations);n.contacts=(n.contacts||[]).map(t=>{const o=(e[t.id]||(Array.isArray(t.messages)?t.messages.map(G):[])).map(F),i=o[o.length-1];return{...t,messages:o,lastMessage:i?.text||t.lastMessage||"",lastTime:i?.time||t.lastTime||""}}),n.conversations=e}function At(e=[],t=[]){const a=new Map;return e.map(Y).forEach(o=>a.set(o.id.toLowerCase(),o)),t.map(Y).forEach(o=>{const i=o.id.toLowerCase(),r=a.get(i);if(!r){a.set(i,o);return}const s=le(r.messages||[],o.messages||[]),c={...o,...r,id:r.id||o.id,agent_id:r.agent_id||o.agent_id||r.id||o.id,name:r.name||o.name,display_name:r.display_name||r.name||o.display_name||o.name,bio:r.bio||o.bio,status:r.status||o.status,handle:r.handle||o.handle,roleTag:r.roleTag||o.roleTag,avatar:r.avatar||o.avatar,settings:{...o.settings||{},...r.settings||{}},messages:s.map(F),lastMessage:r.lastMessage||o.lastMessage||s[s.length-1]?.content||"",lastTime:r.lastTime||o.lastTime||s[s.length-1]?.time||""};a.set(i,c)}),[...a.values()]}function qi(e=[]){for(let t=e.length-1;t>=0;t-=1){const a=String(e[t]?.session_id||"").trim();if(a)return a}return""}function Je(e={},t=""){const a=String(e.role||"").toLowerCase()==="user"?"user":"ai",o=String(e.created_at||""),i=ke(e),r=String(e.model||"");return G({id:e.id||`${t}|${a}|${o}|${i}`,session_id:e.session_id||"",agent_id:e.agent_id||t,role:a,content:i,text:i,created_at:o,time:o?ce(o,{fallback:""}):"",model:r,...r.toLowerCase()==="codex"?{source:"codex",provider:"codex"}:{}})}async function We(e,{silent:t=!0}={}){const a=b(e);if(!a?.id)return 0;try{const o=ta(a),i=[];console.info("[murmur] history request",{contact_id:a.id,tried:o});for(const m of o){const f=new URLSearchParams({agent_id:m,limit:"200"}),h=await fetch(`${$}/api/murmur/messages?${f.toString()}`);if(!h.ok){console.warn("[murmur] history fetch failed",{agent_id:m,status:h.status});continue}const v=await h.json().catch(()=>({})),S=Array.isArray(v?.messages)?v.messages:[];i.push(...S)}const r=le([],i.map(m=>Je(m,a.id)).filter(Mt));if(console.info("[murmur] history loaded",{agent_id:a.id,tried:o,raw:i.length,renderable:r.length,first:r[0]||null}),!r.length)return 0;const s=H({conversations:vt(a)}),c=le(vt(a),r);n.conversations={...n.conversations||{},[a.id]:c},a.messages=c.map(F);const l=a.messages[a.messages.length-1];l&&(a.lastMessage=l.text||"",a.lastTime=l.time||"");const p=qi(c);return p&&(a.sessionId=p),H({conversations:c})!==s&&(de(),Ie(300)),n.currentContactId===a.id&&n.currentView==="room"&&u(),r.length}catch(o){return console.error("[murmur] history load failed",o),0}}function Li(e={}){const t=String(e.agent_id||e.agentId||"").trim(),a=String(e.content||"").trim(),o=String(e.created_at||e.createdAt||new Date().toISOString());return G({id:e.id?`proactive_${e.id}`:`proactive_${t}_${o}_${a}`,agent_id:t,role:"ai",content:a,created_at:o,source:"proactive"})}async function ha(e){const t=String(e||"").trim();if(t)try{await fetch(`${$}/api/proactive/${encodeURIComponent(t)}/read`,{method:"POST"})}catch(a){console.warn("[proactive] mark read failed",a)}}async function ba({silent:e=!0}={}){if(!ln){ln=!0;try{const t=await fetch(`${$}/api/proactive?limit=20`);if(!t.ok)throw new Error(`HTTP ${t.status}`);const a=await t.json().catch(()=>({})),o=Array.isArray(a?.messages)?a.messages:[];if(!o.length)return;let i=!1;for(const r of o){const s=Li(r),c=s.agent_id||String(r.agent_id||"").trim();if(!c||!s.content){await ha(r.id);continue}let l=b(c);l||(l=Jn({id:c,agent_id:c,name:String(r.agent_name||r.display_name||c),handle:`@${c}`,messages:[]}));const p=(n.conversations?.[l.id]||l.messages||[]).length,m=le(n.conversations?.[l.id]||l.messages||[],[s]);n.conversations={...n.conversations||{},[l.id]:m},l.messages=m.map(F);const f=l.messages[l.messages.length-1];f&&(l.lastMessage=f.text||"",l.lastTime=f.time||""),m.length>p&&!(n.currentView==="room"&&n.currentContactId===l.id)&&(l.unread=Number(l.unread||0)+1),i=!0,await ha(r.id)}i&&(Ge(),de(),u(),n.currentView==="room"&&j())}catch(t){e||console.warn("[proactive] poll failed",t)}finally{ln=!1}}}function Di(){pa||(pa=window.setInterval(()=>{ba({silent:!0})},15e3))}function va(e=[],t=[]){const a=new Map;return e.map(P).forEach(o=>a.set(o.id,o)),t.map(P).forEach(o=>{const i=a.get(o.id);if(!i){a.set(o.id,o);return}const r=X(o.updated_at||o.created_at||o.time),s=X(i.updated_at||i.created_at||i.time);a.set(o.id,r>s?{...i,...o}:{...o,...i})}),[...a.values()].sort((o,i)=>X(i.updated_at||i.created_at||i.time)-X(o.updated_at||o.created_at||o.time))}function Oi(e=[],t=[],a="id"){const o=new Map;return[...e||[],...t||[]].forEach(i=>{if(!i||typeof i!="object")return;const r=String(i[a]||i.id||"").trim();r&&o.set(r,{...o.get(r)||{},...i})}),[...o.values()]}const Ri="https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&q=80";function ya(e){return!e||String(e)===Ri}function Vi(e={},t={}){const a={...e||{},...t||{}};return!ya(e?.avatar)&&ya(t?.avatar)&&(a.avatar=e.avatar),a}function de(){const e=Pi(),t=H(e);if(!t||t===_t)return!1;_t=t;try{return localStorage.setItem(kt,JSON.stringify({client_updated_at:new Date().toISOString(),payload:e})),!0}catch{return!1}}function wa(){return dn||(dn=new Map(lt.map(e=>{const t=Y(e);return[t.id,H(t)]}))),dn}function $a(){return pn||(pn=new Map(qn.map(e=>{const t=P(e);return[t.id,H(t)]}))),pn}function Sa(e){if(!Array.isArray(e))return!1;const t=wa();return e.some(a=>{const o=Y(a);return!la.has(o.id)||t.get(o.id)!==H(o)})}function gn(e){if(!e||typeof e!="object")return!1;const t=String(e.id||e.agent_id||"").trim().toLowerCase();if(!la.has(t))return!1;const a=wa(),o=Y({...e,id:t});if(a.get(t)===H(o))return!0;const i=String(e.avatar||"").trim(),r=Array.isArray(e.topics)?e.topics.map(c=>String(c?.id||"")):[],s=Array.isArray(e.messages)?e.messages.map(c=>String(c?.id||"")):[];return t==="ayan"?i.includes("photo-1517841905240-472988babdf9")||s.some(c=>["m1","m2","m3"].includes(c))||r.some(c=>["t1","t2","t3"].includes(c)):t==="azheng"?i.includes("photo-1500530855697-b586d89ba3ee")||s.includes("m4")||r.some(c=>["t4","t5"].includes(c)):t==="xiaoying"?i.includes("photo-1507525428034-b723cf961d3e")||s.includes("m5")||r.includes("t6"):!1}function ka(e){return Array.isArray(e)?e.filter(t=>!gn(t)):[]}function Bi(e){return Array.isArray(e)&&e.length>0&&e.every(t=>gn(t))}function hn(e={}){if(!e||typeof e!="object")return{};const t={...e};return Array.isArray(t.contacts)&&(t.contacts=ka(t.contacts).map(a=>Y(a))),t.conversations&&typeof t.conversations=="object"&&(t.conversations=He(t.conversations)),t.rpMessages&&typeof t.rpMessages=="object"&&(t.rpMessages=He(t.rpMessages)),Array.isArray(t.moments)&&(t.moments=Tt(t.moments).map(P)),t}function zi(e){if(!Array.isArray(e))return!1;const t=$a();return e.some(a=>{const o=P(a);return!da.has(o.id)||t.get(o.id)!==H(o)})}function Ia(e){if(!e||typeof e!="object")return!1;const t=String(e.id||"").trim();if(!da.has(t))return!1;const a=$a(),o=P(e);return a.get(t)===H(o)?!0:t==="p0"?String(e.image||"").includes("photo-1507525428034-b723cf961d3e")||String(e.content||"").includes("天空很温柔"):t==="p1"?String(e.content||"").includes("醉了先看这个"):t==="p2"?String(e.content||"").includes("晚上跑了三公里"):!1}function Tt(e){return Array.isArray(e)?e.filter(t=>!Ia(t)):[]}function Hi(e){return Array.isArray(e)&&e.length>0&&e.every(t=>Ia(t))}function _a(e,{source:t="local"}={}){if(!(!e||typeof e!="object")){if(Array.isArray(e.contacts)){const a=e.contacts.map(r=>Y(r)),o=ka(a).map(r=>Y(r)),i=Sa(n.contacts);o.length?(n.contacts=At(n.contacts,o),b(n.currentContactId)||(n.currentContactId=n.contacts[0]?.id||"")):Bi(a)?(i||(n.contacts=[]),b(n.currentContactId)||(n.currentContactId=n.contacts[0]?.id||""),console.warn(`[sync] ignored ${t} default mock contacts`)):i?n.contacts=n.contacts.map(r=>Y(r)):(n.contacts=[],n.currentContactId="")}else n.contacts=n.contacts.map(a=>Y(a));if(e.conversations&&typeof e.conversations=="object"?(n.conversations=ga(n.conversations,e.conversations),Ge()):ae(),Array.isArray(e.moments)){const a=e.moments.map(P),o=Tt(a).map(P),i=zi(n.moments);o.length?n.moments=va(Tt(n.moments),o):Hi(a)?(i||(n.moments=[]),console.warn(`[sync] ignored ${t} default mock moments`)):i?n.moments=Tt(n.moments).map(P):n.moments=[]}Array.isArray(e.rpRooms)&&(n.rpRooms=Oi(n.rpRooms||[],e.rpRooms||[],"room_id")),e.rpMessages&&typeof e.rpMessages=="object"&&(n.rpMessages=ga(n.rpMessages,e.rpMessages)),Array.isArray(e.actions)&&(n.actions=e.actions),e.globalSettings&&typeof e.globalSettings=="object"&&(n.globalSettings={...n.globalSettings,...e.globalSettings}),e.accountProfile&&typeof e.accountProfile=="object"&&(n.accountProfile=Vi(n.accountProfile,e.accountProfile)),z(),Lt()}}function Ni(){try{const e=localStorage.getItem(kt);if(!e)return;const t=JSON.parse(e);if(!t?.payload)return;_a(t.payload,{source:"local"});const a=hn(t.payload);_t=H(a),H(t.payload)!==_t&&localStorage.setItem(kt,JSON.stringify({client_updated_at:t.client_updated_at||new Date().toISOString(),payload:a}))}catch{}}function Ie(e=600){if(Ye)return;const t=Xe();ze({...t,pending:!0}),cn&&clearTimeout(cn),cn=window.setTimeout(()=>{Ma()},e)}async function Ma(){if(It||Ye)return;const e=Xe();if(!e.pending)return;let t=null;try{t=JSON.parse(localStorage.getItem(kt)||"null")}catch{}if(!t?.payload){ze({...e,pending:!1});return}const a=hn(t.payload);It=!0;try{const o=await fetch(`${$}/api/sync/push`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({device_id:mn(),client_updated_at:t.client_updated_at||new Date().toISOString(),payload:a})});if(!o.ok)throw new Error(`HTTP ${o.status}`);const i=await o.json().catch(()=>({}));ze({last_server_updated_at:i.server_updated_at||e.last_server_updated_at||"",pending:!1})}catch(o){console.warn("[sync] push failed",o),ze({...e,pending:!0})}finally{It=!1}}async function xa(){if(It)return;const e=Xe();if(e.pending&&(await Ma(),Xe().pending))return;const t=new URLSearchParams({device_id:mn()});e.last_server_updated_at&&t.set("since",e.last_server_updated_at);try{const a=await fetch(`${$}/api/sync/pull?${t.toString()}`);if(!a.ok)return;const o=await a.json().catch(()=>({})),i=Sa(n.contacts);if(!o?.has_update||!o?.payload||o?.is_self&&i){o?.server_updated_at&&ze({...e,last_server_updated_at:o.server_updated_at,pending:e.pending});return}Ye=!0,_a(o.payload,{source:"remote"}),de(),ze({last_server_updated_at:o.server_updated_at||e.last_server_updated_at||"",pending:!1}),u()}catch(a){console.warn("[sync] pull failed",a)}finally{Ye=!1}}function ji(e={}){const t=ge(e.agent_id||e.id);if(!t)return null;const a=String(e.display_name||e.name||t).trim()||t;return Y({id:t,agent_id:t,name:a,display_name:a,bio:String(e.description||e.subtitle||"").trim(),status:"在线",handle:String(e.display_handle||`@${t}`),roleTag:String(e.source||"agent"),avatar:String(e.avatar||"").trim(),pinned:!1,unread:0,lastMessage:"",lastTime:"",topics:[],messages:[]})}function Ca(e={}){const t=ge(e.agent_id||e.id);if(!t)return null;const a=lt.find(r=>String(r.id||"").toLowerCase()===t),o=String(e.last_message_at||""),i=String(e.last_message||"").trim();return Y({id:t,agent_id:t,name:String(a?.name||e.display_name||e.name||t).trim()||t,display_name:String(a?.name||e.display_name||e.name||t).trim()||t,bio:"",status:"在线",handle:`@${t}`,roleTag:"recovered",avatar:"",pinned:!1,unread:0,lastMessage:i,lastTime:o?ce(o,{fallback:""}):"",sessionId:String(e.session_id||""),messageCount:Number(e.message_count||0)||0,topics:[],messages:[]})}async function Fi({silent:e=!0}={}){try{const t=await fetch(`${$}/api/agents?include_inactive=true`);if(!t.ok)throw new Error(`HTTP ${t.status}`);const a=await t.json().catch(()=>({})),o=(Array.isArray(a?.agents)?a.agents:[]).filter(r=>r?.is_active!==!1).map(ji).filter(Boolean).filter(r=>!gn(r));if(console.info("[agents] loaded",o.map(r=>({id:r.id,name:r.name,source:r.roleTag||""}))),!o.length)return;const i=H({contacts:n.contacts});n.contacts=At(n.contacts,o),Ge(),(!n.currentContactId||!n.contacts.some(r=>r.id===n.currentContactId))&&(n.currentContactId=n.contacts[0]?.id||""),H({contacts:n.contacts})!==i&&(de(),Ie(100)),Et(o),u()}catch(t){e||console.warn("[agents] load contacts failed",t)}}async function Ui({silent:e=!0}={}){try{const t=await fetch(`${$}/api/murmur/message-agents?limit=1000`);if(t.status===404){await Ki({silent:e});return}if(!t.ok)throw new Error(`HTTP ${t.status}`);const a=await t.json().catch(()=>({})),o=(Array.isArray(a?.agents)?a.agents:[]).map(Ca).filter(Boolean);if(console.info("[murmur] message agents loaded",o.map(r=>({id:r.id,lastMessage:r.lastMessage,count:r.messageCount||0}))),!o.length)return;const i=H({contacts:n.contacts});n.contacts=At(n.contacts,o),Ge(),(!n.currentContactId||!n.contacts.some(r=>r.id===n.currentContactId))&&(n.currentContactId=n.contacts[0]?.id||""),H({contacts:n.contacts})!==i&&(de(),Ie(100)),Et(o),u()}catch(t){e||console.warn("[murmur] load message agents failed",t)}}async function Ki({silent:e=!0}={}){const t=Array.from(new Set([...lt.map(r=>ge(r.id)).filter(Boolean),...n.contacts.map(r=>ge(r.id)).filter(Boolean)])),a=[];for(const r of t)if(r)try{const s=new URLSearchParams({agent_id:r,limit:"1"}),c=await fetch(`${$}/api/murmur/messages?${s.toString()}`);if(!c.ok)continue;const l=await c.json().catch(()=>({})),p=Array.isArray(l?.messages)?l.messages:[];if(!p.length)continue;const m=p[p.length-1]||{};a.push(Ca({agent_id:r,last_message:m.content||"",last_message_at:m.created_at||"",message_count:p.length,session_id:m.session_id||""}))}catch(s){e||console.warn("[murmur] message probe failed",r,s)}const o=a.filter(Boolean);if(console.info("[murmur] message agents probed",o.map(r=>({id:r.id,lastMessage:r.lastMessage,count:r.messageCount||0}))),!o.length)return;const i=H({contacts:n.contacts});n.contacts=At(n.contacts,o),Ge(),(!n.currentContactId||!n.contacts.some(r=>r.id===n.currentContactId))&&(n.currentContactId=n.contacts[0]?.id||""),H({contacts:n.contacts})!==i&&(de(),Ie(100)),Et(o),u()}async function Qi(){await Fi(),await Ui(),await Et(n.contacts),Do(120)}async function Et(e=[]){const t=[...new Set((e||[]).map(a=>a?.id).filter(Boolean))];for(const a of t)if(!(n.historyLoadingContactIds[a]||n.historyLoadedContactIds[a])){n.historyLoadingContactIds[a]=!0;try{await We(a)&&(n.historyLoadedContactIds[a]=!0)}finally{delete n.historyLoadingContactIds[a]}}}function k(e=800){Ye||(un&&clearTimeout(un),un=window.setTimeout(()=>{de()&&Ie(500)},e))}function q(){const e=new Date;return`${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`}function bn(e){const t=e?.settings?.modelProviderId||A("chat")?.providerId||"",a=W(t);if(!a?.baseUrl||!a?.apiKey)return{};const o=Ze(a.apiPath||a.api_path||"",{allowEmpty:!0});return{base_url:a.baseUrl,api_key:a.apiKey,...o?{api_path:o}:{}}}function vn(e){const t=e?.settings||{},a=Number(t.temperature);return Number.isFinite(a)?{temperature:a}:{}}function yn(e,t=""){let a="",o="";try{const s=JSON.parse(e),c=/^(thinking|reasoning|reason|thought|cot|inner_thought)$/i.test(t),l=/^(chat|message|content|text|assistant|reply|response|output)$/i.test(t);c?o=s.thinking??s.reasoning??s.reasoning_content??s.reasoningContent??s.content??s.text??s.delta??"":(a=s.content??s.text??s.delta??"",o=s.thinking??s.reasoning??s.reasoning_content??s.reasoningContent??"")}catch{/^(thinking|reasoning|reason|thought|cot|inner_thought)$/i.test(t)?o=e:a=e}const i=/^tool_call$/i.test(t);let r=null;if(i)try{const s=JSON.parse(e);s.name&&(r={name:String(s.name),status:String(s.status||"done")})}catch{}return{text:ye(a),thinking:ye(o),toolCall:r}}async function Pt(e){const t=String(e?.sessionId||"").trim();if(t){try{const r=await fetch(`${$}/api/sessions/${encodeURIComponent(t)}`);if(r.ok)return t;if(r.status!==404)throw new Error(`校验会话失败（HTTP ${r.status}）`)}catch(r){throw String(r?.message||"").includes("HTTP"),r}e.sessionId=""}const a=await fetch(`${$}/api/sessions`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:String(e?.name||"新对话").trim()||"新对话",model:String(e?.settings?.model||n.globalSettings?.defaultModel||"echo").trim()||"echo",source_app:"yui_nook"})}),o=await a.json().catch(()=>({}));if(!a.ok)throw new Error(o.detail||`创建会话失败（HTTP ${a.status}）`);const i=String(o?.session?.id||"").trim();if(!i)throw new Error("创建会话失败：后端没有返回 session.id");return e.sessionId=i,k(120),i}async function wn(e,t,a,o="/api/chat"){let i=await fetch(`${$}${o}`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify(t),...a?{signal:a}:{}});if(i.ok)return i;let r="";try{const s=await i.json();r=String(s?.detail||"").trim()}catch{}if(o==="/api/chat"&&i.status===404&&r.includes("会话不存在")){e.sessionId="";const s=await Pt(e);if(t.session_id=s,i=await fetch(`${$}${o}`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify(t),...a?{signal:a}:{}}),i.ok)return i}throw new Error(`HTTP ${i.status}`)}async function Yi(){const e=y()?.querySelector(".chat-input"),t=e?.value?.trim();if(!t||!n.currentRpRoomId)return;const a=b(n.currentContactId)||n.contacts[0],o=Gt();if(!a||!o)return;const i=!!a?.settings?.reasoning_visibility,r=`rp_u_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;Ct(n.currentRpMessages,{id:r,client_message_id:r,role:"user",text:t,content:t,time:q(),timestamp:new Date().toISOString(),created_at:new Date().toISOString()}),e.value="";const s="rp_ai_"+Date.now();n.currentRpMessages.push({id:s,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0}),n.currentRpRoomId&&(n.rpMessages[n.currentRpRoomId]=n.currentRpMessages.map(G)),k(120),u(),j();const c={room_id:n.currentRpRoomId,agent_id:o.agent_id||a.id,content:t,client_message_id:r,...a.persona?{persona:a.persona}:{},...a.settings.model?{model:a.settings.model}:{},...vn(a),...bn(a)},l=new AbortController;n.streamingAbortController=l,u();let p="";try{const m=await wn(a,c,l.signal,"/api/rp/chat"),f=()=>n.currentRpMessages.findIndex(_=>_.id===s);n.currentRpMessages[f()]={id:s,role:"ai",text:"",time:q(),typing:!1,streaming:!0},u();const h=m.body.getReader(),v=new TextDecoder;let S="",I="",T="";for(;;){const{done:_,value:C}=await h.read();if(_)break;S+=v.decode(C,{stream:!0});const V=S.split(`
`);S=V.pop()??"";for(const oe of V){const E=oe.trim();if(!E){T="";continue}if(E.startsWith("event:")){T=E.slice(6).trim();continue}if(!E.startsWith("data:"))continue;const O=E.slice(5).trim();if(O==="[DONE]")continue;const B=yn(O,T);let K=B.text;const he=Nt(B.thinking,p,I),me=i?he:"";me&&I.length<Ht&&(I=jt(I,me)),K&&(p+=K);const ee=f();ee!==-1&&(n.currentRpMessages[ee]={id:s,role:"ai",text:p,content:p,...i&&I?{thinking:I}:{},time:q(),typing:!1,streaming:!0},u(),j())}}const x=n.currentRpMessages.findIndex(_=>_.id===s);x!==-1&&p.trim()?n.currentRpMessages[x]={...n.currentRpMessages[x],text:p,content:p,...i&&I?{thinking:I}:{},streaming:!1,typing:!1,time:q(),created_at:new Date().toISOString()}:x!==-1&&n.currentRpMessages.splice(x,1),n.streamingAbortController=null,await bt(o.agent_id||a.id,{silent:!0}),n.currentRpRoomId&&(n.rpMessages[n.currentRpRoomId]=n.currentRpMessages.map(G)),k(120),u(),j()}catch(m){const f=m.name==="AbortError",h=n.currentRpMessages.findIndex(v=>v.id===s);h!==-1&&(f&&!p.trim()?n.currentRpMessages.splice(h,1):n.currentRpMessages[h]={id:s,role:"ai",text:m.name==="AbortError"?p:`连接失败：${m.message}`,content:m.name==="AbortError"?p:`连接失败：${m.message}`,time:q(),created_at:new Date().toISOString(),typing:!1}),n.streamingAbortController=null,n.currentRpRoomId&&(n.rpMessages[n.currentRpRoomId]=n.currentRpMessages.map(G)),k(120),u()}}function qt(e=n.currentContactId){const t=b(e)||b(n.currentContactId);if(t){if(n.currentContactId=t.id,!Wt(t)){t.settings={...t.settings||{},ccEnabled:!1},n.toast="只有阿筝能切 Claude Code",u(),window.setTimeout(()=>{n.toast="",u()},1200);return}t.settings={...t.settings||{},ccEnabled:!t.settings?.ccEnabled},n.toast=t.settings.ccEnabled?"Claude Code 已接管这个窗口":"Claude Code 已关闭",k(120),u(),window.setTimeout(()=>{n.toast="",u()},1200)}}async function $n(){const e=y()?.querySelector(".chat-input"),t=e?.value?.trim()||"",a=(n.chatAttachments||[]).map(Be).filter(Boolean);if(!t&&!a.length)return;const o=nn(t,a),i=b(n.currentContactId);if(!i)return;De();const r=`u_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;Ct(i.messages,{id:r,client_message_id:r,role:"user",text:t,content:t,attachments:a,time:q(),created_at:new Date().toISOString()}),i.lastMessage=an(t,a),i.lastTime="刚刚",e.value="",n.chatAttachments=[];const s="ai_"+Date.now();i.messages.push({id:s,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0,source:"claude-code"}),ae(),k(120),u(),j();const c=new AbortController;n.streamingAbortController=c,u();try{const l=await fetch(`${$}/api/claude-code/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({conversation_key:`yui:${i.id}`,agent_id:i.id,content:o,client_message_id:r,reset:!1}),signal:c.signal}),p=await l.json().catch(()=>({}));if(!l.ok)throw new Error(p.detail||`HTTP ${l.status}`);const m=String(p.reply||"").trim(),f=p.user_message&&typeof p.user_message=="object"?Je(p.user_message,i.id):null,h=p.assistant_message&&typeof p.assistant_message=="object"?{...Je(p.assistant_message,i.id),source:"claude-code",provider:"claude-code"}:null,v=i.messages.findIndex(I=>I.id===r);v!==-1&&f&&(i.messages[v]=F({...f,content:t,text:t,attachments:a,client_message_id:r}));const S=i.messages.findIndex(I=>I.id===s);S!==-1&&m?i.messages[S]={...h?F(h):{},id:h?.id||s,role:"ai",text:m,content:m,source:"claude-code",provider:"claude-code",time:h?.time||q(),created_at:h?.created_at||new Date().toISOString(),typing:!1}:S!==-1&&i.messages.splice(S,1),i.lastMessage=m||o,i.lastTime=q(),ae(),k(120),u(),j()}catch(l){const p=l.name==="AbortError";p||console.error("[cc chat] error:",l);const m=i.messages.findIndex(f=>f.id===s);if(m!==-1){const f=p?"":`Claude Code 连接失败：${l.message}`;f?i.messages[m]={id:s,role:"ai",text:f,content:f,source:"claude-code",provider:"claude-code",time:q(),created_at:new Date().toISOString(),typing:!1}:i.messages.splice(m,1)}ae(),k(120),u()}finally{n.streamingAbortController=null,u()}}async function Sn(){const e=y()?.querySelector(".chat-input"),t=e?.value?.trim()||"",a=(n.chatAttachments||[]).map(Be).filter(Boolean);if(!t&&!a.length)return;const o=nn(t,a),i=b(n.currentContactId);if(!i)return;De();const r=`u_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;Ct(i.messages,{id:r,client_message_id:r,role:"user",text:t,content:t,attachments:a,time:q(),created_at:new Date().toISOString()}),i.lastMessage=an(t,a),i.lastTime="刚刚",e.value="",n.chatAttachments=[];const s="ai_"+Date.now();i.messages.push({id:s,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0,source:"codex"}),ae(),k(120),u(),j();const c=new AbortController;n.streamingAbortController=c,u();try{const l=await fetch(`${$}/api/codex/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({conversation_key:`yui:${i.id}`,agent_id:i.id,content:o,client_message_id:r,reset:!1}),signal:c.signal}),p=await l.json().catch(()=>({}));if(!l.ok)throw new Error(p.detail||`HTTP ${l.status}`);const m=String(p.reply||"").trim(),f=p.user_message&&typeof p.user_message=="object"?Je(p.user_message,i.id):null,h=p.assistant_message&&typeof p.assistant_message=="object"?{...Je(p.assistant_message,i.id),source:"codex",provider:"codex"}:null,v=i.messages.findIndex(I=>I.id===r);v!==-1&&f&&(i.messages[v]=F({...f,content:t,text:t,attachments:a,client_message_id:r}));const S=i.messages.findIndex(I=>I.id===s);S!==-1&&m?i.messages[S]={...h?F(h):{},id:h?.id||s,role:"ai",text:m,content:m,source:"codex",provider:"codex",time:h?.time||q(),created_at:h?.created_at||new Date().toISOString(),typing:!1}:S!==-1&&i.messages.splice(S,1),i.lastMessage=m||o,i.lastTime=q(),ae(),k(120),u(),j()}catch(l){const p=l.name==="AbortError";p||console.error("[codex chat] error:",l);const m=i.messages.findIndex(f=>f.id===s);if(m!==-1){const f=p?"":`Codex 连接失败：${l.message}`;f?i.messages[m]={id:s,role:"ai",text:f,content:f,source:"codex",provider:"codex",time:q(),created_at:new Date().toISOString(),typing:!1}:i.messages.splice(m,1)}ae(),k(120),u()}finally{n.streamingAbortController=null,u()}}const Ne={},Aa=1500;async function Ta(e){const t=Ne[e];if(!t||!t.texts.length)return;const a=t.texts.splice(0);t.timer&&(clearTimeout(t.timer),t.timer=null),t.listener&&(y()?.querySelector(".chat-input")?.removeEventListener("input",t.listener),t.listener=null),delete Ne[e];const o=b(e);if(!o)return;const i=!!o?.settings?.reasoning_visibility;let r="";try{r=await Pt(o)}catch{return}const s=a.join(`
`),c=`u_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,l="ai_"+Date.now();o.messages.push({id:l,role:"ai",text:"",content:"",time:"",created_at:new Date().toISOString(),typing:!0}),ae(),k(120),u(),j();let p=0,m=!1,f=null;const h=()=>{const E=y()?.querySelector(`#thinking-${l}`);if(!E)return;E.textContent=Ke(C),E.classList.add("open","thinking-active"),E.setAttribute("aria-hidden","false");const O=y()?.querySelector(`#cot-wrapper-${l}`);O&&O.removeAttribute("data-slow"),n.openThinkingIds[l]=!0},v=()=>{f===null&&(f=requestAnimationFrame(()=>{f=null,h()}))},S=()=>{f!==null&&(cancelAnimationFrame(f),f=null)},I=setInterval(()=>{if(!p)return;const E=y()?.querySelector(`#cot-wrapper-${l}`);E&&E.toggleAttribute("data-slow",Date.now()-p>8e3)},2e3),T={session_id:r,agent_id:o.id,content:s,client_message_id:c,...o.persona?{persona:o.persona}:{},...o.settings.model?{model:o.settings.model}:{},...vn(o),...bn(o)},x=new AbortController;n.streamingAbortController=x,u();let _="",C="",V=null,oe=!1;try{const E=await wn(o,T,x.signal),O=()=>o.messages.findIndex(rt=>rt.id===l);o.messages[O()]={id:l,role:"ai",text:"",content:"",time:q(),created_at:new Date().toISOString(),typing:!1,streaming:!0},u();const B=E.body.getReader(),K=new TextDecoder;let he="",me="";for(;;){const{done:rt,value:Ee}=await B.read();if(rt)break;he+=K.decode(Ee,{stream:!0});const st=he.split(`
`);he=st.pop()??"";let be=0;for(const Rt of st){const Pe=Rt.trim();if(!Pe){me="";continue}if(Pe.startsWith("event:")){me=Pe.slice(6).trim();continue}if(!Pe.startsWith("data:"))continue;const ct=Pe.slice(5).trim();if(ct==="[DONE]")continue;const te=yn(ct,me);let qe=te.text;const ut=Nt(te.thinking,_,C),Le=i?ut:"";if(Le){C.length<Ht&&(C=jt(C,Le)),p=Date.now();const re=O();re!==-1&&(o.messages[re]={id:l,role:"ai",text:_,thinking:C,time:q(),typing:!1,streaming:!0},m?v():(m=!0,n.openThinkingIds[l]=!0,u(),j()))}if(te.toolCall){const re=te.toolCall;V||(V=[]);const Ja=V.find(Wa=>Wa.name===re.name&&Wa.status!=="done");Ja?Ja.status=re.status:V.push({name:re.name,status:re.status});const Pn=O();Pn!==-1&&(o.messages[Pn]={...o.messages[Pn],toolCalls:V.slice(),streaming:!0},u())}if(qe){_+=qe;const re=O();re!==-1&&(o.messages[re]={...o.messages[re],text:_,content:_,time:q(),typing:!1,streaming:!0},oe?to(l,_,C):(oe=!0,u(),j()))}be+=1,be>=32&&(be=0,v(),await Vn())}}clearInterval(I),S(),n.streamingAbortController=null;const ee=O(),N=_.trim();o.lastMessage=N||"已处理",o.lastTime=q();const Te=y()?.querySelector(`#thinking-${l}`);Te&&Te.classList.remove("thinking-active");const it=y()?.querySelector(`#cot-wrapper-${l}`);it&&it.removeAttribute("data-slow"),i&&C&&delete n.openThinkingIds[l];const Fe=zn(N);ee!==-1&&Fe.length>1?(o.messages.splice(ee,1),u(),j(),await Bn(180),await Hn(o,Fe,{startIndex:ee,thinking:i?C:"",toolCalls:V})):(ee!==-1&&N?o.messages[ee]={id:l,role:"ai",text:N,content:N,...i&&C?{thinking:C}:{},...V?{toolCalls:V}:{},time:q(),created_at:new Date().toISOString(),typing:!1}:ee!==-1&&o.messages.splice(ee,1),ae(),k(120),u(),j())}catch(E){clearInterval(I),S(),n.streamingAbortController=null;const O=E.name==="AbortError";O||console.error("[chat SSE] error:",E);const B=o.messages.findIndex(K=>K.id===l);if(B!==-1){const K=O?_.trim():`连接失败：${E.message}，请稍后再试。`;K?o.messages[B]={id:l,role:"ai",text:K,content:K,...i&&C?{thinking:C}:{},time:q(),created_at:new Date().toISOString(),typing:!1}:o.messages.splice(B,1)}O&&_&&(o.lastMessage=_,o.lastTime=q()),ae(),k(120),u()}}async function kn(){const e=y()?.querySelector(".chat-input"),t=e?.value?.trim()||"",a=(n.chatAttachments||[]).map(Be).filter(Boolean);if(!t&&!a.length)return;const o=nn(t,a),i=b(n.currentContactId);if(!i)return;De();let r="";try{r=await Pt(i)}catch{n.toast="无法创建会话，请稍后再试。",u(),window.setTimeout(()=>{n.toast="",u()},1500);return}const s=`u_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;Ct(i.messages,{id:s,client_message_id:s,session_id:r,agent_id:i.id,role:"user",text:t,content:t,attachments:a,time:q(),created_at:new Date().toISOString()}),i.lastMessage=an(t,a),i.lastTime="刚刚",e.value="",n.chatAttachments=[],ae(),k(120),u(),j(),Ne[i.id]||(Ne[i.id]={texts:[],timer:null,listener:null});const c=Ne[i.id];c.texts.push(o),c.timer&&clearTimeout(c.timer),c.listener||(c.listener=()=>{const l=Ne[i.id];l?.texts.length&&(clearTimeout(l.timer),l.timer=setTimeout(()=>Ta(i.id),Aa))},e.addEventListener("input",c.listener)),c.timer=setTimeout(()=>Ta(i.id),Aa)}async function Xi(e){const t=b(n.currentContactId);if(!t)return;De();const a=!!t?.settings?.reasoning_visibility,o=t.messages.findIndex(l=>l.id===e);if(o===-1||t.messages[o].role!=="ai")return;let i="";try{i=await Pt(t)}catch(l){console.error("[session] create failed:",l),n.toast=`无法创建会话：${l.message}`,u(),window.setTimeout(()=>{n.toast="",u()},1500);return}t.messages[o]={...t.messages[o],typing:!0,text:"",streaming:!1},u();const r=[...t.messages].reverse().find(l=>l.role==="user");if(!r)return;const s={session_id:i,agent_id:t.id,content:r.text,...t.persona?{persona:t.persona}:{},...t.settings.model?{model:t.settings.model}:{},...vn(t),...bn(t)},c=new AbortController;n.streamingAbortController=c;try{const l=await wn(t,s,c.signal);t.messages[o]={...t.messages[o],typing:!1,text:"",streaming:!0},u();const p=l.body.getReader(),m=new TextDecoder;let f="",h="",v="",S=null;const I=e;let T="",x=0,_=!1,C=null;const V=()=>{const N=y()?.querySelector(`#thinking-${I}`);if(!N)return;N.textContent=Ke(v),N.classList.add("open","thinking-active"),N.setAttribute("aria-hidden","false");const Te=y()?.querySelector(`#cot-wrapper-${I}`);Te&&Te.removeAttribute("data-slow"),n.openThinkingIds[I]=!0},oe=()=>{C===null&&(C=requestAnimationFrame(()=>{C=null,V()}))},E=()=>{C!==null&&(cancelAnimationFrame(C),C=null)},O=setInterval(()=>{if(!x)return;const N=y()?.querySelector(`#cot-wrapper-${I}`);N&&N.toggleAttribute("data-slow",Date.now()-x>8e3)},2e3);for(;;){const{done:N,value:Te}=await p.read();if(N)break;f+=m.decode(Te,{stream:!0});const it=f.split(`
`);f=it.pop()??"";let Fe=0;for(const rt of it){const Ee=rt.trim();if(!Ee){T="";continue}if(Ee.startsWith("event:")){T=Ee.slice(6).trim();continue}if(!Ee.startsWith("data:"))continue;const st=Ee.slice(5).trim();if(st==="[DONE]")continue;const be=yn(st,T);let Rt=be.text;const Pe=Nt(be.thinking,h,v),ct=a?Pe:"";if(ct){v.length<Ht&&(v=jt(v,ct)),x=Date.now();const te=t.messages.findIndex(qe=>qe.id===I);te!==-1&&(t.messages[te]={...t.messages[te],thinking:v,streaming:!0},_?oe():(_=!0,n.openThinkingIds[I]=!0,u()))}if(be.toolCall){const te=be.toolCall;S||(S=[]);const qe=S.find(Le=>Le.name===te.name&&Le.status!=="done");qe?qe.status=te.status:S.push({name:te.name,status:te.status});const ut=t.messages.findIndex(Le=>Le.id===I);ut!==-1&&(t.messages[ut]={...t.messages[ut],toolCalls:S.slice(),streaming:!0},u())}Rt&&(h+=Rt),Fe+=1,Fe>=32&&(Fe=0,oe(),await Vn())}}clearInterval(O),E(),n.streamingAbortController=null;const B=t.messages.findIndex(N=>N.id===I),K=h.trim(),he=y()?.querySelector(`#thinking-${I}`);he&&he.classList.remove("thinking-active");const me=y()?.querySelector(`#cot-wrapper-${I}`);me&&me.removeAttribute("data-slow"),a&&v&&delete n.openThinkingIds[I];const ee=zn(K);B!==-1&&ee.length>1?(t.messages.splice(B,1),u(),await Bn(180),await Hn(t,ee,{startIndex:B,thinking:a?v:"",toolCalls:S})):(B!==-1&&K?t.messages[B]={...t.messages[B],text:K,...a&&v?{thinking:v}:{},...S?{toolCalls:S}:{},streaming:!1}:B!==-1&&t.messages.splice(B,1),u())}catch(l){clearInterval(_rerollSlowTimer),_cancelRerollFlush(),n.streamingAbortController=null;const p=l.name==="AbortError";p||console.error("[reroll SSE] error:",l);const m=t.messages.findIndex(f=>f.id===rerollId);if(m!==-1){const f=p?fullText.trim():`重试失败：${l.message}`;f?t.messages[m]={...t.messages[m],text:f,...fullThinking?{thinking:fullThinking}:{},...fullToolCalls?{toolCalls:fullToolCalls}:{},streaming:!1}:t.messages.splice(m,1)}u()}}function j(){requestAnimationFrame(()=>{const e=y()?.querySelector(".messages-panel");e&&(e.scrollTop=e.scrollHeight)})}function Gi(){const e=b(n.currentContactId)||n.contacts[0]||{},t=n.currentView==="room"?Ut(e).map(Ro).join(""):"";return`
    ...
    `}const Ea=window.openPage;typeof Ea=="function"&&(window.openPage=function(t,a){Ea(t,a),t==="page-chat"&&Qn()});const Ji=[{id:"openai",name:"OpenAI",enabled:!0,baseUrl:"https://api.openai.com/v1",apiPath:"",apiKey:"",models:["gpt-5.4","gpt-5.4-mini","gpt-4.1-mini"],defaultModel:"gpt-5.4"},{id:"openrouter",name:"OpenRouter",enabled:!0,baseUrl:"https://openrouter.ai/api/v1",apiPath:"",apiKey:"",models:["openai/gpt-5","anthropic/claude-sonnet-4.5","anthropic/claude-opus-4.1","anthropic/claude-3.7-sonnet"],defaultModel:"openai/gpt-5"},{id:"gemini",name:"Gemini",enabled:!0,baseUrl:"https://generativelanguage.googleapis.com/v1beta/openai",apiPath:"",apiKey:"",models:["gemini-2.5-pro","gemini-2.5-flash"],defaultModel:"gemini-2.5-pro"},{id:"deepseek",name:"DeepSeek",enabled:!1,baseUrl:"https://api.deepseek.com/v1",apiPath:"",apiKey:"",models:["deepseek-chat","deepseek-reasoner"],defaultModel:"deepseek-chat"},{id:"qwen",name:"阿里云千问",enabled:!1,baseUrl:"https://dashscope.aliyuncs.com/compatible-mode/v1",apiPath:"",apiKey:"",models:["qwen-max","qwen-plus","qwen-turbo"],defaultModel:"qwen-max"},{id:"zhipu",name:"智谱",enabled:!1,baseUrl:"https://open.bigmodel.cn/api/paas/v4",apiPath:"",apiKey:"",models:["glm-4.5","glm-4-air"],defaultModel:"glm-4.5"},{id:"siliconflow",name:"SiliconFlow",enabled:!1,baseUrl:"https://api.siliconflow.cn/v1",apiPath:"",apiKey:"",models:["deepseek-ai/DeepSeek-V3","deepseek-ai/DeepSeek-R1","Qwen/Qwen2.5-72B-Instruct"],defaultModel:"deepseek-ai/DeepSeek-V3"}],Pa={openai:["gpt-5.4","gpt-5.4-mini","gpt-4.1","gpt-4.1-mini","o4-mini"],openrouter:["openai/gpt-5","openai/gpt-4.1","anthropic/claude-sonnet-4.5","anthropic/claude-opus-4.1","anthropic/claude-3.7-sonnet","anthropic/claude-3.5-sonnet","anthropic/claude-3.5-haiku","google/gemini-2.5-pro","google/gemini-2.5-flash","deepseek/deepseek-chat","deepseek/deepseek-r1","qwen/qwen-max"],aggregate:["openai/gpt-5","gpt-5","gpt-4.1","anthropic/claude-sonnet-4.5","anthropic/claude-opus-4.1","anthropic/claude-3.7-sonnet","claude-sonnet-4-5","claude-opus-4-1","claude-3-7-sonnet-latest","google/gemini-2.5-pro","google/gemini-2.5-flash","deepseek/deepseek-chat","qwen/qwen-max"],anthropic:["claude-opus-4-5","claude-sonnet-4-5","claude-haiku-4-5","claude-opus-4-1","claude-sonnet-4-0","claude-3-7-sonnet-latest","claude-3-5-haiku-latest"],gemini:["gemini-2.5-pro","gemini-2.5-flash","gemini-2.0-flash","gemini-1.5-pro"],deepseek:["deepseek-chat","deepseek-reasoner"],zhipu:["glm-4.5","glm-4-air","glm-4-flash"],siliconflow:["deepseek-ai/DeepSeek-V3","deepseek-ai/DeepSeek-R1","Qwen/Qwen2.5-72B-Instruct","Qwen/Qwen2.5-32B-Instruct","THUDM/GLM-4-9B-0414"]},In=new Set(["aiInterface","defaultModels","modelSlot","providerCatalog","providerEditor","promptEditor","themeSettings","accountSettings","memoryService","backendSync","exportSettings","mcpLibrary"]),Wi=mt,Zi=Qt,er=Yt,qa=sn;n.viewStack=n.viewStack||[],n.activeModelSlot=n.activeModelSlot||"chat",n.activeModelSlotContext=n.activeModelSlotContext||"global",n.activeModelProviderId=n.activeModelProviderId||"",n.providerDraftId=n.providerDraftId||null,n.providerAdvancedOpen=!!n.providerAdvancedOpen,n.providerEditorDraft=n.providerEditorDraft||null,n.providerModelMenuOpen=!!n.providerModelMenuOpen,n.providerModelSyncingId=n.providerModelSyncingId||"",n.providerModelSyncStatus=n.providerModelSyncStatus&&typeof n.providerModelSyncStatus=="object"?n.providerModelSyncStatus:{},n.providerKeyVisible=!!n.providerKeyVisible,n.modelSlotMenuOpen=!!n.modelSlotMenuOpen,n.providerSearch=n.providerSearch||"",n.activePromptSlot=n.activePromptSlot||"summary";function U(e){const t=String(e||"").trim();return t?t==="ocr"?"vision":t==="title"?"summary":t:"chat"}n.aiSettingsSaving=!1,n.memoryServiceEntries=Array.isArray(n.memoryServiceEntries)?n.memoryServiceEntries:[],n.memoryServiceLoading=!!n.memoryServiceLoading,n.slotVendorGroupOpen=n.slotVendorGroupOpen&&typeof n.slotVendorGroupOpen=="object"?n.slotVendorGroupOpen:{},n.providerModelVendorOpen=n.providerModelVendorOpen&&typeof n.providerModelVendorOpen=="object"?n.providerModelVendorOpen:{};function tr(){return"/chat/completions"}function Ze(e,{allowEmpty:t=!1}={}){const a=String(e||"").trim();return a?a.startsWith("/")?a:`/${a}`:t?"":tr()}function nr(e={}){return Ze(e.apiPath||e.api_path||"",{allowEmpty:!1})}function et(e={}){const t=Ze(e.apiPath||e.api_path||"",{allowEmpty:!0});return{...e,baseUrl:e.baseUrl||e.base_url||"",apiKey:e.apiKey||e.api_key||"",apiPath:t,api_path:t,models:J(e.models),defaultModel:L(e.defaultModel||e.default_model||"")}}function L(e){if(typeof e!="string")return"";const t=e.trim().replace(/\s+/g," ");return!t||t.length>180||/[<>]/.test(t)||/<\/?[a-z][\s\S]*>/i.test(t)||/<!doctype|<html|<\/div|<\/body/i.test(t)||/[\u0000-\u001f\u007f]/.test(t)?"":t}function J(e){const t=Array.isArray(e)?e:[],a=new Set,o=[];return t.forEach(i=>{const r=typeof i=="string"?i:i&&typeof i=="object"?i.id||i.name||i.model||i.slug:"",s=L(r),c=s.toLowerCase();s&&!a.has(c)&&(a.add(c),o.push(s))}),o}function _n(e={}){const t=String(e.id||"").toLowerCase(),a=String(e.name||"").toLowerCase(),o=String(e.baseUrl||e.base_url||"").toLowerCase();return t.includes("openrouter")||a.includes("openrouter")||o.includes("openrouter.ai")?"openrouter":t.includes("jiushi")||a.includes("玖时")||o.includes("jiushi.xin")?"aggregate":t.includes("silicon")||a.includes("silicon")||o.includes("siliconflow")?"siliconflow":t.includes("deepseek")||a.includes("deepseek")||o.includes("deepseek")?"deepseek":t.includes("anthropic")||t.includes("claude")||a.includes("anthropic")||a.includes("claude")||o.includes("anthropic.com")?"anthropic":t.includes("gemini")||a.includes("gemini")||o.includes("generativelanguage")?"gemini":t.includes("zhipu")||a.includes("智谱")||o.includes("bigmodel")?"zhipu":t.includes("openai")||a.includes("openai")||o.includes("openai.com")?"openai":t||"custom"}function ar(e={}){const t=_n(e);if(t==="aggregate"||t==="openrouter")return!0;if(["openai","anthropic","gemini","deepseek","zhipu","siliconflow"].includes(t))return!1;const a=String(e.baseUrl||e.base_url||"").toLowerCase();return a?!/(openai\.com|anthropic\.com|generativelanguage|deepseek\.com|bigmodel\.cn|siliconflow\.cn)/.test(a):!1}function Mn(e={}){const t=_n(e),a=Pa[t]||[],o=ar(e)?Pa.aggregate:[];return J([...a,...o])}function or(e=""){const t=String(e||"").trim();if(!t)return"";const a=t.slice(-4);return`${t.startsWith("sk-")?"sk-":""}••••${a}`}function _e(e,t,a){const o=String(e||n.providerDraftId||"current");n.providerModelSyncStatus[o]={type:t,message:a}}function ir(e,t="模型"){const a=L(e);if(!a)throw new Error(`${t} 不是合法模型 ID，不能包含 HTML、控制字符或过长内容`);return a}function xn(e={}){const t={...e||{}};return t.model&&(t.model=L(t.model)),t.providerId&&(t.providerId=String(t.providerId||"").trim()),t}function Me(){return{providers:Ji.map(e=>et({...e,models:[...e.models]})),defaultModels:{chat:{providerId:"openai",model:"gpt-5.4",useChatModel:!1},summary:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},vision:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},translate:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},consciousness:{providerId:"openai",model:"gpt-5.4-mini",useChatModel:!1},voice:{provider:"",service_url:"",base_url:"",voice_id:"",speaker:"",emotion:"",speed:1,format:""}},defaultPrompts:{chat:"Respond naturally, stay consistent with the current role and context, and keep the tone warm and clear.",summary:"Write a concise conversation summary with key facts, action items, and follow-ups.",translate:"Translate the content accurately while preserving tone and formatting when possible.",vision:"Extract visible text from the image and explain key visual information clearly.",consciousness:"Review recent context, infer useful next-step thoughts, and keep the result concise and actionable."},mcpLibrary:eo()}}function z(){if(!n.globalSettings.aiSettings)n.globalSettings.aiSettings=Me();else{const e=n.globalSettings.aiSettings;e.defaultModels=e.defaultModels||{},e.defaultPrompts=e.defaultPrompts||{},e.providers=Array.isArray(e.providers)?e.providers:[];const t=new Map(e.providers.map(a=>[a.id,et(a)]));Me().providers.forEach(a=>{t.has(a.id)||t.set(a.id,a)}),e.providers=[...t.values()],e.defaultModels.ocr&&!e.defaultModels.vision&&(e.defaultModels.vision={...e.defaultModels.ocr}),e.defaultPrompts.ocr&&!e.defaultPrompts.vision&&(e.defaultPrompts.vision=e.defaultPrompts.ocr),delete e.defaultModels.ocr,delete e.defaultPrompts.ocr,delete e.defaultModels.title,delete e.defaultPrompts.title,Object.entries(Me().defaultModels).forEach(([a,o])=>{e.defaultModels[a]||(e.defaultModels[a]={...o}),a!=="voice"&&(e.defaultModels[a]=xn(e.defaultModels[a]))}),Object.entries(Me().defaultPrompts).forEach(([a,o])=>{typeof e.defaultPrompts[a]!="string"&&(e.defaultPrompts[a]=o)})}return n.globalSettings.aiSettings}function rr(e={}){const t=Me(),a={...e||{}};a.defaultModels?.ocr&&!a.defaultModels?.vision&&(a.defaultModels={...a.defaultModels,vision:a.defaultModels.ocr}),a.defaultPrompts?.ocr&&!a.defaultPrompts?.vision&&(a.defaultPrompts={...a.defaultPrompts,vision:a.defaultPrompts.ocr});const o={providers:t.providers,defaultModels:{...t.defaultModels},defaultPrompts:{...t.defaultPrompts},mcpLibrary:{...t.mcpLibrary,tools:[...t.mcpLibrary?.tools||[]]}};if(Array.isArray(a.providers)&&a.providers.length){const i=new Map(t.providers.map(r=>[r.id,r]));a.providers.forEach(r=>{const s=et(r);i.set(s.id,{...i.get(s.id),...s,models:Array.isArray(s.models)&&s.models.length?s.models:i.get(s.id)?.models||[]})}),o.providers=[...i.values()]}a.defaultModels&&Object.keys(o.defaultModels).forEach(i=>{if(a.defaultModels[i]){const r={...o.defaultModels[i],...a.defaultModels[i]};o.defaultModels[i]=i==="voice"?r:xn(r)}}),a.defaultPrompts&&Object.keys(o.defaultPrompts).forEach(i=>{typeof a.defaultPrompts[i]=="string"&&(o.defaultPrompts[i]=a.defaultPrompts[i])}),a.mcpLibrary&&Array.isArray(a.mcpLibrary.tools)&&(o.mcpLibrary={...o.mcpLibrary,...a.mcpLibrary,tools:a.mcpLibrary.tools.map(fe)}),n.globalSettings.aiSettings=o,typeof a.consciousnessLoop=="boolean"&&(n.globalSettings.consciousnessLoop=a.consciousnessLoop),Lt()}function Lt(){const e=z(),t=e.defaultModels.chat,a=e.providers.find(o=>o.id===t.providerId);n.globalSettings.defaultModel=L(t.model)||Me().defaultModels.chat.model,n.globalSettings.provider=a?.name||"OpenAI"}function W(e){return z().providers.find(t=>t.id===e)}function Cn(e=n.providerDraftId){const t=et(W(e)||{id:e||`custom_${Date.now()}`,name:"",enabled:!0,baseUrl:"",apiPath:"",apiKey:"",models:[],defaultModel:""}),a=J(t.models),o=J([...a,...Mn(t)]).map(tt);return{...t,models:a,_allModels:o,_selectedModelIds:new Set(a),_apiKeyDirty:!1}}function R(){return(!n.providerEditorDraft||n.providerEditorDraft.id!==n.providerDraftId)&&(n.providerEditorDraft=Cn()),n.providerEditorDraft}function La(e="",t=[]){const a=String(e||"").trim().toLowerCase(),o=J(t);return a?o.filter(i=>String(i||"").toLowerCase().includes(a)):o}function Da(e){const t=String(e||"").toLowerCase();return/deepseek/.test(t)?"DeepSeek":/\bglm\b|chatglm/.test(t)?"GLM":/\bqwen\b|qwq/.test(t)?"Qwen":/\bgpt[-\d]|^gpt|^o[134][-\d]|text-davinci|text-curie/.test(t)?"OpenAI":/claude/.test(t)?"Anthropic":/gemini|gemma/.test(t)?"Google":/\bllama\b|meta-llama/.test(t)?"Meta":/mistral|mixtral|codestral/.test(t)?"Mistral":/\byi[-/_]/.test(t)?"01.AI":/moonshot|kimi/.test(t)?"Moonshot":/hunyuan/.test(t)?"Hunyuan":/ernie|wenxin/.test(t)?"ERNIE":/doubao/.test(t)?"Doubao":/baichuan/.test(t)?"Baichuan":/spark/.test(t)?"Spark":/internlm/.test(t)?"InternLM":"Other"}function Oa(e){const t=String(e||"").toLowerCase(),a=["chat","text"];return/vl\b|vision|visual|\bvision\b|-v\d|\bimg\b/.test(t)&&a.push("vision"),/reason|r1\b|think\b|cot\b/.test(t)&&a.push("reasoning"),/image|draw|flux|paint|artist|diffusion/.test(t)&&a.push("image"),a.push("tools"),a}function tt(e){const t=String(e||"").trim();return{id:t,name:t,vendor:Da(t),capabilities:Oa(t)}}const sr={chat:"瀵硅瘽",text:"鏂囨湰",reasoning:"鎺ㄧ悊",tools:"宸ュ叿璋冪敤",vision:"瑙嗚",image:"鐢熷浘"},cr=["reasoning","tools","vision","image"],ur={reasoning:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path d="M5 .5A3 3 0 0 0 2.8 5.9l.2.3V8h4V6.2l.2-.3A3 3 0 0 0 5 .5zm-1.2 8h2.4v.5c0 .28-.22.5-.5.5H4.3a.5.5 0 0 1-.5-.5V8.5z"/></svg>',tools:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path d="M7.5 1a2 2 0 0 0-1.86 2.73L1.2 8.16a.6.6 0 0 0 .84.84l4.43-4.44A2 2 0 1 0 7.5 1zm0 3a1 1 0 1 1 0-2 1 1 0 0 1 0 2z"/></svg>',vision:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M5 2C2.5 2 .8 5 .8 5S2.5 8 5 8s4.2-3 4.2-3S7.5 2 5 2zm0 4.5A1.5 1.5 0 1 1 5 3.5a1.5 1.5 0 0 1 0 3z"/></svg>',image:'<svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M1.5 1A.5.5 0 0 0 1 1.5v7a.5.5 0 0 0 .5.5h7a.5.5 0 0 0 .5-.5v-7A.5.5 0 0 0 8.5 1h-7zM2 8l2-2.5 1.3 1.7 1.7-2.2L9 8H2zm.8-4.3a.7.7 0 1 0 1.4 0 .7.7 0 0 0-1.4 0z"/></svg>'};function lr(e){return(Array.isArray(e?.capabilities)?e.capabilities:Oa(e?.name||"")).filter(a=>cr.includes(a)).map(a=>`<span class="model-cap-badge cap-${a}" title="${sr[a]||a}">${ur[a]||a}</span>`).join("")}function Ra(e="",t=[]){const a=String(e||"").trim();return!Array.isArray(t)||!t.length?"还没有已同步模型，仍可手动输入并保存。":a?t.some(i=>String(i).toLowerCase()===a.toLowerCase())?"已匹配到已同步列表中的模型。":"当前模型不在已同步列表中，可继续手动保存。":`已同步 ${t.length} 个模型，可搜索或展开列表选择。`}function An(e="",t=[]){const a=String(e||"").trim();return!Array.isArray(t)||!t.length?"当前供应商还没有同步模型，可切换供应商或先同步。":a?t.some(i=>String(i).toLowerCase()===a.toLowerCase())?"已匹配到当前供应商模型。":"当前输入不在同步列表中。":`已同步 ${t.length} 个模型，可搜索或展开列表选择。`}function xe(){const e=R(),t=document.getElementById("provider-default-model-input"),a=document.getElementById("provider-default-model-menu"),o=document.getElementById("provider-default-model-hint");if(!a||!o)return;const i=t?.value||e.defaultModel||"",r=J([...Array.isArray(e.models)?e.models:[],...Array.isArray(e._allModels)?e._allModels.map(c=>c?.id||c?.name||""):[]]),s=La(i,r);if(o.textContent=Ra(i,r),!n.providerModelMenuOpen){a.innerHTML="",a.classList.remove("open");return}a.classList.add("open"),a.innerHTML=s.length?s.map((c,l)=>`
        ...
        `).join(""):'<div class="provider-model-empty">没有获取到模型，仍可手动输入保存。</div>'}function A(e){return z().defaultModels[U(e)]}function dr(){const e=n.activeModelSlot,t=n.activeModelSlotContext==="contact",a=b(n.currentContactId)||n.contacts[0],o=t?{providerId:a?.settings?.modelProviderId||n.activeModelProviderId||A("chat")?.providerId||"openai",model:e==="consciousness"?a?.settings?.loopModel||"":a?.settings?.model||""}:Dt(e),i=W(o?.providerId)||W(A("chat")?.providerId);return{slot:o,provider:i,models:i?.models||[]}}function nt(){const e=document.getElementById("model-slot-menu"),t=document.getElementById("model-slot-hint"),a=document.getElementById("model-slot-input");if(!e||!t)return;const{slot:o,models:i}=dr(),r=a?.value||o?.model||"";t.textContent=An(r,J(i)),e.innerHTML="",e.classList.remove("open")}function Dt(e){return A(U(e))}function je(e){const t=U(e);return{chat:"聊天模型",summary:"摘要模型",vision:"Vision 模型",translate:"翻译模型",consciousness:"意识循环模型",voice:"语音模型"}[t]||t}function Ot(e){const t=U(e);return{chat:"全局默认使用的聊天模型。",summary:"用于生成对话摘要，推荐选择便宜且稳定的模型。",vision:"用于识图、OCR 与截图分析的统一入口。",translate:"用于翻译消息内容，推荐选择速度快的模型。",consciousness:"用于意识循环、主动思考与相关后台能力。",voice:"用于文本转语音，读取语音服务地址与 voice ID。"}[t]||""}function Va(e){const t=U(e),a=Dt(t);if(t==="voice"){if(!a)return"未设置";const i=a.provider||"语音服务",r=a.voice_id||a.voiceId||"未设置";return`${i} / ${r}`}const o=W(a?.providerId);return a?`${o?.name||"未设置"} / ${a.model||"未设置"}`:"未设置"}function pr(e){return z().defaultPrompts?.[U(e)]||""}function mr(e){const t=U(e);return{chat:g("comment"),summary:g("file"),vision:g("search"),translate:g("chatArrow"),consciousness:g("history"),voice:g("mic")}[t]||g("file")}function fr(e){const t=U(e);return t!=="chat"&&t!=="voice"}function gr(e){return`
      <article class="default-model-card">
        <div class="default-model-head">
          <div class="default-model-icon">${mr(e)}</div>
          <div class="default-model-copy">
            <strong>${d(je(e))}</strong>
            <p>${d(Ot(e))}</p>
          </div>
          ${fr(e)?`<button class="model-gear-btn" data-action="open-prompt-editor" data-slot="${e}" aria-label="提示词设置">${g("settings")}</button>`:'<span class="header-spacer"></span>'}
        </div>
    ...
    `}function hr(){const e=U(n.activePromptSlot),t=pr(e);return`
    ...
    `}function br(){const e=[{id:"奶油粉",key:"rose",desc:"柔和粉白"},{id:"云雾灰",key:"mist",desc:"冷淡浅灰"},{id:"奶油杏",key:"cream",desc:"暖调米白"}],t=[{id:"windowsill",key:"windowsill",name:"窗台",desc:"鼠尾草·陶土·亚麻 · 冷静工具感"},{id:"tape",key:"tape",name:"磁带",desc:"磨砂玻璃·钓色·等宽字 · 软件诚实"}],a=n.globalSettings.theme,o=Ln.includes(a);return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>全局配色</h3>
    ...
    `}function vr(){const e=n.accountProfile||{};return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>我的账号</h3>
    ...
    `}function yr(e){const t=Math.max(0,Math.min(100,Number(e)||0)),a=t>60?"#c9908a":t>30?"#c8a07a":"#b0b0b8";return`<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;color:${a};">
          ...
          ${t>0?`热度 ${t}`:""}
        </span>`}function wr(){const e=b(n.currentContactId)||n.contacts[0],t=Array.isArray(n.memoryServiceEntries)?n.memoryServiceEntries:[],a=Array.isArray(n.memoryCandidates)?n.memoryCandidates:[],o=n.memoryServiceSort||"updated_at",i=[{key:"updated_at",label:"最新"},{key:"importance",label:"最重要"},{key:"temperature",label:"有温度"}];return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>记忆服务</h3>
    ...
    `}function Tn(){return String(n.currentContactId||b(n.currentContactId)?.id||"default").trim()||"default"}async function Ce(e=Tn(),{silent:t=!0}={}){const a=String(e||"").trim();if(a){n.memoryServiceLoading=!0,u();try{const o=n.memoryServiceSort||"updated_at",i=new URLSearchParams({agent_id:a,sort_by:o,order:"desc",limit:"100"}),[r,s]=await Promise.all([fetch(`${$}/api/memories?${i.toString()}`),fetch(`${$}/api/consciousness/memory-candidates?agent_id=${encodeURIComponent(a)}&limit=20`)]);if(!r.ok)throw new Error(`HTTP ${r.status}`);const c=await r.json().catch(()=>({}));if(n.memoryServiceEntries=Array.isArray(c?.memories)?c.memories:[],s.ok){const l=await s.json().catch(()=>({}));n.memoryCandidates=Array.isArray(l?.candidates)?l.candidates:[]}}catch(o){console.warn("[memory service] load failed",o),t||(n.toast="记忆加载失败",window.setTimeout(()=>{n.toast="",u()},1200))}finally{n.memoryServiceLoading=!1,u()}}}async function $r(e){const t=Tn();try{const a=await fetch(`${$}/api/consciousness/memory-candidates/${encodeURIComponent(e)}/promote?agent_id=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"});if(!a.ok)throw new Error(`HTTP ${a.status}`);n.memoryCandidates=(n.memoryCandidates||[]).filter(o=>String(o.id)!==String(e)),n.toast="✓ 已采纳为正式记忆",window.setTimeout(()=>{n.toast="",u()},1800),await Ce(t,{silent:!0})}catch(a){console.warn("[memory] promote failed",a)}}async function Sr(e){try{const t=await fetch(`${$}/api/consciousness/memory-candidates/${encodeURIComponent(e)}`,{method:"DELETE"});if(!t.ok)throw new Error(`HTTP ${t.status}`);n.memoryCandidates=(n.memoryCandidates||[]).filter(a=>String(a.id)!==String(e)),u()}catch(t){console.warn("[memory] dismiss failed",t)}}function Ba(e=null){const t=e||{},a=window.prompt("记忆内容",String(t.raw_content||t.content||"").trim());if(a===null)return null;const o=window.prompt("分层 / category（core_profile / recent_pending / deep / ephemeral）",String(t.category||"recent_pending"));if(o===null)return null;const i=window.prompt("可见范围（private / shared / public）",String(t.visibility||"private"));if(i===null)return null;const r=window.prompt("重要度（1-5）",String(t.importance??3));if(r===null)return null;const s=window.prompt("过期时间 ISO（可留空）",String(t.expires_at||""));return s===null?null:{agent_id:Tn(),content:String(a||"").trim(),raw_content:String(a||"").trim(),category:String(o||"").trim()||"recent_pending",visibility:String(i||"").trim()||"private",importance:Math.max(1,Math.min(5,Number(r)||3)),expires_at:String(s||"").trim()||null}}async function kr(){const e=Ba();if(!e||!e.content)return;const t=await fetch(`${$}/api/memories`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)}),a=await t.json().catch(()=>({}));if(!t.ok)throw new Error(a?.detail||`HTTP ${t.status}`)}async function Ir(e){const t=n.memoryServiceEntries.find(r=>String(r.id)===String(e));if(!t)return;const a=Ba(t);if(!a||!a.content)return;const o=await fetch(`${$}/api/memories/${encodeURIComponent(e)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}),i=await o.json().catch(()=>({}));if(!o.ok)throw new Error(i?.detail||`HTTP ${o.status}`)}async function _r(e){if(!window.confirm("删除这条记忆？"))return;const t=await fetch(`${$}/api/memories/${encodeURIComponent(e)}`,{method:"DELETE"}),a=await t.json().catch(()=>({}));if(!t.ok)throw new Error(a?.detail||`HTTP ${t.status}`)}function Mr(){const e=Xe();return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>同步后端</h3>
    ...
    `}function xr(){const e=n.globalSettings;return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel compact-panel">
          <h3>导出格式</h3>
          <div class="theme-choice-list">
    ...
    `}function Z(e,t){n.viewStack.push(n.currentView),typeof t=="function"&&t(),n.currentView=e,u()}function Cr(){n.currentView=n.viewStack.pop()||"settings",u()}async function Ar(){try{const e=await fetch(`${$}/api/settings/ai`);if(!e.ok)return;const t=await e.json();rr(t.settings?.aiSettings||t.settings?.ai||t.settings?.ai_settings||t.settings||{}),u()}catch(e){console.warn("[ai settings] load failed",e)}}async function za({silent:e=!0}={}){try{const t=new URLSearchParams({viewer_type:"user",viewer_id:"me"}),a=await fetch(`${$}/api/moments?${t.toString()}`);if(!a.ok){if(!e)throw new Error(`HTTP ${a.status}`);return}const o=await a.json().catch(()=>({}));if(!Array.isArray(o?.moments))return;o.moments.length>0&&(n.moments=va(n.moments,o.moments),k(120)),u()}catch(t){console.warn("[moments] load failed",t),e||(n.toast="朋友圈加载失败",u(),window.setTimeout(()=>{n.toast="",u()},1400))}}async function Tr(e){const t=await fetch(`${$}/api/moments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)}),a=await t.json().catch(()=>({}));if(!t.ok)throw new Error(a?.detail||`HTTP ${t.status}`);return P(a?.moment||e)}async function Er(e,t){const a=await fetch(`${$}/api/moments/${encodeURIComponent(e)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),o=await a.json().catch(()=>({}));if(!a.ok)throw new Error(o?.detail||`HTTP ${a.status}`);return o}async function Pr(e,t,a){const o=new URLSearchParams({author_type:String(t||"user"),author_id:String(a||"me")}),i=await fetch(`${$}/api/moments/${encodeURIComponent(e)}?${o.toString()}`,{method:"DELETE"}),r=await i.json().catch(()=>({}));if(!i.ok)throw new Error(r?.detail||`HTTP ${i.status}`);return r}async function qr(e,t){const a=await fetch(`${$}/api/moments/${encodeURIComponent(e)}/like`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({actor_type:t.author_type,actor_id:t.author_id,actor_name:t.author_name})}),o=await a.json().catch(()=>({}));if(!a.ok)throw new Error(o?.detail||`HTTP ${a.status}`);return P(o?.moment||{})}async function Lr(e,t,a){const o=await fetch(`${$}/api/moments/${encodeURIComponent(e)}/comments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({actor_type:t.author_type,actor_id:t.author_id,actor_name:t.author_name,text:a})}),i=await o.json().catch(()=>({}));if(!o.ok)throw new Error(i?.detail||`HTTP ${o.status}`);return P(i?.moment||{})}async function Dr(e,t){const a=n.currentContactId||"",o=(t||"").trim()||null,i={agentId:a};if(e==="impression")i.impression=o;else if(e==="relationshipProgress")i.relationshipProgress=o;else if(e==="likesSummary")i.likesSummary=o;else return;try{n.toast="保存中…",u();const r=await fetch(`${$}/api/companion-state/summary`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!r.ok)throw new Error(`HTTP ${r.status}`);const s=await r.json().catch(()=>({}));n.companionState=Oe(s?.state||n.companionState),n.toast="已保存",u(),window.setTimeout(()=>{n.toast="",u()},1200)}catch(r){console.warn("[insight save]",r),n.toast="保存失败",u(),window.setTimeout(()=>{n.toast="",u()},1400)}}async function pe(e=n.currentContactId,{silent:t=!0}={}){try{const a=String(e||"").trim(),o=a?`?agent_id=${encodeURIComponent(a)}`:"",i=await fetch(`${$}/api/companion-state${o}`);if(!i.ok){if(!t)throw new Error(`HTTP ${i.status}`);return}const r=await i.json().catch(()=>({}));n.companionState=Oe(r?.state||{}),u()}catch(a){console.warn("[companion state] load failed",a),t||(n.toast="状态读取失败",u(),window.setTimeout(()=>{n.toast="",u()},1200))}}async function at(e,{silent:t=!0}={}){const a=String(e||"").trim();if(!a)return"";try{const o=await fetch(`${$}/api/agents/${encodeURIComponent(a)}/persona`);if(!o.ok){if(!t)throw new Error(`HTTP ${o.status}`);return""}const i=await o.json().catch(()=>({})),r=b(a);return r&&(r.persona=String(i?.persona||""),n.currentView==="contactSettings"&&n.currentContactId===a&&u()),String(i?.persona||"")}catch(o){return console.warn("[agent persona] load failed",o),t||(n.toast="浜鸿璇诲彇澶辫触",u(),window.setTimeout(()=>{n.toast="",u()},1200)),""}}async function Or(e,t){const a=String(e||"").trim();if(a)try{await fetch(`${$}/api/agents/${encodeURIComponent(a)}/persona`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({persona:String(t||"")})})}catch(o){console.warn("[agent persona] save failed",o)}}function Rr(e,t,a=260){const o=String(e||"").trim();if(!o)return;ve.has(o)&&clearTimeout(ve.get(o));const i=window.setTimeout(()=>{ve.delete(o),Or(o,t)},a);ve.set(o,i)}async function Ha({silent:e=!0}={}){try{const t=await fetch(`${$}/api/mcp/library`);if(!t.ok){if(!e)throw new Error(`HTTP ${t.status}`);return}const a=await t.json();if(!Array.isArray(a.tools))return;const o=z(),i=a.tools.map(fe).filter(r=>$e(r.id));o.mcpLibrary={...o.mcpLibrary||{},tools:i},D(),u()}catch(t){console.warn("[mcp library] load failed",t),e||(n.toast="同步 MCP 工具失败",u(),window.setTimeout(()=>{n.toast="",u()},1300))}}async function D(){Lt();const e=z();e.providers=(e.providers||[]).map(et),Object.keys(e.defaultModels||{}).forEach(t=>{t!=="voice"&&(e.defaultModels[t]=xn(e.defaultModels[t]))}),n.aiSettingsSaving=!0;try{await fetch(`${$}/api/settings/ai`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({settings:{...n.globalSettings,aiSettings:e}})})}catch(t){console.error("[ai settings] save failed",t)}finally{n.aiSettingsSaving=!1}}function Vr(){const e=z(),t=(e.mcpLibrary?.tools||[]).filter(a=>a.enabled!==!1).length;return`
      <section class="settings-page page-block ai-settings-page">
        <div class="settings-group glass-frost ai-panel">
          <h3>AI 接口</h3>
    ...
    `}function zr(){return`
      <section class="settings-page page-block ai-settings-page">
        <div class="default-model-list">
          ${["chat","summary","vision","translate","consciousness","voice"].map(t=>gr(t)).join("")}
        </div>
      </section>
    ...
    `}function jr(){const e=n.providerSearch.trim().toLowerCase(),t=z().providers.filter(a=>!e||a.name.toLowerCase().includes(e)||a.id.toLowerCase().includes(e)).sort((a,o)=>{const i=+!!o.enabled-+!!a.enabled;return i!==0?i:String(a.name||a.id||"").localeCompare(String(o.name||o.id||""),"zh-Hans-CN")});return`
      <section class="settings-page page-block ai-settings-page">
    ...
    `}function Fr(e){const t=new Set,a=(Array.isArray(e._allModels)?e._allModels:[]).filter(h=>{const v=L(h?.id||h?.name||""),S=v.toLowerCase();return!v||t.has(S)?!1:(t.add(S),!0)}),o=e._selectedModelIds instanceof Set?e._selectedModelIds:new Set(e._selectedModelIds||[]),i=o.size,r={};for(const h of a){const v=h.vendor||"Other";r[v]||(r[v]=[]),r[v].push(h)}const s=["OpenAI","Anthropic","Google","DeepSeek","Qwen","GLM","Meta","Mistral","Moonshot","Doubao","ERNIE","Hunyuan","Baichuan","Spark","01.AI","InternLM","Other"],c=[...new Set([...s.filter(h=>r[h]),...Object.keys(r)])],l=a.map(h=>h.id),p=l.length>0&&l.every(h=>o.has(h)),m=c.map(h=>{const v=r[h]||[],S=!!n.providerModelVendorOpen[h],I=v.filter(_=>o.has(_.id)).length,T=v.length>0&&v.every(_=>o.has(_.id)),x=S?`
          <div class="vendor-group-body">
            ${v.map(_=>{const C=o.has(_.id),V=a.findIndex(oe=>oe.id===_.id);return`
              <div class="pool-model-row">
                ...
                <span class="pool-model-caps">${lr(_)}</span>
        ...
        <div class="vendor-group">
          <div class="vendor-group-head">
            <button class="vendor-group-toggle" data-action="toggle-provider-vendor-group" data-vendor="${d(h)}" type="button">
              ...
              <span class="vendor-group-chevron${S?" open":""}">${g("chevron")}</span>
            </button>
      ...
      <div class="prov-model-pool">
      ...
      </div>`}function Na(e,t,a){const o=J(a),i=L(t?.model||"");return o.length?o.map((r,s)=>`
        ...
        `).join(""):'<div class="model-choice-empty">当前供应商还没有可选模型，请先在“模型供应商”页同步并保存。</div>'}function Ur(){const e=R(),t=Ze(e.apiPath||e.api_path||"",{allowEmpty:!0}),a=nr(e),o=!!n.providerAdvancedOpen||!!t,i=n.providerModelSyncStatus?.[e.id],r=or(e.apiKey||""),s=!!e._apiKeyDirty,c=s?String(e.apiKey||""):r,l=s?n.providerKeyVisible?"text":"password":"text",p=s?n.providerKeyVisible?"隐藏":"显示":r?"更换":"显示";return`
    ...
    `}async function Kr(){const e=R();if(n.providerModelSyncingId)return;const t=document.getElementById("provider-base-input")?.value?.trim()||"",a=document.getElementById("provider-key-input"),o=e._apiKeyDirty?a?.value?.trim()||"":e.apiKey||"",i=_n({...e,baseUrl:t});if(!t){const r=Mn(e);if(!r.length){_e(e.id,"error","请先填写 Base URL 再同步模型"),u();return}const s=r.map(tt);e._allModels=s,s.forEach(c=>{n.providerModelVendorOpen[c.vendor]=!0}),_e(e.id,"success",`已载入内置列表 ${r.length} 个模型`),u();return}n.providerModelSyncingId=e.id||n.providerDraftId||"syncing",_e(e.id,"muted","正在同步模型..."),u();try{const r=await fetch(`${$}/api/settings/ai/discover-models`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({provider_id:e.id||i,provider_name:e.name||"",base_url:t,api_key:o})}),s=r.headers.get("content-type")||"",c=await r.text();if(!r.ok){let x="";try{const _=s.includes("application/json")?JSON.parse(c):null;x=_?.detail||_?.message||""}catch{}throw new Error(x||`HTTP ${r.status}`)}if(!s.includes("application/json"))throw new Error("后端返回的不是 JSON，已阻止写入模型列表");let l={};try{l=JSON.parse(c||"{}")}catch{throw new Error("后端返回 JSON 解析失败，已阻止写入模型列表")}const p=J(Array.isArray(l.models)?l.models:[]),m=Mn({...e,baseUrl:t}),f=J([...p,...m]);if(!f.length){_e(e.id,"error","没有获取到模型，已保留当前默认模型和已有列表。"),u();return}const h=f.map(tt),v=new Set(f.map(x=>x.toLowerCase())),S=[...h];(e._allModels||[]).forEach(x=>{const _=L(x?.id||x?.name||"");!_||v.has(_.toLowerCase())||S.push(tt(_))}),e._allModels=S,[...new Set(h.map(x=>x.vendor))].forEach(x=>{n.providerModelVendorOpen[x]=!0}),e._selectedModelIds instanceof Set||(e._selectedModelIds=new Set(e._selectedModelIds||[])),e.models=[...e._selectedModelIds];const T=Math.max(0,f.length-p.length);_e(e.id,"success",l.is_fallback||l.source==="fallback"?`已载入内置列表 ${f.length} 个模型`:T?`已同步 ${p.length} 个模型，补充内置 ${T} 个`:`已同步 ${f.length} 个模型`),u(),xe()}catch(r){const s=String(r?.message||"同步模型失败");if(s.includes("Failed to fetch")){_e(e.id,"error","同步失败：当前前端连不上后端接口。"),u();return}_e(e.id,"error",`同步失败：${s}`),u()}finally{n.providerModelSyncingId="",u()}}mt=function(){return In.has(n.currentView)?!1:Wi()},Qt=function(){if(!In.has(n.currentView))return Zi();const t={aiInterface:"AI 接口",mcpLibrary:"MCP 工具库",themeSettings:"主题模式",accountSettings:"我的账号",memoryService:"记忆服务",backendSync:"同步后端",exportSettings:"导出格式",defaultModels:"默认模型",modelSlot:je(n.activeModelSlot),providerCatalog:"模型供应商",providerEditor:"编辑供应商",promptEditor:"提示词"},a=`chat-page-title ${n.currentView==="providerCatalog"?"provider-catalog-title":""}`.trim(),o=n.currentView==="providerCatalog"?`<button class="icon-btn ghost-circle" data-action="open-provider-editor-new" aria-label="新增供应商">${g("plus")}</button>`:'<span class="header-spacer"></span>';return`
      <header class="chat-page-header simple-header">
